...
let stdout;
try {
  stdout = execSync('npx knip --reporter json', { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024, cwd: process.cwd() });
} catch(e) {
...
if (first > -1) {
    try {
        ...
        fs.writeFileSync('knip_clean_data.json', JSON.stringify(itemsToFix, null, 2));
        console.log(`Successfully wrote ${itemsToFix.length} issues to knip_clean_data.json`);
    } catch(err) {
        console.error("Failed to parse the extracted JSON:", err.message);
    }
} else {
    console.error('No JSON found in output. Output was:', stdout.substring(0, 500));
}
