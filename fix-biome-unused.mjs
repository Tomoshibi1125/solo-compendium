...
const unusedVarIssues = issues.filter(i => i.rule === 'lint/correctness/noUnusedVariables');
const unusedImportIssues = issues.filter(i => i.rule === 'lint/correctness/noUnusedImports');
const anyIssues = issues.filter(i => i.rule === 'lint/suspicious/noExplicitAny');
  ...
  for (const issue of fileIssues) {
    ...
    if (isUsedElsewhere) {
      ...
      if (isType && !isExported) {
        ...
        const newLine = line.slice(0, symbolStart) + '_' + line.slice(symbolStart);
...
for (const issue of unusedImportIssues) {
  ...
  writeFileSync(filePath, fileLines.join('\n'), 'utf-8');
  console.log(`Fixed unused import in ${issue.file}:${issue.line} (${symbolName})`);
}
