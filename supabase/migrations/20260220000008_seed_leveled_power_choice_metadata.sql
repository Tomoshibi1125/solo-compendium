...
WITH eligible_features AS (
  SELECT
    ...
    f.level AS feature_level,
    CASE
      WHEN lower(j.name) IN ('mage','necromancer','technomancer','oracle','resonant','invoker') THEN
        CASE WHEN f.level >= 17 THEN 9 WHEN f.level >= 15 THEN 8 WHEN f.level >= 13 THEN 7 WHEN f.level >= 11 THEN 6 WHEN f.level >= 9 THEN 5 WHEN f.level >= 7 THEN 4 WHEN f.level >= 5 THEN 3 WHEN f.level >= 3 THEN 2 ELSE 1 END
      WHEN lower(j.name) IN ('crusader','stalker') THEN
        CASE WHEN f.level >= 17 THEN 5 WHEN f.level >= 13 THEN 4 WHEN f.level >= 9 THEN 3 WHEN f.level >= 5 THEN 2 WHEN f.level >= 2 THEN 1 ELSE 0 END
      WHEN lower(j.name) IN ('contractor') THEN
        CASE WHEN f.level >= 9 THEN 5 WHEN f.level >= 7 THEN 4 WHEN f.level >= 5 THEN 3 WHEN f.level >= 3 THEN 2 ELSE 1 END
      ELSE 0
    END AS max_power_level
  FROM public.compendium_job_features f
  JOIN public.compendium_jobs j ON j.id = f.job_id
  ...
  WHERE ef.max_power_level >= 1
    AND EXISTS (
      SELECT 1 FROM public.compendium_powers p
      WHERE p.power_level BETWEEN 1 AND ef.max_power_level
        AND (p.job_names @> ARRAY[ef.job_name]::text[] OR (ef.path_name IS NOT NULL AND p.path_names @> ARRAY[ef.path_name]::text[]))
    )
), eligible_groups AS (
  SELECT
    g.id AS group_id,
