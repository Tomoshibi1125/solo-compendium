-- Widen the rarity guard inside warden_grant_character_equipment to accept the
-- full 8-tier ladder now that the `rarity` enum carries epic / mythic / artifact
-- (see 20260604000000). Previously the guard cast only the original 5 labels to
-- public.rarity and NULLed everything else, so an epic/mythic/artifact item
-- delivered through this RPC silently lost its rarity on character_equipment.
--
-- This is a separate migration from the ADD VALUE statements (separate
-- transaction) so the new enum labels are committed and usable. Only the rarity
-- IN-list changed versus 20260506153000; the rest of the body is reproduced
-- verbatim. relic_tier handling is unchanged.
  ...
  LOOP
    ...
    ELSE
      INSERT INTO public.character_equipment (
        ...
        CASE
          WHEN v_item->>'rarity' IN ('common', 'uncommon', 'rare', 'very_rare', 'epic', 'legendary', 'mythic', 'artifact')
