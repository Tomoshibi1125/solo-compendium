...
import { calculateTotalChoices, getChoiceGrantDetails, type ChoiceGrant } from '../choiceCalculations';
...
describe('Choice Calculations Integration', () => {
  describe('calculateTotalChoices', () => {
    ...
    it('should handle jobs with multiple awakening features granting choices', () => {
      const jobWithMultipleFeatures = {
        ...mockDbJob,
        awakeningFeatures: [
          { name: 'Feature 1', description: 'Gain proficiency in two additional skills of your choice.', level: 1 },
          { name: 'Feature 2', description: 'Gain one additional feat of your choice.', level: 1 }
        ],
        jobTraits: [],
      };

      const result = calculateTotalChoices(jobWithMultipleFeatures, null, [], 1);
  ...
  describe('getChoiceGrantDetails', () => {
    it('should return detailed choice grants for UI display', () => {
      ...
      const expertiseGrant = details.find((g: ChoiceGrant) => g.type === 'expertise');
  ...
  describe('Pattern Matching', () => {
    it('should correctly parse written numbers in descriptions', () => {
      const testCases = [
        { desc: 'Gain proficiency in two additional skills of your choice.', expected: { type: 'skills', count: 2 } },
        { desc: 'Gain one additional feat of your choice.', expected: { type: 'feats', count: 1 } },
        { desc: 'Double proficiency on four chosen skill proficiencies.', expected: { type: 'expertise', count: 4 } },
        { desc: 'Gain proficiency in 2 additional skills of your choice.', expected: { type: 'skills', count: 2 } },
        { desc: 'Learn 3 additional feats of your choice.', expected: { type: 'feats', count: 3 } },
      ];

      for (const testCase of testCases) {
        const details = getChoiceGrantDetails({
          skill_choice_count: 0,
          awakeningFeatures: [{ name: 'Test', description: testCase.desc, level: 1 }],
          jobTraits: []
        }, null, [], 1);

        const grant = details.find((g: ChoiceGrant) => g.type === testCase.expected.type);
