...
describe("filterAccessiblePaths", () => {
	...
	it("returns only paths at or below the character level", () => {
		expect(filterAccessiblePaths(1, paths).map((p) => p.pathName)).toEqual([
			"Early Path",
		]);
		expect(filterAccessiblePaths(3, paths).map((p) => p.pathName)).toEqual([
			"Early Path",
			"Mid Path",
		]);
		expect(filterAccessiblePaths(10, paths).map((p) => p.pathName)).toEqual([
			"Early Path",
			"Mid Path",
			"Late Path",
		]);
...
describe("getFeaturesUnlockedAtLevel", () => {
	...
	it("returns only features at the exact level", () => {
		expect(
			getFeaturesUnlockedAtLevel(3, features).map((f) => f.featureName),
		).toEqual(["B", "C"]);
		expect(
			getFeaturesUnlockedAtLevel(1, features).map((f) => f.featureName),
		).toEqual(["A"]);
...
describe("filterAccessiblePowers", () => {
	...
	it("opens more powers at higher levels", () => {
		...
		expect(accessible.map((p) => p.powerName)).toEqual([
			"Cantrip",
			"Shield",
			"Fireball",
		]);
...
describe("buildLevelUpGatingSummary", () => {
	...
	it("builds correct summary for path unlock level", () => {
		const summary = buildLevelUpGatingSummary(
			3,
			"Vanguard",
			null,
			paths,
