...
import type { CharacterFeature, FeatureModifier } from '@/hooks/useCharacterFeatures';
import { sumCustomModifiers } from '@/lib/customModifiers';
import { resolveAdvantageForRoll } from '@/lib/rollAdvantage';

function buildFeature(partial: Partial<CharacterFeature> & { id: string; character_id: string; name: string }): CharacterFeature {
  return {
    id: partial.id,
...
describe('awakening + feat modifier wiring', () => {
  ...
  it('applies advantage modifiers to the global roll resolver', () => {
    const modifiers: FeatureModifier[] = [
      { type: 'advantage', value: 0, target: 'initiative', source: 'Predator Instinct' },
    ];
