...
describe("C2 retry/source hardening", () => {
	it("puts an authoritative retry gate in front of tame attempts", () => {
		expect(migration).toContain("RENAME TO resolve_companion_tame_attempt_c2_unchecked");
		expect(migration).toContain("CREATE OR REPLACE FUNCTION public.resolve_companion_tame_attempt_c2(");
		expect(migration).toContain("v_latest.outcome IN ('failure', 'invalid')");
		expect(migration).toContain("'RETRY_ADJUDICATION_REQUIRED'");
		expect(migration).toContain("v_adjudication.retry_of_attempt_id IS DISTINCT FROM v_latest.id");
	});

	it("only lets the public wrapper execute the unchecked implementation", () => {
		expect(migration).toContain("REVOKE ALL ON FUNCTION public.resolve_companion_tame_attempt_c2_unchecked(");
		expect(migration).toContain("FROM PUBLIC, anon, authenticated");
		expect(migration).toContain("RETURN public.resolve_companion_tame_attempt_c2_unchecked(");
		expect(migration).toContain("GRANT EXECUTE ON FUNCTION public.resolve_companion_tame_attempt_c2(");
	});

	it("uses the latest tame outcome so a later distinct creature can start a fresh chain after success", () => {
		expect(migration).toContain("ORDER BY attempt.created_at DESC, attempt.id DESC");
		expect(migration).toContain("LIMIT 1");
		expect(migration).not.toContain("AND attempt.outcome IN ('failure', 'invalid')\n  ORDER BY");
	});
