...
describe("C1 companion export/import identity", () => {
	it("stores portable frozen source envelopes on personal and campaign tamed rows", () => {
		...
		expect(
			migration.match(
				/ADD COLUMN IF NOT EXISTS companion_source_snapshot JSONB/g,
			),
		).toHaveLength(2);
	...
	it("prevents clients from rewriting identity or frozen source fields directly", () => {
		...
		expect(migration).toContain(
			"current_setting('app.companion_mapping_write', true) = 'on'",
		);
	});
	...
	it("keeps unknown additive row fields through the import sanitizer", () => {
		...
		const stripBlock = exportImport.slice(
			stripStart,
			stripEnd > stripStart ? stripEnd : undefined,
		);
		expect(stripBlock).toContain("delete next.id");
		expect(stripBlock).toContain("delete next.character_id");
