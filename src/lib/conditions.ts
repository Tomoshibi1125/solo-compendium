...
const CONDITIONS: Record<string, ConditionEffect> = {
	blinded: {
		name: "Blinded",
		description:
			"Can't see and automatically fails any check that requires sight.",
		effects: [
	...
	deafened: {
		...
		effects: ["Fails any ability check requiring hearing"],
		mechanical: {
	...
	frightened: {
		name: "Frightened",
		description:
			"Has disadvantage while the source of fear is in line of sight.",
		effects: [
...
export function getActiveConditionEffects(conditions: string[]): {
	...
	const activeEffects = conditions
		.map((c) => CONDITIONS[c.toLowerCase()])
		.filter(Boolean);
	...
	return {
		...
		speedModifier:
			activeEffects.find((e) => e.mechanical.speedModifier !== undefined)
				?.mechanical.speedModifier ?? null,
