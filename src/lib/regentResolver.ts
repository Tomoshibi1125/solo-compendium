...
import type { RegentRef, ActiveEffectRef } from '@/lib/domainEvents';
 ...
 */
export function toRegentEffectRefs(state: RegentRuntimeState): ActiveEffectRef[] {
...
export function canUnlockRegent(
  ...
  if (characterLevel >= regentUnlockLevel) {
    return { allowed: true, reason: `Regent unlocks at level ${regentUnlockLevel}.` };
  }
  return {
    allowed: false,
 ...
 */
export function canActivateGemini(
  unlockedRegentCount: number
): { allowed: boolean; reason: string } {
  if (unlockedRegentCount >= 2) {
    return { allowed: true, reason: 'Two or more regents unlocked — Gemini Protocol available.' };
  }
  return {
    allowed: false,
