...
export class OfflineStorageManager {
		...
		return new Promise((resolve, reject) => {
			...
			request.onupgradeneeded = (event) => {
				...
				if (!db.objectStoreNames.contains("compendium")) {
					const compendiumStore = db.createObjectStore("compendium", {
						keyPath: "id",
					});
					compendiumStore.createIndex("type", "type", { unique: false });
					compendiumStore.createIndex("searchText", "searchText", {
						unique: false,
					});
				}
				...
				if (!db.objectStoreNames.contains("diceRolls")) {
					const diceRollsStore = db.createObjectStore("diceRolls", {
						keyPath: "id",
					});
					diceRollsStore.createIndex("timestamp", "timestamp", {
						unique: false,
					});
				}
			};
	...
	async getCompendiumItem(id: string): Promise<CompendiumCacheItem | null> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = () => reject(request.error);
			request.onsuccess = () =>
				resolve((request.result as CompendiumCacheItem | null) ?? null);
		});
	}
	...
	async searchCompendium(
		...
		return new Promise((resolve, reject) => {
			...
			request.onsuccess = () => {
				const results =
					(request.result as CompendiumCacheItem[] | undefined) ?? [];
	...
	async getCharacter(id: string): Promise<CharacterCacheItem | null> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = () => reject(request.error);
			request.onsuccess = () =>
				resolve((request.result as CharacterCacheItem | null) ?? null);
		});
	}
	...
	async getUserCharacters(userId: string): Promise<CharacterCacheItem[]> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = () => reject(request.error);
			request.onsuccess = () =>
				resolve((request.result as CharacterCacheItem[]) ?? []);
		});
	}
...
export class BackgroundSyncManager {
	...
	private async processItem(item: SyncQueueItem): Promise<void> {
		const processor = this.processors.get(
			this.buildProcessorKey(item.type, item.action),
		);
		if (processor) {
