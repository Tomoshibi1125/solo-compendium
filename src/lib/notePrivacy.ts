...
export interface SecuredNote {
    ...
    category: 'journal' | 'handout' | 'campaign_note' | 'session_note' | 'character_note';
}
 ...
 */
export function canViewNote(note: SecuredNote, userId: string, isGM: boolean): boolean {
    if (isGM) return true;
 ...
 */
export function canEditNote(note: SecuredNote, userId: string, isGM: boolean): boolean {
    if (isGM) return true;
    if (note.privacy.ownerId === userId) return true;
 ...
 */
export function canSeeNoteExists(note: SecuredNote, userId: string, isGM: boolean): boolean {
    if (isGM) return true;
    if (note.privacy.ownerId === userId) return true;
