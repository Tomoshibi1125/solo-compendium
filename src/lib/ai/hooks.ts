...
export function useAIService() {
	...
	useEffect(() => {
		...
		const hasEnabledService = config.services.some(
	...
	const filterContent = useCallback(
		async (content: string) => {
			...
			try {
				...
				setError(
					err instanceof Error ? err.message : "Content filtering failed",
				);
				throw err;
			} finally {
...
export function useAIVariations() {
	...
	const createVariation = useCallback(
		async (originalContent: string, variationType: string) => {
			if (!isAvailable) {
			...
			try {
				const result = await aiService.createVariation(
					originalContent,
					variationType,
				);
				setVariation(result);
				...
				setError(
					err instanceof Error ? err.message : "Variation creation failed",
				);
				throw err;
			} finally {
				setIsCreating(false);
...
export function useAIBatchProcessor() {
	...
	const processBatch = useCallback(
		async (
			requests: Array<Omit<AIRequest, "service"> & { service?: string }>,
		) => {
			if (!isAvailable) {
