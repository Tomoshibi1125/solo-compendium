...
import type { GeminiStateRef, ActiveEffectRef } from '@/lib/domainEvents';
...
export function buildGeminiRuntimeState(
  ...
  const modifiers = resolveFusionModifiers(abilities, sovereignName ?? 'Unknown Sovereign');
 ...
 */
export function toGeminiEffectRefs(state: GeminiRuntimeState): ActiveEffectRef[] {
 ...
 */
export function getPowerMultiplierFactor(multiplierLabel: string | null): number {
