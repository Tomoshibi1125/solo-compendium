 ...
 */
export function rollCompounding(
	formula: string,
	compoundOn: number,
): RollResult {
	const roll = parseFormula(formula);
 ...
 */
export function rollPenetrating(
	formula: string,
	penetrateOn: number,
): RollResult {
	const roll = parseFormula(formula);
 ...
 */
export function applyProficiencyBonus(
	roll: RollResult,
	level: number,
): RollResult {
 ...
 */
export function applyExpertise(
	roll: RollResult,
...
export function formatRollResult(roll: RollResult): string {
	...
	if (roll.rerolledRolls && roll.rerolledRolls.length > 0) {
		const rerollStr = roll.rerolledRolls
			.map((r) => `${r.original}→${r.rerolled}`)
			.join(", ");
