export type CampaignInviteStatus = 'active' | 'expired' | 'revoked' | 'used_up' | 'unknown';

const SHARE_CODE_REGEX = /^[A-Z0-9]{6}$/;

export const normalizeInviteAccessKey = (value: string | null | undefined): string => {
  return (value ?? '').trim();
};

export const isLikelyShareCode = (value: string | null | undefined): boolean => {
...
export const buildCampaignInviteUrl = (origin: string, accessKey: string): string => {
...
export const deriveCampaignInviteStatus = (invite: {
  ...
  if (normalizedStatus === 'active' || normalizedStatus === 'expired' || normalizedStatus === 'revoked') {
...
export const isCampaignInviteJoinable = (status: CampaignInviteStatus): boolean => {
  return status === 'active' || status === 'used_up';
};

export const campaignInviteStatusLabel = (status: CampaignInviteStatus): string => {
  switch (status) {
    case 'active':
      return 'Active';
    case 'expired':
...
export const campaignInviteStatusMessage = (status: CampaignInviteStatus): string => {
  switch (status) {
    case 'active':
