...
export function isQuestActive(quest: DailyQuestInstance): boolean {
	return (
		["pending", "in_progress"].includes(quest.status) && !isQuestExpired(quest)
	);
}
...
export function calculateQuestReward(
	...
	return {
		...
		relic_shards: template.base_rewards.relic_shards
			...
			: undefined,
		experience: template.base_rewards.experience
			? Math.max(1, Math.floor(template.base_rewards.experience * multiplier))
			: undefined,
