...
import { buildPrompt, generateCacheKey, getPreset } from "./types";
...
class ArtPipelineService {
	...
	constructor() {
		...
		this.primaryBucket = this.normalizeBucket(
			import.meta.env.VITE_ART_BUCKET,
			"generated-art",
		);
		this.fallbackBucket = this.normalizeBucket(
			import.meta.env.VITE_ART_BUCKET_FALLBACK,
			"custom-tokens",
		);
		this.imageTimeoutMs = this.parseTimeout(
			import.meta.env.VITE_ART_IMAGE_TIMEOUT_MS,
			60000,
		);
		this.operationTimeoutMs = this.parseTimeout(
			import.meta.env.VITE_ART_OPERATION_TIMEOUT_MS,
			20000,
		);
	}
	...
	private async processAndSave(
		...
		try {
			...
			if (
				...
				if (import.meta.env.DEV) {
					logger.warn(
						"Art storage unavailable, saving external-only asset.",
						error,
					);
				}
			...
			throw new AppError(
				`Failed to upload art: ${this.formatStorageError(error)}`,
				"UNKNOWN",
				error,
			);
		}
		...
		try {
			...
			if (
				...
				if (import.meta.env.DEV) {
					logger.warn(
						"Art metadata storage failed; storing metadata inline only.",
						error,
					);
				}
		...
		if (storeMetadata) {
			...
			try {
				...
				if (
					...
					if (import.meta.env.DEV) {
						logger.warn(
							"Art metadata bucket missing; storing metadata inline only.",
							error,
						);
					}
				} else {
	...
	private async uploadWithFallback(
		...
		try {
			...
			if (fallback && fallback !== primary) {
				if (import.meta.env.DEV) {
					logger.warn(
						"Primary art storage bucket failed, retrying with fallback.",
						error,
					);
				}
				try {
	...
	private async withTimeout<T>(
		...
		const timeoutPromise = new Promise<T>((_, reject) => {
			timeoutId = setTimeout(
				() => reject(new AppError(message, "GENERATION_FAILED")),
				timeoutMs,
			);
		});
		try {
	...
	private getImageEndpoint(
		...
		const base = (
			import.meta.env.VITE_FREE_IMAGE_API ||
	...
	private async fetchImage(
		...
		try {
			const fetchPromise = (async () => {
				...
				if (!blob || blob.size === 0) {
					throw new AppError(
						"Image generation returned empty content",
						"GENERATION_FAILED",
					);
				}
				return blob;
			...
			const timeoutPromise = new Promise<Blob>((_, reject) => {
				timeoutId = setTimeout(() => {
					...
					reject(
						new AppError("Image generation timed out", "GENERATION_FAILED"),
					);
				}, this.imageTimeoutMs);
			});
	...
	private async saveAssetRecord(asset: ArtAsset): Promise<void> {
		try {
			const { error } = await this.withTimeout(
				supabase.from("art_assets").upsert(
					{
						id: asset.id,
