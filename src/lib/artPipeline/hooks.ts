...
export function useArtPipeline() {
	...
	const generateArt = useCallback(
		async (request: ArtRequest): Promise<GenerationResult> => {
			if (!enabled) {
				return {
					success: false,
			...
			const rawTimeout = Number.parseInt(
				import.meta.env.VITE_ART_OVERALL_TIMEOUT_MS || "",
				10,
			);
			const maxTimeoutMs =
				Number.isFinite(rawTimeout) && rawTimeout > 0 ? rawTimeout : 90000;
...
export function useArtAsset(
	...
	const createAsset = useCallback(
		async (title: string, options: Partial<ArtRequest> = {}) => {
