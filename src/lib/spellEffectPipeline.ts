...
export type EffectTarget =
    ...
    | 'str_save' | 'agi_save' | 'vit_save' | 'int_save' | 'sense_save' | 'pre_save'
    | 'spell_dc'
    | 'initiative';
...
const SPELL_EFFECT_MAP: Record<string, SpellModifier[]> = {
    'bless': [
        { target: 'attack_rolls', value: 3, type: 'bonus', description: '+1d4 (avg 2.5, rounded up)' },
        { target: 'saving_throws', value: 3, type: 'bonus', description: '+1d4 (avg 2.5, rounded up)' },
    ],
    'shield of faith': [
        { target: 'ac', value: 2, type: 'bonus' },
    ],
    'shield': [
        { target: 'ac', value: 5, type: 'bonus' },
    ],
    'haste': [
        { target: 'ac', value: 2, type: 'bonus' },
    ...
    'barkskin': [
        { target: 'ac', value: 16, type: 'set', description: 'AC cannot be less than 16' },
    ],
    'aid': [
        { target: 'hp_max', value: 5, type: 'bonus', description: '+5 HP max (scales)' },
    ],
    'heroism': [
        { target: 'hp_temp', value: 0, type: 'bonus', description: 'Gain temp HP = casting mod each turn' },
    ],
    'protection from evil and good': [
        { target: 'attack_rolls', value: 0, type: 'disadvantage', description: 'Enemy attacks have disadvantage' },
    ],
    'blur': [
        { target: 'attack_rolls', value: 0, type: 'disadvantage', description: 'Enemy attacks have disadvantage' },
    ],
    'enhance ability': [
        { target: 'ability_checks', value: 0, type: 'advantage', description: 'Chosen ability checks' },
    ],
    'longstrider': [
        { target: 'speed', value: 10, type: 'bonus' },
    ],
    'resistance': [
        { target: 'saving_throws', value: 3, type: 'bonus', description: '+1d4 (avg 2.5, rounded up)' },
    ],
    'guidance': [
        { target: 'ability_checks', value: 3, type: 'bonus', description: '+1d4 (avg 2.5, rounded up)' },
    ],
};
...
export function createActiveSpellEffect(
    ...
    const effects = overrides?.length
        ? overrides.map((o) => ({ target: 'ac' as EffectTarget, value: 0, type: 'bonus' as const, ...o }))
 ...
 */
export function spellEffectsToEngineEffects(activeSpells: ActiveSpellEffectEntry[]): SpellEngineEffect[] {
