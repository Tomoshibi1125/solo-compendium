...
const collectSourcebooksFromRpcData = (data: unknown, target: Set<string>) => {
...
export async function getAccessibleSourcebookSet(
	...
	if (authError) {
		logger.warn(
			"Failed to resolve auth state for sourcebook access:",
			authError,
		);
		return null;
	}
...
export async function filterRowsBySourcebookAccess<T>(
	...
	return filterRowsByAccessibleSourcebooks(
		rows,
		getSourcebook,
		accessibleSourcebooks,
	);
}
