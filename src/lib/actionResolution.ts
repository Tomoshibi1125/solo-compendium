...
export type ActionResolutionPayload = {
  ...
  name: string;
  source: {
    type: 'spell' | 'monster_action' | 'technique' | 'item' | 'artifact' | 'relic';
    entryId: string;
  };
  kind: ResolutionKind;
  attack?: {
...
export function resolveAttack(payload: ActionResolutionPayload, targetAC: number): ResolutionOutcome {
  if (!payload.attack) {
  ...
  if (!hit || !payload.damage) {
    return {
      ...
      attackMode: mode,
      attackD20: (attack as Record<string, any>).rolls?.length === 2 ? [(attack as Record<string, any>).rolls[0], (attack as Record<string, any>).rolls[1]] : [((attack as Record<string, any>).rolls?.[0] ?? attack.total)],
      targetAC,
      hit,
    };
  }
...
export function resolveSave(payload: ActionResolutionPayload): ResolutionOutcome {
  if (!payload.save) {
  ...
  if (!payload.damage) {
    return {
      ...
      saveMode: mode,
      saveD20: (save as Record<string, any>).rolls?.length === 2 ? [(save as Record<string, any>).rolls[0], (save as Record<string, any>).rolls[1]] : [((save as Record<string, any>).rolls?.[0] ?? save.total)],
      dc,
      success,
    };
  }
...
export function resolveHealing(payload: ActionResolutionPayload): ResolutionOutcome {
  if (!payload.healing) {
...
export function resolveDamage(payload: ActionResolutionPayload): ResolutionOutcome {
  if (!payload.damage) {
