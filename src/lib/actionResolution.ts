...
export function resolveAttack(payload: ActionResolutionPayload, targetAC: number): ResolutionOutcome {
  ...
  if (!hit || !payload.damage) {
    return {
      ...
      attackMode: mode,
      attackD20: (attack as any).rolls?.length === 2 ? [(attack as any).rolls[0], (attack as any).rolls[1]] : [((attack as any).rolls?.[0] ?? attack.total)],
      targetAC,
      hit,
    };
  }
...
export function resolveSave(payload: ActionResolutionPayload): ResolutionOutcome {
  ...
  if (!payload.damage) {
    return {
      ...
      saveMode: mode,
      saveD20: (save as any).rolls?.length === 2 ? [(save as any).rolls[0], (save as any).rolls[1]] : [((save as any).rolls?.[0] ?? save.total)],
      dc,
      success,
    };
  }
