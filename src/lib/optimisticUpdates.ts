import { useMutation, useQueryClient, type MutationFunction, type MutationKey } from '@tanstack/react-query';
 ...
 */
export function useOptimisticMutation<TData, TVariables, TContext = { previousData: any; mutationKey: MutationKey }>(
    ...
    return useMutation({
        ...
        onMutate: async (variables) => {
            ...
            queryClient.setQueryData(resolvedKey, (old: any) => optimisticUpdateFn(old, variables));
        ...
        onError: (err, newTodo, context) => {
            ...
            const typedContext = context as { previousData: any; mutationKey: MutationKey } | undefined;
            if (typedContext?.previousData && typedContext.mutationKey) {
                queryClient.setQueryData(typedContext.mutationKey, typedContext.previousData);
            }
            toast({
                title: 'Action Failed',
        ...
        onSettled: (data, error, variables, context) => {
            ...
            if (navigator.onLine) {
                const resolvedKey = (context as any)?.mutationKey || mutationKeyFn(variables as TVariables);
