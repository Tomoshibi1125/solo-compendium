...
export const CONDITION_EFFECTS: Record<string, ConditionEffect> = {
	...
	incapacitated: {
		...
		mechanicalEffects: [{ type: "incapacitated", target: "actions" }],
	},
	invisible: {
		name: "Invisible",
		description:
			"Advantage on attack rolls. Attacks against have disadvantage.",
		mechanicalEffects: [{ type: "advantage", target: "attack_rolls" }],
	},
	paralyzed: {
		name: "Paralyzed",
		description:
			"Incapacitated. Auto-fail STR and AGI saves. Attacks have advantage. Melee hits are crits.",
		mechanicalEffects: [
	...
	petrified: {
		name: "Petrified",
		description:
			"Incapacitated. Auto-fail STR and AGI saves. Resistance to all damage.",
		mechanicalEffects: [
			{ type: "incapacitated", target: "actions" },
			{ type: "auto_fail", target: "STR_saves" },
			{ type: "auto_fail", target: "AGI_saves" },
	...
	prone: {
		name: "Prone",
		description:
			"Disadvantage on attack rolls. Melee attacks within 5ft have advantage. Ranged attacks have disadvantage.",
		mechanicalEffects: [{ type: "disadvantage", target: "attack_rolls" }],
	},
	restrained: {
		name: "Restrained",
		description:
			"Speed 0. Disadvantage on AGI saves. Disadvantage on attack rolls. Attacks against have advantage.",
		mechanicalEffects: [
	...
	stunned: {
		name: "Stunned",
		description:
			"Incapacitated. Auto-fail STR and AGI saves. Attacks against have advantage.",
		mechanicalEffects: [
			{ type: "incapacitated", target: "actions" },
			{ type: "auto_fail", target: "STR_saves" },
			{ type: "auto_fail", target: "AGI_saves" },
	...
	unconscious: {
		name: "Unconscious",
		description:
			"Incapacitated. Drops items. Auto-fail STR and AGI saves. Attacks have advantage. Melee crits.",
		mechanicalEffects: [
			{ type: "incapacitated", target: "actions" },
			{ type: "auto_fail", target: "STR_saves" },
			{ type: "auto_fail", target: "AGI_saves" },
...
export function isIncapacitated(conditions: string[]): boolean {
	...
	return conditions.some((c) =>
		incapConditions.includes(c.trim().toLowerCase()),
	);
}
...
export function getActivePenaltySummary(
	...
	if (exhaustionLevel > 0 && exhaustionLevel <= 5) {
		summaries.push(
			`Exhaustion ${exhaustionLevel}: ${EXHAUSTION_TABLE[exhaustionLevel].description}`,
		);
...
function tickRoundDurations(conditions: ActiveCondition[]): {
	...
	for (const condition of conditions) {
		if (
			...
			condition.duration.remainingRounds !== undefined
		) {
