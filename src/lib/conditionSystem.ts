...
export interface ConditionEntry {
    ...
    sourceType: 'spell' | 'feature' | 'item' | 'manual' | 'regent' | 'environment';
 ...
 */
export function getActiveConditionNames(conditions: ConditionEntry[]): string[] {
    return [...new Set(conditions.filter((c) => c.isActive).map((c) => c.conditionName))];
}

/**
 * Convert legacy string[] conditions to ConditionEntry[] format
 */
export function migrateLegacyConditions(legacyConditions: string[]): ConditionEntry[] {
 ...
 */
export function hasCondition(conditions: ConditionEntry[], conditionName: string): boolean {
    return conditions.some((c) => c.isActive && c.conditionName === conditionName.toLowerCase());
}
