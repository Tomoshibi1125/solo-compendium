...
export interface SenseSource {
    ...
    name: string;
    sense: keyof Omit<CharacterSenses, 'passivePerception' | 'passiveInvestigation' | 'passiveInsight'>;
    range: number;
}
...
const JOB_SENSES: Record<string, SenseSource[]> = {
    ...
    'shadow': [{ type: 'job', name: 'Shadow Monarch (Regent)', sense: 'darkvision', range: 120 }],
    summoner: [{ type: 'job', name: 'Summoner', sense: 'darkvision', range: 60 }],
    technomancer: [{ type: 'job', name: 'Technomancer', sense: 'tremorsense', range: 30 }],
};
...
export function computeSenses(
    ...
    for (const regentId of regentIds) {
        ...
        if (rKey === 'shadow' || rKey === 'umbral') {
            allSources.push({ type: 'regent', name: 'Shadow Regent', sense: 'darkvision', range: 120 });
        }
        if (rKey === 'dragon') {
            allSources.push({ type: 'regent', name: 'Dragon Regent', sense: 'blindsight', range: 30 });
            allSources.push({ type: 'regent', name: 'Dragon Regent', sense: 'darkvision', range: 120 });
        }
        if (rKey === 'beast') {
            allSources.push({ type: 'regent', name: 'Beast Regent', sense: 'darkvision', range: 60 });
            allSources.push({ type: 'regent', name: 'Beast Regent', sense: 'tremorsense', range: 30 });
        }
    }

    // Resolve best-of stacking
    const darkvision = Math.max(0, ...allSources.filter((s) => s.sense === 'darkvision').map((s) => s.range));
    const blindsight = Math.max(0, ...allSources.filter((s) => s.sense === 'blindsight').map((s) => s.range));
    const tremorsense = Math.max(0, ...allSources.filter((s) => s.sense === 'tremorsense').map((s) => s.range));
    const truesight = Math.max(0, ...allSources.filter((s) => s.sense === 'truesight').map((s) => s.range));
    ...
    const passivePerception = 10 + wisdomModifier + perceptionProf + perceptionExp + observantBonus;
    const passiveInvestigation = 10 + intelligenceModifier + (investigationProficient ? proficiencyBonus : 0) + observantBonus;
