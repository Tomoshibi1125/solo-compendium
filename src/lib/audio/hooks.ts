...
import { deleteAudioFile, saveAudioFile } from "./storage";
import type {
	AudioPlayerState,
	AudioSettings,
	AudioTrack,
...
const DEFAULT_TRACKS: StoredAudioTrack[] = [
	...
	{
		id: "default-epic-combat",
		title: "Epic Combat",
		artist: "System",
		category: "combat",
		duration: 180,
		url: "/audio/music/epic-combat.ogg",
		volume: 0.8,
		loop: true,
		tags: ["epic", "combat", "orchestral"],
		mood: "epic",
		license: "CC0",
		source: "OpenGameArt",
		isLocal: true,
	...
	{
		id: "default-tavern-ambience",
		title: "Tavern Ambience",
		artist: "System",
		category: "social",
		duration: 300,
		url: "/audio/music/tavern-ambience.ogg",
		volume: 0.6,
		loop: true,
		tags: ["tavern", "inn", "social", "ambient"],
		mood: "social",
		license: "CC0",
		source: "OpenGameArt",
		isLocal: true,
	...
	{
		id: "default-magic-spell",
		title: "Magic Spell",
		artist: "System",
		category: "sfx",
		duration: 2,
		url: "/audio/sfx/magic-spell.mp3",
		volume: 0.8,
		loop: false,
		tags: ["magic", "spell", "cast", "sfx"],
		mood: "magical",
		license: "CC0",
		source: "OpenGameArt",
		isLocal: true,
...
export function useAudioPlayer() {
	...
	const loadPlaylist = useCallback(
		(playlist: Playlist, tracks: AudioTrack[] = [], startIndex = 0) => {
...
export function useAudioLibrary() {
	...
	const loadLibrary = useCallback(async () => {
		...
		try {
			...
			const mappedPlaylists = (playlistsResult.data || []).map(
				(row: AudioPlaylistRow) => ({
					id: row.id,
					name: row.name,
	...
	const updateTrack = useCallback(
		async (trackId: string, updates: Partial<AudioTrack>) => {
			if (!isAuthed || !user?.id) {
		...
		async (playlist: Playlist) => {
			...
			setPlaylists((prev) => [
				...prev,
				{
					...
					category: data.category as Playlist["category"],
					autoPlay: data.auto_play ?? false,
					shuffle: data.shuffle ?? false,
					repeat: (data.repeat ?? "none") as Playlist["repeat"],
	...
	const getTracksByMood = useCallback(
		(mood: string) => {
...
export function useAudioUpload() {
	...
	const uploadAudioFile = useCallback(
		async (file: File, metadata: Partial<AudioTrack>) => {
			setIsUploading(true);
			setUploadProgress(0);
			setError(null);
			...
			if (e.ctrlKey || e.metaKey) {
				switch (e.key) {
					...
					case "ArrowUp":
						e.preventDefault();
						setMasterVolume(
							Math.min(1, audioService.getSettings().masterVolume + 0.1),
						);
						break;
					case "ArrowDown":
						e.preventDefault();
						setMasterVolume(
							Math.max(0, audioService.getSettings().masterVolume - 0.1),
						);
						break;
				}
			}
