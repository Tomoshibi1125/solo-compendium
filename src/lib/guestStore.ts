...
export function getLocalCharacterState(
	characterId: string,
): GuestCharacterState | null {
	const state = loadGuestState();
...
export function createLocalCharacter(
	...
	const character: CharacterRow = {
		...
		base_class:
			((data as Record<string, unknown>).base_class as string) ?? null,
		str: data.str ?? 10,
		agi: data.agi ?? 10,
		vit: data.vit ?? 10,
		int: data.int ?? 10,
...
export function setLocalAbilities(
	characterId: string,
	abilities: Record<AbilityScore, number>,
): void {
	const entry = getLocalCharacterState(characterId);
...
export function getLocalCharacterSheetState(
	characterId: string,
...
export function setLocalCharacterSheetState(
	characterId: string,
	sheetState: CharacterSheetState,
): void {
	const state = loadGuestState();
...
export function addLocalEquipment(
	characterId: string,
	item: Omit<EquipmentInsert, "character_id">,
): EquipmentRow {
	const entry = getLocalCharacterState(characterId);
...
export function updateLocalEquipment(
	equipmentId: string,
...
export function addLocalPower(
	characterId: string,
	power: Omit<PowerInsert, "character_id">,
): PowerRow {
	const entry = getLocalCharacterState(characterId);
	if (!entry) throw new AppError("Local character not found", "NOT_FOUND");
...
export function addLocalFeature(
	characterId: string,
	feature: Omit<FeatureInsert, "character_id">,
): FeatureRow {
	const entry = getLocalCharacterState(characterId);
	if (!entry) throw new AppError("Local character not found", "NOT_FOUND");
	...
	const next: FeatureRow = {
		...
		homebrew_id:
			((feature as Record<string, unknown>).homebrew_id as string) ?? null,
		modifiers:
			((feature as Record<string, unknown>)
				.modifiers as FeatureRow["modifiers"]) ?? null,
	};
...
export function updateLocalFeature(
	featureId: string,
...
export function upsertLocalSpellSlot(
	...
	const now = nowIso();
	const existing = entry.spellSlots.find(
		(s) => s.spell_level === insert.spell_level,
	);
	...
	const row: SpellSlotRow = existing
		...
		: {
				...
				slots_recovered_on_short_rest:
					insert.slots_recovered_on_short_rest ?? 0,
...
export function updateLocalSpellSlotRow(
	slotId: string,
	updates: SpellSlotUpdate,
): void {
	const state = loadGuestState();
	const entries = Object.values(state.characters);
...
export function listLocalRollHistory(characterId?: string): RollHistoryRow[] {
	const state = loadGuestState();
	const characterRolls = Object.values(state.characters).flatMap(
		(c) => c.rollHistory,
	);
	const globalRolls = state.rollHistory || [];
	const all = [...globalRolls, ...characterRolls];
	const filtered = characterId
		? all.filter((r) => r.character_id === characterId)
		: all;
