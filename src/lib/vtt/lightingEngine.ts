...
export function rayWallIntersection(
    ...
    for (const wall of walls) {
        ...
        const intersection = lineSegmentIntersection(
            ox, oy, ox + dx * 9999, oy + dy * 9999,
            wall.x1, wall.y1, wall.x2, wall.y2,
        );

        if (intersection) {
            const dist = Math.sqrt((intersection.x - ox) ** 2 + (intersection.y - oy) ** 2);
...
export function computeLightLevel(
    ...
    for (const light of lights) {
        ...
        if (dist <= light.brightRadius) {
            ...
            const factor = 1 - (dist - light.brightRadius) / (light.dimRadius - light.brightRadius);
 ...
 */
export function toggleDoor(walls: WallSegment[], wallId: string): WallSegment[] {
    return walls.map((w) =>
