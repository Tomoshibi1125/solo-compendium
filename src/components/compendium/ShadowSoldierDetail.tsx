					...
					{entry.role && (
						<div className="flex items-center gap-2 mt-2">
							...
							<span className="text-lg text-muted-foreground capitalize">
								{formatRegentVernacular(entry.role)}
							</span>
						</div>
					)}
				</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div className="p-4 bg-card border rounded-lg">
							...
							<p className="text-sm text-muted-foreground">
								{entry.role === "Tank" &&
									"Specialized in defense, crowd control, and protecting allies. High durability and threat generation."}
								{entry.role === "Assassin" &&
									"Specialized in stealth, reconnaissance, and eliminating high-value targets. High burst damage."}
								{entry.role === "Mage" &&
									"Specialized in shadow magic and ranged spellcasting. Area control and debuff capabilities."}
								{entry.role === "Archer" &&
									"Specialized in precise long-range attacks and reconnaissance. High accuracy and mobility."}
								{entry.role === "Destroyer" &&
									"Balanced melee combatant with good offense and defense capabilities. Versatile frontline fighter."}
								{entry.role === "Scout" &&
									"Specialized in reconnaissance, scouting, and rapid movement. High speed and detection abilities."}
							</p>
						</div>
					</div>
				</div>
