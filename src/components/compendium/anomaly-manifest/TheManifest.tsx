			...
			<div className="grid lg:grid-cols-2 xl:grid-cols-3 gap-6">
				{sortedAnomalies.slice(0, 50).map(
					(
						...
						// Resolve thematic image: data.image → data.image_url → keyword resolver
						const thumbnailSrc = getAnomalyImageSrc({
							id: anomaly.id,
							name: anomaly.name,
							type: anomaly.type,
							image: (anomaly as { image?: string | null }).image,
							image_url: (anomaly as { image_url?: string | null }).image_url,
						});
						...
						return (
							...
							>
								...
								<div className="relative w-full h-36 overflow-hidden bg-void">
									...
									<span
										className={`absolute top-2 right-2 px-2 py-0.5 font-mono text-sm font-bold rounded border uppercase shadow-[0_0_10px_rgba(0,0,0,0.5)] ${
											anomaly.rank === "S"
