			...
			if (data.job_id) {
				...
				if (job)
					components.push({
						...
						id: job.id,
						name:
							(job as { display_name?: string | null }).display_name ||
							job.name,
					});
			}
			...
			if (data.path_id) {
				...
				if (path)
					components.push({
						...
						id: path.id,
						name:
							(path as { display_name?: string | null }).display_name ||
							path.name,
					});
			}
			...
			>
				<div className="space-y-4">
					<p className="text-sm text-muted-foreground">
						This Sovereign was created through the Gemini Protocol as a
						permanent subclass overlay of the following components:
					</p>
