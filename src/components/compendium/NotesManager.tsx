import { Calendar, Edit, FileText, Plus, Save, Trash2, X } from "lucide-react";
...
export function NotesManager({
	...
	React.useEffect(() => {
		...
		const loadLocal = () => {
			...
			try {
				const parsed = JSON.parse(savedNotes) as Array<
					Note & { createdAt: string; updatedAt: string }
				>;
		...
		const loadRemote = async () => {
			...
			const { data, error } = await supabase
				...
				.select(
					"id, content, tags, created_at, updated_at, entry_id, entry_type, user_id",
				)
									...
									>
										...
										<div className="flex items-center gap-2 text-xs text-muted-foreground">
											...
											{note.updatedAt.getTime() !==
												note.createdAt.getTime() && <span>(edited)</span>}
										</div>
									</div>
								))}
							</div>
