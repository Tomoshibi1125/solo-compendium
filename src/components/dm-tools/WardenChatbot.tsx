...
DM: ${userMessage.content}`;
            ...
            toast({
                title: 'Error',
                description: error instanceof Error ? error.message : 'The Warden is currently unavailable.',
                variant: 'destructive',
            });
        } finally {
            setIsLoading(false);
    ...
    const handleSaveToNPC = async (content: string) => {
        ...
        if (!npcData) {
            toast({
                title: 'Error',
                description: 'Could not parse a valid NPC stat block from this message.',
                variant: 'destructive',
            });
            return;
        }
    ...
    return (
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="w-[400px] sm:w-[540px] flex flex-col p-0 border-l border-primary/20 bg-background/95 backdrop-blur">
                    ...
                    {messages.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-muted-foreground space-y-4 pt-20">
                            ...
                            <p className="text-center text-sm px-8">
                                I am The Warden. Ask me to generate NPC stat blocks, design encounters, or clarify system rules.
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {messages.map((msg) => {
                                ...
                                const hasJson = !isUser && extractJsonBlock(msg.content) !== null;

                                return (
                                    <div key={msg.id} className={isUser ? 'flex justify-end' : 'flex justify-start'}>
                                        <div className={'flex gap-3 max-w-[85%] ' + (isUser ? 'flex-row-reverse' : 'flex-row')}>
                                            <div className={'flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ' + (isUser ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground')}>
                                                {isUser ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                                            </div>
                                            <div className={'rounded-lg p-3 text-sm ' + (isUser ? 'bg-primary text-primary-foreground' : 'bg-muted border border-border prose prose-sm dark:prose-invert prose-p:leading-relaxed prose-pre:bg-background/50 prose-pre:border prose-pre:border-border')}>
                                                {isUser ? (
                                                    <div className="whitespace-pre-wrap">{msg.content}</div>
                                                ) : (
                                                    <>
                                                        <ReactMarkdown>{msg.content.replace(/```(?:json)?\s*[\s\S]*?\s*```/, '[JSON STAT BLOCK READY TO SAVE]')}</ReactMarkdown>
                                                        {hasJson && (
                                ...
                                <div className="flex justify-start">
                                    <div className="flex gap-3 max-w-[85%] flex-row">
                                        ...
                                        <div className="rounded-lg p-3 text-sm bg-muted border border-border flex items-center gap-2">
                                            ...
                                            <span className="text-muted-foreground">The Warden is thinking...</span>
                                        </div>
                                    </div>
                                </div>
                            )}
