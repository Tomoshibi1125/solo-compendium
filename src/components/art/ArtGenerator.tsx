...
import { Image, Loader2, Settings, Sparkles } from "lucide-react";
	...
	const handleGenerate = async () => {
		if (!isAvailable) {
			setGenerationResult({
				success: false,
				error:
					"Art generation is unavailable. Enable the feature flag or check your connection.",
			});
			return;
		}
	...
	return (
		<div className={cn("space-y-6", className)}>
			...
			{!isAvailable && (
				<Alert>
					<AlertDescription>
						Art generation is currently unavailable. Enable it in settings or
						try again later.
					</AlertDescription>
				</Alert>
			)}
						...
						<div>
							...
							<Select
								...
								onValueChange={(value) =>
									setFormData((prev) => ({
										...prev,
										variant: value as ArtRequest["variant"],
									}))
								}
							>
								<SelectTrigger>
