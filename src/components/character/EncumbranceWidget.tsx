import { Weight, AlertTriangle, AlertOctagon, CheckCircle2 } from 'lucide-react';
...
export function EncumbranceWidget({ characterId, compact = false }: EncumbranceWidgetProps) {
    const { equipment } = useEquipment(characterId);
    const { data: character } = useCharacter(characterId);
    const { ignoreCurrencyWeight } = useEncumbranceSettings(characterId);

    const strScore = (character?.abilities as Record<string, number> | undefined)?.STR ?? 10;
        ...
        return (
            <Tooltip>
                ...
                <TooltipContent side="bottom" className="text-xs space-y-1 max-w-xs">
                    ...
                    <p>Carrying: <span className="font-mono">{totalWeight.toFixed(1)} lb</span> of <span className="font-mono">{carryingCapacity} lb</span></p>
                    {cfg.penalty && <p className="text-orange-400">{cfg.penalty}</p>}
                    <p className="text-muted-foreground">Push/drag/lift: {enc.pushDragLift} lb</p>
                </TooltipContent>
            </Tooltip>
        );
    }
    ...
    return (
        <div
            className={cn(
                ...
                enc.status !== 'heavy' && enc.status !== 'overloaded' && 'border-border bg-muted/20',
            )}
        >
            {/* Header row */}
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-display tracking-wider text-muted-foreground">WEIGHT</span>
                </div>
                <span className={cn('text-xs font-semibold', cfg.iconClass)}>{cfg.label}</span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span className="font-mono tabular-nums">
                    {totalWeight.toFixed(1)} <span className="opacity-60">/ {carryingCapacity} lb</span>
                </span>
