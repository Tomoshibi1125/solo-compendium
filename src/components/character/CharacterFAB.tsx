...
import { Dice5, Plus, Tent, Moon, PlusCircle, X, ScrollText } from 'lucide-react';
...
export function CharacterFAB({ characterId, campaignId, onShortRest, onLongRest }: CharacterFABProps) {
            ...
            <div className="fixed bottom-20 right-4 sm:right-8 z-50 flex flex-col items-end gap-3 pointer-events-none">
                ...
                >
                    <Button
                        ...
                        onClick={() => { onLongRest(); setIsOpen(false); }}
                    >
                        <span>Long Rest</span>
                    ...
                    <Button
                        ...
                        size="sm"
                        className="rounded-full shadow-lg gap-2 justify-end px-4 h-10 w-fit self-end bg-card border-border"
                        onClick={() => { onShortRest(); setIsOpen(false); }}
                    >
                    ...
                    <Button
                        ...
                        size="sm"
                        className="rounded-full shadow-lg gap-2 justify-end px-4 h-10 w-fit self-end bg-card border-border"
                        onClick={() => { setAddItemOpen(true); setIsOpen(false); }}
                    >
                        <span>Add Item</span>
                ...
                <Button
                    ...
                    className={cn(
                        ...
                        isOpen ? "bg-secondary text-secondary-foreground rotate-45" : "bg-primary text-primary-foreground hover:scale-105"
                    )}
                    onClick={toggleOpen}
                    aria-label="Quick Actions"
                >
                    <Plus className={cn("w-6 h-6 transition-all", isOpen ? "scale-110" : "")} />
                </Button>
            </div>
