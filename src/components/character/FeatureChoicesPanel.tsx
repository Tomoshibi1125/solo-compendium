					...
					if (grant.type === "feat" && typeof grant.name === "string") {
						...
						const buildFeatModifiers = (
							...
							for (const raw of benefitLines) {
								...
								const hpPerLevel = lower.match(
									/hp\s+maximum\s+increases\s+by\s+(\d+)\s+per\s+level/i,
								);
								if (hpPerLevel?.[1]) {
