...
export function RegentSelection({
  ...
  const getRegentStatus = (regentId: string) => {
    if (!isRegentAvailable(regentId)) {
      return { status: 'locked', icon: Lock, color: 'text-gray-500', message: 'Quest completion required' };
    }
    if (regentId === selectedRegent) {
      return { status: 'selected', icon: CheckCircle, color: 'text-green-500', message: 'Currently selected' };
    }
    return { status: 'available', icon: Crown, color: 'text-blue-500', message: 'Available for selection' };
  };
        ...
        {selectedRegentData && (
          <div className="space-y-4">
            ...
            {selectedRegentData.spellcasting && (
              <div className="space-y-3">
                <h4 className="font-medium">Spell Slots at Level {selectedLevel}</h4>
                <div className="grid grid-cols-9 gap-2 text-center">
                  {Array.from({ length: 9 }, (_, i) => i + 1).map((level) => {
                    const slotKey = `${level}${level === 1 ? 'st' : level === 2 ? 'nd' : level === 3 ? 'rd' : 'th'}` as keyof typeof selectedRegentData.spellcasting.spell_slots;
                    const slots = selectedRegentData.spellcasting.spell_slots[slotKey]?.[selectedLevel - 1] || 0;
                    return (
