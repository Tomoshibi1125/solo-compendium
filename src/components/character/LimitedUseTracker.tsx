...
export function LimitedUseTracker({ characterId }: LimitedUseTrackerProps) {
    ...
    const { data: runeInscriptions = [] } = useCharacterRuneInscriptions(characterId);
    ...
    const updateRuneUses = useMutation({
        mutationFn: async ({ id, usesCurrent }: { id: string; usesCurrent: number }) => {
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['character-rune-inscriptions', characterId] });
            queryClient.invalidateQueries({ queryKey: ['equipment-runes'] });
        }
    });

    const limitedFeatures = features.filter((f) => f.uses_max !== null);
    const limitedRunes = runeInscriptions.filter((r) => r.uses_max !== null && r.is_active);

    const hasAnyUses = limitedFeatures.length > 0 || limitedRunes.length > 0;

    if (!hasAnyUses) return null;

    const handleFeatureToggle = (id: string, usesMax: number, currentUses: number | null, indexToToggle: number) => {
        ...
        if (featureDetails) {
            ...
            playerTools.trackCustomFeatureUsage(characterId, featureDetails.name, actionType, '5e').catch(console.error);
    ...
    const handleRuneToggle = (id: string, usesMax: number, currentUses: number | null, indexToToggle: number) => {
        ...
        if (runeDetails && runeDetails.rune) {
            const isSpending = newUses < actualCurrent;
            const actionType = isSpending ? 'spend' : 'regain';
            playerTools.trackCustomFeatureUsage(characterId, runeDetails.rune.name, actionType, 'SA').catch(console.error);
        ...
        <SystemWindow title="LIMITED USE" compact>
            <div className="space-y-4 pt-4">
                {limitedFeatures.map((feature) => (
                    <div key={feature.id} className="flex flex-col gap-1.5 border-b border-white/5 pb-3">
                        <div className="flex justify-between items-end">
                            <span className="text-sm font-semibold capitalize text-foreground">{feature.name}</span>
                            {feature.recharge && (
                                <span className="text-[10px] text-muted-foreground uppercase">{feature.recharge}</span>
                            )}
                        </div>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                            {Array.from({ length: feature.uses_max! }).map((_, i) => (
                                <button
                                    ...
                                    onClick={() => handleFeatureToggle(feature.id, feature.uses_max!, feature.uses_current, i)}
                                >
                                    <div className={cn(
                                        "w-2.5 h-2.5 rounded-sm transition-transform",
                                        i < (feature.uses_current ?? feature.uses_max!) ? "bg-cyan-400 scale-100" : "scale-0"
                                    )} />
                                </button>
                            ))}
                        </div>
                ...
                {limitedRunes.map((entry) => (
                    <div key={entry.id} className="flex flex-col gap-1.5 border-b border-white/5 pb-3 last:border-0 last:pb-0">
                        <div className="flex justify-between items-end">
                            <span className="text-sm font-semibold capitalize text-amethyst-purple">{entry.rune?.name} (Rune)</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                            {Array.from({ length: entry.uses_max! }).map((_, i) => (
                                <button
                                    ...
                                    onClick={() => handleRuneToggle(entry.id, entry.uses_max!, entry.uses_current, i)}
                                >
                                    <div className={cn(
                                        "w-2.5 h-2.5 rounded-sm transition-transform",
                                        i < (entry.uses_current ?? entry.uses_max!) ? "bg-amethyst-purple scale-100" : "scale-0"
                                    )} />
                                </button>
                            ))}
                        </div>
