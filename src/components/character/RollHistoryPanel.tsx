...
import { Dice5, History, Shield, Sparkles, Sword, Target } from "lucide-react";
...
const rollTypeConfig: Record<
	string,
	{ icon: typeof Dice5; color: string; label: string }
> = {
	attack: { icon: Sword, color: "text-destructive", label: "Attack" },
	damage: { icon: Target, color: "text-gate-a", label: "Damage" },
	save: { icon: Shield, color: "text-shadow-blue", label: "Save" },
	skill: { icon: Sparkles, color: "text-success", label: "Skill" },
	ability: { icon: Dice5, color: "text-resurge", label: "Ability" },
...
export function RollHistoryPanel({
	...
	const getRollConfig = (type: string) =>
		rollTypeConfig[type] || rollTypeConfig.default;

	const isNat20 = (roll: RollRecord) =>
		roll.dice_formula.includes("d20") && roll.rolls.some((r) => r === 20);
	const isNat1 = (roll: RollRecord) =>
		roll.dice_formula.includes("d20") && roll.rolls.some((r) => r === 1);
						...
						<div className="space-y-2">
							{rolls.map((roll) => {
								...
								return (
									...
									>
										<div
											className={cn("p-2 rounded-lg bg-muted/50", config.color)}
										>
