import { Filter, Music, Play, Plus, Search, Trash2 } from "lucide-react";
...
export function AudioLibrary({ onTrackSelect }: AudioLibraryProps) {
	...
	const filteredTracks = useMemo(() => {
		return tracks.filter((track) => {
			...
			const matchesGenre =
				selectedGenre === "all" || track.category === selectedGenre;
			const matchesMood =
				selectedMood === "all" ||
	...
	const totalSize = filteredTracks.reduce(
		(sum, track) => sum + (track.fileSize || 0),
		0,
	);
	...
	const handleUpload = async () => {
		...
		try {
			const track = await uploadAudioFile(pendingFile, {
				title:
					uploadMetadata.title || pendingFile.name.replace(/\.[^/.]+$/, ""),
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Browse and manage your audio library
					</p>
				</div>
			...
			<Card>
				...
				<CardContent>
					...
					{!isLoading && filteredTracks.length === 0 && (
						<div className="text-sm text-muted-foreground">
							No tracks match your filters yet.
						</div>
					)}
