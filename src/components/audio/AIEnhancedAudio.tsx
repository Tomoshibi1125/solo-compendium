...
export function AIEnhancedAudio({
	onAnalysisComplete,
	className,
}: AIEnhancedAudioProps) {
			...
			<Card>
				...
				<CardContent className="space-y-4">
					<div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
						<input
							aria-label="Upload audio file for AI analysis"
							ref={fileInputRef}
							type="file"
							accept="audio/*"
						...
						<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
							...
							<ul className="text-sm text-blue-800 space-y-1">
								...
								<li>
									- Energy level{" "}
									{analysis.energy > 0.6
										? "high - great for combat"
										: "low - good for exploration"}
								</li>
								<li>
									-{" "}
									{analysis.instruments.length > 0
										? `Features ${analysis.instruments.slice(0, 2).join(" and ")}`
										: "Minimal instrumentation"}
								</li>
								<li>
									- Recommended for {analysis.genre.toLowerCase()} campaigns
								</li>
							</ul>
						</div>
					</CardContent>
