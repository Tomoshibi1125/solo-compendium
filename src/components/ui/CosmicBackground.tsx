...
export const CosmicBackground = ({
	...
	const riftLayers = useMemo(() => {
		...
		const layerCount =
			finalIntensity === "high" ? 8 : finalIntensity === "medium" ? 5 : 3;
	...
	const fogParticles = useMemo(() => {
		...
		const particleCount =
			finalIntensity === "high" ? 15 : finalIntensity === "medium" ? 10 : 5;
	...
	useEffect(() => {
		...
		const animate = () => {
			...
			for (let i = 0; i < 5; i++) {
				...
				const y =
					Math.cos(time * 0.7 + i) * canvas.height * 0.3 + canvas.height / 2;
