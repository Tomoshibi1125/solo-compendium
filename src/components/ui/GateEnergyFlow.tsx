...
export const GateEnergyFlow = ({
  ...
  const energyStreams = useMemo(() => {
    ...
    const streamCount = finalIntensity === 'high' ? 12 : finalIntensity === 'medium' ? 8 : 4;

    for (let i = 0; i < streamCount; i++) {
      const availableTiers: EnergyStream['tier'][] = ['e', 'd', 'c', 'b', 'a', 's', 'ss', 'national'];
      ...
      {energyStreams.map((stream) => (
        <div
          ...
          style={{
            ...
            animationDuration: enableAnimation ? `${4 / stream.speed}s` : undefined,
          }}
        />
      ))}
