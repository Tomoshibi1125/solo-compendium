...
export const SystemHologram = ({
  ...
  const getVariantStyles = () => {
    const baseStyles = "relative bg-gradient-to-br backdrop-blur-xl border overflow-hidden";

    switch (variant) {
      case 'sovereign':
        return cn(baseStyles, "from-monarch-gold/10 via-shadow-purple/15 to-void-black/90 border-monarch-gold/40 shadow-[0_0_30px_hsl(var(--monarch-gold)/0.3)]");
      case 'gate':
        return cn(baseStyles, "from-gate-s/10 via-gate-ss/15 to-void-black/90 border-gate-s/40 shadow-[0_0_25px_hsl(var(--gate-s-glow)/0.3)]");
      case 'system':
        return cn(baseStyles, "from-system-green/10 via-primary/15 to-void-black/90 border-system-green/40 shadow-[0_0_25px_hsl(var(--system-green)/0.3)]");
      default:
        return cn(baseStyles, "from-primary/10 via-secondary/15 to-void-black/90 border-primary/40 shadow-[0_0_25px_hsl(var(--primary)/0.3)]");
    }
  };
  ...
  return (
    <div className={cn("relative rounded-lg", getVariantStyles(), className)}>
          ...
          {dataStreams.map((stream) => (
            <div
              ...
              style={{
                ...
                color: variant === 'sovereign' ? '#ffd700' :
                       variant === 'gate' ? '#ff4500' :
                       variant === 'system' ? '#00ff88' : '#00ffff',
                textShadow: `0 0 8px ${variant === 'sovereign' ? '#ffd700' :
                                       variant === 'gate' ? '#ff4500' :
                                       variant === 'system' ? '#00ff88' : '#00ffff'}`,
      ...
      <div className="absolute bottom-2 left-2 w-2 h-2 rounded-full bg-current opacity-60 animate-pulse" style={{ animationDelay: '1s' }}></div>
      <div className="absolute bottom-2 right-2 w-2 h-2 rounded-full bg-current opacity-60 animate-pulse" style={{ animationDelay: '1.5s' }}></div>
    </div>
  );
};
