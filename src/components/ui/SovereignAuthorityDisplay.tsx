...
export const SovereignAuthorityDisplay = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      ctx.lineWidth = 8;
      ctx.beginPath();
      ctx.arc(centerX, centerY, radius * 0.8, -Math.PI / 2, -Math.PI / 2 + (authority / 100) * Math.PI * 2);
      ctx.stroke();
  ...
  return (
    <div className={cn("relative bg-void-black/80 border border-monarch-gold/30 rounded-lg p-4", className)}>
      {/* Header */}
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <div>
          ...
          <div className="font-display text-sm text-monarch-gold">
            {domain}
          </div>
        </div>
        <div>
...
export const AscensionMeter = ({
  ...
  const tierConfig = useMemo(() => {
    const configs = {
      ascendant: { color: '#8b5cf6', glow: '#a855f7', bg: 'from-purple-900/20 to-purple-800/20' },
      sovereign: { color: '#ffd700', glow: '#ffed4e', bg: 'from-yellow-900/20 to-yellow-800/20' },
      regent: { color: '#f59e0b', glow: '#fbbf24', bg: 'from-orange-900/20 to-orange-800/20' },
      monarch: { color: '#dc2626', glow: '#ef4444', bg: 'from-red-900/20 to-red-800/20' }
    };
    return configs[tier];
  }, [tier]);

  return (
    <div className={cn("relative bg-gradient-to-r rounded-lg p-4 border overflow-hidden", tierConfig.bg, className)}>
