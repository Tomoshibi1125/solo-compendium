...
export function WardenChatbot() {
	...
	const { saveNow: saveNpcState } = useUserToolState<unknown>("npc_generator", {
		...
		storageKey: "solo-compendium.PW-tools.npc-generator.v1",
	});
	...
	const handleSend = async () => {
		...
		try {
			...
			const fullPrompt = `You are "The Warden", an omnipresent, dark-fantasy AI assistant for a Protocol Warden running a game in the "System Ascendant" universe.
If the Protocol Warden asks for stats, provide them as a valid JSON block enclosed in \`\`\`json ... \`\`\`. The JSON should match this interface:
{
  "name": "String",
  "rank": "E, D, C, B, A, or S",
  "role": "String",
...
Protocol Warden: ${userMessage.content}`;
