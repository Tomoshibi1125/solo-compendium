    ...
    <div className="space-y-2">
            ...
            return (
              <Tooltip key={condition}>
                <TooltipTrigger asChild>
                  <Badge className={cn('gap-1 cursor-default select-none', color)}>
                ...
                <TooltipContent side="bottom" className="max-w-xs text-left">
                  <p className="font-semibold mb-1">{info?.name ?? condition}</p>
                  <p className="text-xs">{info?.description ?? 'Custom condition.'}</p>
                  {info?.mechanicalEffects.length ? (
                    <ul className="mt-1 text-xs list-disc pl-3">
                      {info.mechanicalEffects.map((e, i) => (
                        <li key={i}>
                          {e.type === 'disadvantage' && `Disadvantage on ${e.target.replace(/_/g, ' ')}`}
                          {e.type === 'advantage' && `Advantage on ${e.target.replace(/_/g, ' ')}`}
                          {e.type === 'auto_fail' && `Auto-fail ${e.target.replace(/_/g, ' ')}`}
                          {e.type === 'speed_zero' && 'Speed becomes 0'}
                          {e.type === 'incapacitated' && 'Cannot take actions or reactions'}
                        </li>
                      ))}
                    </ul>
                  ) : null}
      ...
      {showPicker && (
        <div className="flex flex-wrap gap-1 rounded-md border p-2 bg-background">
          ...
          {available.length === 0 && (
            <span className="text-xs text-muted-foreground">All conditions active</span>
          )}
        </div>
      )}
    </div>
