...
import { COMMON_CANTRIP_DICE, scaleCantripDamage } from "@/lib/cantripScaling";
...
export function SpellPanel({
	...
	const [castDialog, setCastDialog] = useState<{
		spell: SpellEntry;
		open: boolean;
	} | null>(null);
	...
	const availableUpcastLevels = (baseLevel: number) =>
		spellSlots
			.filter((s) => s.level >= baseLevel && s.current > 0)
			.map((s) => s.level);
		...
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-3 text-sm">
					<span className="text-muted-foreground">
						Save DC{" "}
						<span className="font-bold text-foreground">{spellSaveDC}</span>
					</span>
					<span className="text-muted-foreground">
						Attack{" "}
				...
				<div>
					...
					<div className="flex flex-wrap gap-1.5">
						{cantrips.map((spell) => {
							...
							const scaled = baseDie
								? scaleCantripDamage(`1d${baseDie}`, characterLevel)
								: null;
								...
								<TooltipProvider key={spell.id}>
									<Tooltip>
										...
										>
											...
											{spell.castingTime && (
												<p className="text-xs">Casting: {spell.castingTime}</p>
											)}
											{spell.range && (
												<p className="text-xs">Range: {spell.range}</p>
											)}
											{spell.duration && (
												<p className="text-xs">Duration: {spell.duration}</p>
											)}
											{scaled && (
				...
				.map(([level, levelSpells]) => {
					...
					return (
						<div key={lvl} className="border rounded-md">
							...
							>
								<span>
									Level {lvl} ({levelSpells.length} spells)
								</span>
			...
			{castDialog && (
				<Dialog
					...
					onOpenChange={(o) => !o && setCastDialog(null)}
				>
					<DialogContent>
						<DialogHeader>
						...
						<div className="space-y-3">
							...
							{castDialog.spell.higherLevels &&
								castLevel > castDialog.spell.level && (
