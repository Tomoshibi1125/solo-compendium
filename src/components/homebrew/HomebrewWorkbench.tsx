...
const CONTENT_TYPES: HomebrewContentType[] = ['job', 'path', 'relic', 'spell', 'item'];

const VISIBILITY_OPTIONS: HomebrewVisibilityScope[] = ['private', 'campaign', 'public'];
...
export function HomebrewWorkbench() {
  ...
  const [visibilityScope, setVisibilityScope] = useState<HomebrewVisibilityScope>('private');
  ...
  const campaignScopeId = scope === 'campaign' && scopeCampaignId !== 'none' ? scopeCampaignId : null;
  const { data: records = [], isLoading, error } = useHomebrewLibrary({
    scope,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Could not parse JSON payload.',
        variant: 'destructive',
      });
      return;
    }
    ...
    const result = await saveHomebrew.mutateAsync({
      ...
      visibilityScope,
      campaignId: visibilityScope === 'campaign' && campaignId !== 'none' ? campaignId : null,
      data: parsedData,
    });
  ...
  const handleExport = (record: HomebrewRecord) => {
    downloadJson(record, `homebrew-${record.name.replace(/\s+/g, '-').toLowerCase()}.json`);
  };
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setVisibilityScope(
        parsed.visibility_scope === 'campaign' || parsed.visibility_scope === 'public' || parsed.visibility_scope === 'private'
          ? parsed.visibility_scope
          : 'private'
      );
      setCampaignId(parsed.campaign_id || 'none');
      setDataText(JSON.stringify((parsed.data as Record<string, unknown>) || {}, null, 2));
      toast({
        title: 'Import loaded',
      ...
      toast({
        title: 'Import failed',
        description: error instanceof Error ? error.message : 'Could not parse import file.',
        variant: 'destructive',
      });
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          {scope === 'campaign' && (
            <div>
              ...
              <Select value={scopeCampaignId} onValueChange={setScopeCampaignId}>
          ...
          <div className="md:col-span-2 flex flex-wrap items-end gap-2">
            ...
            />
            {selectedRecord && (
              <Button variant="outline" onClick={() => handleExport(selectedRecord)}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <SystemWindow title="EDITOR" className="xl:col-span-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <Select value={contentType} onValueChange={(value) => setContentType(value as HomebrewContentType)}>
            ...
            <div>
              ...
              <Input id="homebrew-name" value={name} onChange={(event) => setName(event.target.value)} />
            </div>
            ...
            <div>
              ...
              <Input id="homebrew-tags" value={tagsText} onChange={(event) => setTagsText(event.target.value)} />
            </div>
            <div>
              <Label htmlFor="homebrew-source">Source Book</Label>
              <Input id="homebrew-source" value={sourceBook} onChange={(event) => setSourceBook(event.target.value)} />
            </div>
            <div>
              <Label htmlFor="homebrew-visibility">Visibility</Label>
              <Select value={visibilityScope} onValueChange={(value) => setVisibilityScope(value as HomebrewVisibilityScope)}>
            ...
            {selectedRecord && (
              <>
                ...
                >
                  ...
                  {selectedRecord.status === 'published' ? 'Unpublish' : 'Publish'}
                </Button>
                <Button
                  variant="outline"
        ...
        <div className="space-y-6">
          <SystemWindow title="LIBRARY">
            {isLoading ? (
              <p className="text-sm text-muted-foreground">Loading homebrew content...</p>
            ) : records.length === 0 ? (
              <p className="text-sm text-muted-foreground">No homebrew entries found for this scope.</p>
            ) : (
                ...
                {records.map((record) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-heading font-semibold truncate">{record.name}</p>
                      <Badge variant={record.status === 'published' ? 'default' : 'outline'}>
                        {record.status}
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">{record.content_type} · {record.visibility_scope}</p>
          ...
          {selectedRecord && (
            <SystemWindow title="SELECTED ENTRY">
              <div className="space-y-2 text-sm">
                <p className="font-heading font-semibold">{selectedRecord.name}</p>
                <p className="text-muted-foreground">Version {selectedRecord.version}</p>
                <p className="text-muted-foreground">Updated {new Date(selectedRecord.updated_at).toLocaleString()}</p>
                <div className="flex flex-wrap gap-2 pt-2">
                  <Button variant="outline" size="sm" onClick={() => handleExport(selectedRecord)}>
                    <FileJson className="w-4 h-4 mr-2" />
                    Export
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => handleTogglePublish(selectedRecord)}>
                    <Eye className="w-4 h-4 mr-2" />
                    {selectedRecord.status === 'published' ? 'Set Draft' : 'Publish'}
                  </Button>
                </div>
              </div>
            </SystemWindow>
              ...
              {versions.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[220px] overflow-auto pr-1">
                  {versions.map((version) => (
                    <div key={version.id} className="rounded border border-border p-2 text-xs">
                      <p className="font-semibold">v{version.version_number}</p>
                      <p className="text-muted-foreground">{new Date(version.created_at).toLocaleString()}</p>
                      {version.change_note && <p className="mt-1 text-muted-foreground">{version.change_note}</p>}
                      <Button
                        variant="ghost"
                        size="sm"
                        className="mt-1 h-7 px-2"
                        onClick={() => setDataText(JSON.stringify(version.snapshot, null, 2))}
                      >
