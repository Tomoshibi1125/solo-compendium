...
const buildLabelStyle = (
	themeConfig: (typeof DICE_THEMES)[DiceTheme],
	mode: DiceDisplayMode,
): LabelStyle => {
	const accent = themeConfig.accentColor ?? themeConfig.emissiveColor;
	const badge =
		mode === "percentile-tens"
			? "tens"
			: mode === "percentile-ones"
				? "ones"
				: null;
	const badgeColor =
		mode === "percentile-ones" ? themeConfig.particleColor : accent;
	const stroke =
		mode === "percentile-ones" ? themeConfig.particleColor : accent;
...
const getLabelTexture = (label: string, style: LabelStyle) => {
	...
	if (style.badge) {
		const badgeOffsetX =
			style.badge === "tens" ? canvasSize * 0.18 : -canvasSize * 0.18;
...
function ImpactFlash({
	...
	return (
		<pointLight
			ref={lightRef}
			position={[position[0], 0.9, position[2]]}
			distance={5}
			color={color}
			intensity={0}
		/>
...
function DiceTrayColliders({
	...
	const colliderProps = {
		restitution: DICE_PHYSICS.floorRestitution,
		friction: DICE_PHYSICS.floorFriction,
	};
...
function DiceBloomField({
	...
	const texture = useMemo(
		() => getBloomFieldTexture(themeConfig),
		[themeConfig],
	);
...
function DieFlair({
	...
	const sigilTexture = useMemo(
		() => getSigilTexture(sides, themeConfig),
		[sides, themeConfig],
	);
	...
	useFrame((_state, delta) => {
		...
		groupRef.current.rotation.x =
			Math.sin(timeRef.current * 0.7 + sides) * spec.ringTilt;
		groupRef.current.rotation.z =
			Math.cos(timeRef.current * 0.55 + sides) * spec.ringTilt * 0.35;
		groupRef.current.position.y =
			spec.floatHeight + Math.sin(timeRef.current * 1.2) * 0.04;

		const pulse =
			0.75 + Math.sin(timeRef.current * 2.1) * 0.12 + (isActive ? 0.1 : 0);
		...
		if (glyphMaterialRef.current) {
			glyphMaterialRef.current.opacity =
				spec.glyphOpacity * (0.7 + pulse * 0.3);
		}
	});
	...
	return (
		<group ref={groupRef} position={[0, spec.floatHeight, 0]}>
			{Array.from({ length: spec.ringCount }).map((_, index) => (
				...
				>
					<ringGeometry
						args={[spec.ringScale * 0.62, spec.ringScale * 0.8, 64]}
					/>
					<meshBasicMaterial
						ref={(material) => {
...
function Die({
	...
	const labelStyle = useMemo(
		() => buildLabelStyle(themeConfig, displayMode),
		[displayMode, themeConfig],
	);
	const flairSpec = DIE_FLAIR[sides] ?? DEFAULT_DIE_FLAIR;
	const edgeGeometry = useMemo(
		() => new EdgesGeometry(dieModel.geometry, 25),
		[dieModel.geometry],
	);
	...
	const bloomTexture = useMemo(
		() => getBloomTexture(themeConfig),
		[themeConfig],
	);
	const bloomScale = useMemo(
		() => 1.35 + flairSpec.ringScale * 0.7,
		[flairSpec.ringScale],
	);
	const timeRef = useRef(0);
	...
	const resolvedValue = resolveDisplayValue(
		sides,
		displayValue ?? value,
		displayMode,
	);
	...
	useEffect(() => {
		...
		yawRef.current = new Quaternion().setFromAxisAngle(
			UP,
			Math.random() * Math.PI * 2,
		);

		const spawnX = clamp(
			position[0] + (Math.random() - 0.5) * 1.4,
			bounds.minX,
			bounds.maxX,
		);
		const spawnZ = clamp(
			position[2] + (Math.random() - 0.5) * 1.4,
			bounds.minZ,
			bounds.maxZ,
		);
		...
		const impulseStrength =
			DICE_PHYSICS.impulseBase + Math.random() * DICE_PHYSICS.impulseJitter;
		const impulse = {
			x: (Math.random() - 0.5) * 6,
			y: impulseStrength,
			z: (Math.random() - 0.5) * 6,
		};
		const torqueStrength =
			DICE_PHYSICS.torqueBase + Math.random() * DICE_PHYSICS.torqueJitter;
		const torque = {
	...
	useFrame((_state, delta) => {
		...
		if (alignment) {
			...
			const progress = clamp(
				(now - alignment.startTime) / alignment.durationMs,
				0,
				1,
			);
			...
			material.emissive = new Color(themeConfig.emissiveColor).multiplyScalar(
				0.65,
			);
			...
			if (progress >= 1) {
				...
				if (onRollComplete) {
					setTimeout(
						() => onRollComplete(alignment.value),
						DICE_PHYSICS.settleDelayMs,
					);
				}
			}
			return;
		...
		if (isAnimating) {
			...
			const isCalm =
				speed < DICE_PHYSICS.calmLinear && angSpeed < DICE_PHYSICS.calmAngular;
			...
			if (
				!settledRef.current &&
				(settleTimerRef.current > DICE_PHYSICS.calmTime || exceededMax)
			) {
			...
			if (edgeMaterialRef.current) {
				edgeMaterialRef.current.opacity =
					flairSpec.edgeOpacity * (0.85 + pulse * 0.4);
			}
			if (bloomMaterialRef.current) {
				bloomMaterialRef.current.opacity = clamp(
					0.18 + pulse * 0.12,
					0.08,
					0.35,
				);
			...
			if (edgeMaterialRef.current) {
				edgeMaterialRef.current.opacity =
					flairSpec.edgeOpacity * (0.9 + pulse * 0.3);
			}
			if (bloomMaterialRef.current) {
				bloomMaterialRef.current.opacity = clamp(
					0.24 + pulse * 0.18,
	...
	const particleIntensity =
		(isCritical || isFumble ? 1.6 : 1) *
		(flairSpec.particleBoost ?? 1) *
	...
	return (
		...
		>
			<mesh
				...
				receiveShadow={quality.enableShadows}
			>
				...
				<meshPhysicalMaterial
					...
					emissiveIntensity={
						isAnimating
							? themeConfig.glowIntensity * 0.9
							: themeConfig.glowIntensity * 0.35
					}
				/>
			</mesh>
			...
			{dieModel.faces.map((face) => {
				...
				const facePosition = face.center
					.clone()
					.add(face.normal.clone().multiplyScalar(0.06));
				const faceQuaternion = new Quaternion().setFromUnitVectors(
					new Vector3(0, 0, 1),
					face.normal,
				);
				return (
			...
			{quality.enableFlair && (
				<DieFlair
					sides={sides}
					theme={theme}
					isActive={isAnimating || isCritical || isFumble}
				/>
			)}
...
function Dice3DScene({
	...
	return (
		<>
			...
			<Physics
				gravity={[0, DICE_PHYSICS.gravity, 0]}
				interpolate
				colliders={false}
			>
				<DiceTrayColliders
...
export function Dice3DRoller({
	...
	const _themeStyle = useMemo(() => {
		...
		return {
			...
			"--dice-glow-secondary": hexToRgba(
				themeConfig.particleColor,
				glowStrength * 0.85,
			),
			"--dice-panel-border": hexToRgba(
				themeConfig.accentColor ?? themeConfig.emissiveColor,
				0.35,
			),
