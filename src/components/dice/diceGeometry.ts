...
const buildFaceGroups = (geometry: BufferGeometry) => {
	...
	const addTriangle = (a: number, b: number, c: number) => {
		...
		let target = groups.find(
			(group) => group.normal.clone().normalize().dot(normal) > normalThreshold,
		);
		if (!target) {
...
const assignFaceValues = (sides: number, groups: FaceGroup[]): FaceData[] => {
	...
	const pairs: Array<
		Array<{ normal: Vector3; center: Vector3; index: number }>
	> = [];
	...
	pairs.sort((a, b) => {
		const aFace = a[0];
		const bFace = b[0];
		if (bFace.normal.y !== aFace.normal.y)
			return bFace.normal.y - aFace.normal.y;
		if (bFace.normal.z !== aFace.normal.z)
			return bFace.normal.z - aFace.normal.z;
	...
	pairs.forEach((pair, idx) => {
		...
		const [first, second] =
			pair[0].normal.y >= pair[1].normal.y ? pair : [pair[1], pair[0]];
		data.push({
			value: lowValue,
			normal: first.normal,
			center: first.center,
	...
	if (data.length !== expected) {
		return data.slice(0, expected).sort((a, b) => a.value - b.value);
	}
...
const createTrapezohedronGeometry = () => {
	...
	for (let i = 0; i < sides; i += 1) {
		const theta = angle * i;
		vertices.push(
			new Vector3(radius * Math.cos(theta), radius * Math.sin(theta), topZ),
		);
	}
	...
	for (let i = 0; i < sides; i += 1) {
		...
		vertices.push(
			new Vector3(radius * Math.cos(theta), radius * Math.sin(theta), bottomZ),
		);
	}
	...
	faces.forEach((face) => {
		...
		const normal = new Vector3()
			...
			.normalize();
		const centroid = face
			.reduce((sum, idx) => sum.add(vertices[idx]), new Vector3())
