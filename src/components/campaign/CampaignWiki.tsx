...
import { BookOpen, Plus, Search, Edit, Trash2, Globe, Lock, FileText, MapPin, Users, HelpCircle } from 'lucide-react';
import { useCampaignWiki, WikiArticle } from '@/hooks/useCampaignWiki';
...
export function CampaignWiki({ campaignId }: { campaignId: string }) {
    const { articles, isLoading, addArticle, updateArticle, removeArticle } = useCampaignWiki(campaignId);
    ...
    const navigateToArticleTitle = (title: string) => {
        const target = articles.find(a => a.title.toLowerCase() === title.toLowerCase());
            ...
            <div className="col-span-1 border border-border bg-card rounded-md flex flex-col overflow-hidden">
                <div className="p-3 border-b border-border bg-muted/30">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        {hasDMAccess && (
                            <Button size="icon" variant="ghost" className="h-8 w-8 text-primary" onClick={handleCreateNew}>
                ...
                <ScrollArea className="flex-1 p-2">
                    {isLoading ? (
                        <p className="text-xs text-center text-muted-foreground py-4">Loading archives...</p>
                    ) : filteredArticles.length === 0 ? (
                        <p className="text-xs text-center text-muted-foreground py-4">No accessible articles found.</p>
                    ) : (
            ...
            <div className="col-span-1 lg:col-span-3 border border-border bg-card rounded-md flex flex-col overflow-hidden relative">
                {selectedArticle ? (
                        ...
                        <div className="p-3 border-b border-border bg-muted/20 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <Badge variant="outline" className={cn(
                                    "uppercase text-[10px]",
                                    selectedArticle.is_public ? "border-green-500/50 text-green-400" : "border-amber-500/50 text-amber-400"
                                )}>
                                    {selectedArticle.is_public ? <><Globe className="w-3 h-3 mr-1" /> Public</> : <><Lock className="w-3 h-3 mr-1" /> Hidden</>}
                                </Badge>
                            ...
                            {hasDMAccess && (
                                <div className="flex items-center gap-2">
                                    <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => handleEdit(selectedArticle)}>
                                        <Edit className="w-3 h-3 mr-1" /> Edit
                                    </Button>
                                    <Button size="icon" variant="outline" className="h-7 w-7 text-destructive hover:text-white hover:bg-destructive" onClick={() => handleDelete(selectedArticle.id, selectedArticle.title)}>
                                        ...
                                        components={{
                                            a: ({ node, href, children, ...props }) => {
                                                if (href?.startsWith('wiki://')) {
                                                    ...
                                                    const exists = articles.some(a => a.title.toLowerCase() === title.toLowerCase() && (hasDMAccess || a.is_public));
                                                    return (
                                                        <button
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                            ...
                                                            className={cn(
                                                                ...
                                                                exists ? "text-primary hover:bg-primary/20" : "text-muted-foreground decoration-destructive/50 hover:bg-destructive/10"
                                                            )}
                                                            title={exists ? `Go to ${title}` : `Article ${title} not found`}
                                                        >
                                                ...
                                                return <a href={href} target="_blank" rel="noreferrer" className="text-secondary hover:underline" {...props}>{children}</a>;
                                            }
                                        }}
                                    >
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-hatching opacity-50">
                        ...
                        <h2 className="font-display text-xl text-foreground">The System Lore Hub</h2>
                        <p className="text-sm text-muted-foreground max-w-md mt-2 font-heading">
                            Select an article from the index to read. Wrap titles in double brackets to create cross-links (e.g. <code>[[Shadow Monarch]]</code>).
                        </p>
                    </div>
                )}
            </div>
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col border-primary/20 bg-background/95 backdrop-blur-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Log information into the System database. Use Markdown for formatting.
                    </DialogDescription>
                </DialogHeader>

                <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-4 min-h-0 overflow-y-auto pr-2 pb-2">
                    ...
                    <div className="flex items-center gap-3 shrink-0 rounded-md border border-border p-3 bg-muted/20">
                        ...
                        <div className="flex flex-col">
                            <Label htmlFor="public-toggle" className="flex items-center gap-1.5 cursor-pointer">
                                {isPublic ? <Globe className="w-3.5 h-3.5 text-green-400" /> : <Lock className="w-3.5 h-3.5 text-amber-400" />}
                                {isPublic ? 'Public Record' : 'Restricted File (DM Only)'}
                            </Label>
                            <span className="text-[10px] text-muted-foreground">
                                {isPublic ? 'All Ascendants can read this article.' : 'Hidden from players.'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col min-h-[300px] space-y-2">
                        <Label className="flex justify-between items-end">
                            ...
                            <span className="text-[10px] text-muted-foreground font-normal">Link to other articles using [[Article Title]]</span>
                        </Label>
                        <Textarea
                            value={content}
                    ...
                    <DialogFooter className="shrink-0 pt-4">
                        ...
                        <Button type="submit" disabled={isSubmitting || !title.trim()} className="btn-umbral gap-2 shadow-primary/20">
