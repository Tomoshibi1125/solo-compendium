...
export function CampaignProtocolControls({ campaignId }: CampaignProtocolControlsProps) {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { data: members = [] } = useCampaignMembers(campaignId);
  const { data: invites = [], isLoading: invitesLoading } = useCampaignInvites(campaignId);
  const { data: inviteAuditLogs = [], isLoading: inviteAuditLoading } = useCampaignInviteAuditLogs(campaignId);
  ...
  const [inviteRole, setInviteRole] = useState<'hunter' | 'co-system'>('hunter');
  ...
  const [lootDraft, setLootDraft] = useState<LootItemDraft>({ name: '', quantity: 1, value: null });
  ...
  const handleExportEncounter = async (encounterId: string, encounterName: string) => {
    ...
    downloadJson(payload, `encounter-${encounterName.replace(/\s+/g, '-').toLowerCase()}.json`);
  };
  ...
  const handleDeployEncounter = async (encounterId: string) => {
    try {
      const sessionId = await deployEncounter.mutateAsync({ campaignId, encounterId });
      if (sessionId) {
        navigate(
  ...
  return (
    <div className="space-y-6" data-testid="campaign-protocol-controls">
      <SystemWindow title="INVITES & PERMISSIONS" data-testid="campaign-invites-panel">
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <Label htmlFor="invite-role">Role</Label>
              <Select value={inviteRole} onValueChange={(value) => setInviteRole(value as 'hunter' | 'co-system')}>
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              If provided, the app will attempt to send an auth invite email and still generate a copyable link.
            </p>
          </div>
          <Button
        ...
        <div className="mt-6 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {invitesLoading && <span className="text-xs text-muted-foreground">Loading…</span>}
          </div>
          {invites.length === 0 ? (
            ...
            invites.map((invite) => {
              ...
              const remainingUses = Math.max(invite.max_uses - invite.used_count, 0);
              const joinCode = invite.join_code;
              const joinCodeValue = joinCode ?? '';
              return (
                <div key={invite.id} className="rounded-lg border border-border bg-muted/30 p-3">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {inviteStatus !== 'active' && (
                          <Badge variant="destructive">{campaignInviteStatusLabel(inviteStatus)}</Badge>
                        )}
                        {inviteStatus === 'active' && (
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Uses {invite.used_count}/{invite.max_uses} · Remaining {remainingUses}
                      </p>
                      {invite.expires_at && (
                      ...
                      <p className="text-xs text-muted-foreground">
                        Join code{' '}
                        <span className="font-mono text-foreground" data-testid="campaign-invite-join-code">
                          {joinCode ?? 'N/A'}
                        </span>
                      </p>
                      {invite.invite_email && (
                        <p className="text-xs text-muted-foreground">Sent to {invite.invite_email}</p>
                      )}
                      <p className="text-xs text-muted-foreground mt-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleCopyInvite(joinCodeValue || invite.token)}
        ...
        <div className="mt-6 space-y-3" data-testid="campaign-invite-audit-log">
          <div className="flex items-center justify-between">
            ...
            {inviteAuditLoading && <span className="text-xs text-muted-foreground">Loading…</span>}
          </div>
          {inviteAuditLogs.length === 0 ? (
            <p className="text-sm text-muted-foreground">No invite audit events yet.</p>
          ) : (
            <div className="space-y-2">
              {inviteAuditLogs.slice(0, 8).map((entry) => (
                <div key={entry.id} className="rounded border border-border px-3 py-2 text-xs">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-heading text-foreground">{entry.action.replace(/_/g, ' ')}</span>
      ...
      <SystemWindow title="PROTOCOL RULES & ENFORCEMENT" data-testid="campaign-rules-panel">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-lg border border-border bg-muted/30 p-3">
              <div>
                <p className="font-heading font-semibold">Economy Enforcement</p>
                <p className="text-xs text-muted-foreground">Limit loot and relic values by System protocol.</p>
              </div>
              <Switch
                checked={rules.economy_enabled}
                onCheckedChange={(value) => setRules((prev) => ({ ...prev, economy_enabled: value }))}
              />
            </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setRules((prev) => ({
                      ...prev,
                      economy_max_loot_value: toNumberOrNull(event.target.value),
                    }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setRules((prev) => ({
                      ...prev,
                      economy_max_relic_value: toNumberOrNull(event.target.value),
                    }))
                  }
                />
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-lg border border-border bg-muted/30 p-3">
              <div>
                <p className="font-heading font-semibold">Protocol Enforcement</p>
                <p className="text-xs text-muted-foreground">Track System rule adherence and intervention.</p>
              </div>
              <Switch
                checked={rules.protocol_enforcement_enabled}
                onCheckedChange={(value) => setRules((prev) => ({ ...prev, protocol_enforcement_enabled: value }))}
              />
            </div>
            <div className="flex items-center justify-between rounded-lg border border-border bg-muted/30 p-3">
              <div>
                <p className="font-heading font-semibold">Failure Injection</p>
                <p className="text-xs text-muted-foreground">Inject System failures for dramatic tension.</p>
              </div>
              <Switch
                checked={rules.failure_injection_enabled}
                onCheckedChange={(value) => setRules((prev) => ({ ...prev, failure_injection_enabled: value }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setRules((prev) => ({
                      ...prev,
                      failure_injection_rate: Math.min(Math.max(Number(event.target.value) || 0, 0), 100),
                    }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setRules((prev) => ({ ...prev, failure_injection_note: event.target.value }))
                  }
                />
              </div>
            </div>
        ...
        {ruleEvents.length > 0 && (
          <div className="mt-4 space-y-2" data-testid="campaign-rule-events">
            <h4 className="text-xs font-display text-muted-foreground">Recent Rule Events</h4>
      ...
      <SystemWindow title="ENCOUNTER LIBRARY" data-testid="campaign-encounters-panel">
        <div className="space-y-3">
          {encounters.length === 0 ? (
            <p className="text-sm text-muted-foreground">No encounters saved yet.</p>
          ) : (
            encounters.map((encounter) => (
              <div key={encounter.id} className="rounded-lg border border-border bg-muted/30 p-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <Swords className="w-4 h-4 text-primary" />
                      <p className="font-heading font-semibold">{encounter.name}</p>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Updated {formatDistanceToNow(new Date(encounter.updated_at), { addSuffix: true })}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleExportEncounter(encounter.id, encounter.name)}
                    ...
                    <Button
                      size="sm"
                      variant="destructive"
                      onClick={() => deleteEncounter.mutate({ campaignId, encounterId: encounter.id })}
      ...
      <SystemWindow title="LOOT & RELIC DISTRIBUTION" data-testid="campaign-rewards-panel">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="sm:col-span-2">
                ...
                <Input
                  ...
                  onChange={(event) => setLootDraft((prev) => ({ ...prev, name: event.target.value }))}
                />
              </div>
              <div>
              ...
              <div className="flex items-end">
                <Button variant="outline" onClick={handleAddLootItem} className="w-full">
                  <Plus className="w-4 h-4 mr-2" />
                  Add
                </Button>
              </div>
            ...
            {lootItems.length > 0 && (
              <div className="space-y-2">
                {lootItems.map((item, index) => (
                  <div key={`${item.name}-${index}`} className="flex items-center justify-between rounded border border-border px-3 py-2">
                    <div>
                      <p className="text-sm font-heading">{item.name}</p>
                      <p className="text-xs text-muted-foreground">Qty {item.quantity} · Value {item.value ?? 'n/a'}</p>
                    </div>
                    <Button
                      size="icon"
                      variant="ghost"
                      onClick={() => setLootItems((prev) => prev.filter((_, idx) => idx !== index))}
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <SelectContent>
                    ...
                    {memberOptions.map((member) => (
                      <SelectItem key={member.id} value={member.id}>
                        {member.name} · Lv {member.level} {formatMonarchVernacular(member.job)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div>
                ...
                <Select value={lootEncounterId} onValueChange={setLootEncounterId}>
            ...
            <Button onClick={handleAssignLoot} className="w-full" data-testid="campaign-loot-assign">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input id="relic-name" value={relicName} onChange={(event) => setRelicName(event.target.value)} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <Button onClick={handleAssignRelic} className="w-full" data-testid="campaign-relic-assign">
                <Sparkles className="w-4 h-4 mr-2" />
                Assign Relic
              </Button>
            </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          {lootDrops.length === 0 && relicInstances.length === 0 ? (
            <p className="text-sm text-muted-foreground">No loot or relics recorded yet.</p>
          ) : (
            <div className="space-y-2">
              {lootDrops.slice(0, 5).map((drop) => (
                <div key={drop.id} className="rounded border border-border px-3 py-2 text-sm">
                  Loot drop · {new Date(drop.created_at).toLocaleString()}
                </div>
              ))}
              {relicInstances.slice(0, 5).map((relic) => (
                <div key={relic.id} className="rounded border border-border px-3 py-2 text-sm">
