...
type ChatFn = (message: string, type?: 'chat' | 'dice' | 'system' | 'whisper' | 'emote' | 'desc' | 'gmroll') => void;
...
export function VTTCharacterPanel({ characterId, onRoll, onChat, readOnly = false, compact = false }: VTTCharacterPanelProps) {
  ...
  const calculatedStats = useMemo(() => {
    ...
    const abilities = character.abilities || { STR: 10, AGI: 10, VIT: 10, INT: 10, SENSE: 10, PRE: 10 };
  ...
  const finalAbilities = useMemo(() => {
    if (!character) return { STR: 10, AGI: 10, VIT: 10, INT: 10, SENSE: 10, PRE: 10 };
    return character.abilities || { STR: 10, AGI: 10, VIT: 10, INT: 10, SENSE: 10, PRE: 10 };
  }, [character]);

  // Skills
  const allSkills = useMemo(() => getAllSkills(), []);
  const skills = useMemo(() => {
    if (!character || !calculatedStats) return {} as Record<string, { modifier: number; proficient: boolean; expertise: boolean; ability: AbilityScore }>;
    return allSkills.reduce<Record<string, { modifier: number; proficient: boolean; expertise: boolean; ability: AbilityScore }>>((acc, skill) => {
  ...
  if (isLoading) {
    return (
      <SystemWindow title="CHARACTER" compact>
        <div className="text-xs text-muted-foreground text-center py-4">Loading character...</div>
      </SystemWindow>
    );
  }
  ...
  if (!character || !calculatedStats) {
    return (
      <SystemWindow title="CHARACTER" compact>
        <div className="text-xs text-muted-foreground text-center py-4">Character not found.</div>
      </SystemWindow>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {/* Header */}
      <SystemWindow title={character.name?.toUpperCase() || 'CHARACTER'} compact>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1">
              <div className="flex items-center gap-1 text-xs">
                ...
                <span className={cn(hpPercent < 25 && 'text-red-400', hpPercent < 50 && hpPercent >= 25 && 'text-amber-400')}>
                  {hpCurrent}/{hpMax}
                </span>
              </div>
              <div className="flex items-center gap-1 text-xs">
        ...
        <div className="flex gap-1.5 mb-2">
          ...
          <Button
            ...
            size="sm"
            className="flex-1 text-[10px] h-7"
            onClick={() => rollCustom('Hit Die', `1d${character.hit_dice_size || 8}${getAbilityModifier(finalAbilities.VIT) >= 0 ? '+' : ''}${getAbilityModifier(finalAbilities.VIT)}`)}
          >
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {ABILITY_KEYS.map((ability) => {
            ...
            return (
              ...
              >
                <div className="text-[9px] text-muted-foreground uppercase tracking-wide">{ability}</div>
                <div className="font-display text-sm font-bold">{score}</div>
                <div className={cn('text-xs font-heading', mod >= 0 ? 'text-green-400' : 'text-red-400')}>
                  {formatMod(mod)}
          ...
          {ABILITY_KEYS.map((ability) => {
            ...
            return (
              ...
              >
                ...
                <span className={cn('font-bold', save >= 0 ? 'text-green-400' : 'text-red-400')}>
                  {formatMod(save)}
          ...
          {Object.entries(skills)
            ...
            .map(([name, skill]) => (
              ...
              >
                ...
                <span className={cn('font-bold', skill.modifier >= 0 ? 'text-green-400' : 'text-red-400')}>
        ...
        <SystemWindow title="PASSIVES" compact>
          <div className="grid grid-cols-3 gap-1.5 text-center">
            <div className="p-1.5 rounded border border-border/50 bg-muted/20">
              ...
              <div className="font-bold text-sm text-foreground">{10 + (skills['Perception']?.modifier ?? 0)}</div>
            </div>
            <div className="p-1.5 rounded border border-border/50 bg-muted/20">
              <div className="text-[9px] text-muted-foreground">Investigation</div>
              <div className="font-bold text-sm text-foreground">{10 + (skills['Investigation']?.modifier ?? 0)}</div>
            </div>
            <div className="p-1.5 rounded border border-border/50 bg-muted/20">
              <div className="text-[9px] text-muted-foreground">Insight</div>
              <div className="font-bold text-sm text-foreground">{10 + (skills['Insight']?.modifier ?? 0)}</div>
            </div>
          </div>
        </SystemWindow>
      )}
