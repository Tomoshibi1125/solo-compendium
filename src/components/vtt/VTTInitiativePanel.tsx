...
export function VTTInitiativePanel({
    ...
    const adjustHP = useCallback(
        (entry: TrackerEntry, delta: number) => {
            if (!activeSessionId) return;
            const newHp = Math.max(0, Math.min((entry.hp ?? 0) + delta, entry.maxHp ?? 999));
            upsertCombatants.mutate({
                campaignId,
                sessionId: activeSessionId,
                combatants: [
        ...
        <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                    <Button variant="ghost" size="icon" className="h-6 w-6" onClick={nextTurn} title="Next turn">
                        <SkipForward className="h-3 w-3" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-6 w-6" onClick={resetCombat} title="Reset combat">
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto">
                {sorted.map((entry, idx) => {
                    ...
                    const hpPct = entry.maxHp ? Math.round(((entry.hp ?? 0) / entry.maxHp) * 100) : null;
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className={cn('flex-1 truncate font-medium', isActive && 'text-primary')}>
                                    {entry.name}
                                </span>
                                {hpPct !== null && (
                                    <span
                                        className={cn(
                                            'text-[10px] font-mono',
                                            hpPct > 50 ? 'text-emerald-400' : hpPct > 25 ? 'text-amber-400' : 'text-red-400',
                                        )}
                                    >
                                ...
                                {entry.ac != null && (
                                    <span className="text-[10px] text-muted-foreground">AC{entry.ac}</span>
                                )}
                                {isExpanded ? (
            ...
            {isGM && (
                <div className="flex gap-1">
                    ...
                    <Input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && addCombatant()}
                    />
                    <Button variant="outline" size="icon" className="h-7 w-7" onClick={addCombatant}>
