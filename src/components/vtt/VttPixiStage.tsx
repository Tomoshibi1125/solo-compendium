...
import { createHexGrid, computeVisibilityPolygon } from '@/lib/vtt';
...
export function VttPixiStage({
  ...
  useEffect(() => {
    ...
    const renderWeather = () => {
      ...
      if (weather === 'rain') {
        config = {
          alpha: { list: [{ value: 0.5, time: 0 }, { value: 0.1, time: 1 }], isStepped: false },
          scale: { list: [{ value: 0.1, time: 0 }, { value: 0.8, time: 1 }], isStepped: false },
          color: { list: [{ value: "ffffff", time: 0 }, { value: "aaaaaa", time: 1 }], isStepped: false },
          speed: { list: [{ value: 800, time: 0 }, { value: 900, time: 1 }], isStepped: false },
        ...
        config = {
          alpha: { list: [{ value: 0.8, time: 0 }, { value: 0, time: 1 }], isStepped: false },
          scale: { list: [{ value: 0.5, time: 0 }, { value: 0.2, time: 1 }], isStepped: false },
          color: { list: [{ value: "ffffff", time: 0 }, { value: "ffffff", time: 1 }], isStepped: false },
          speed: { list: [{ value: 50, time: 0 }, { value: 100, time: 1 }], isStepped: false },
        ...
        config = {
          ...
          scale: { list: [{ value: 0.5, time: 0 }, { value: 0.1, time: 1 }], isStepped: false },
          color: { list: [{ value: "ff6600", time: 0 }, { value: "ff0000", time: 1 }], isStepped: false },
          speed: { list: [{ value: 100, time: 0 }, { value: 50, time: 1 }], isStepped: false },
        ...
        config = {
          alpha: { list: [{ value: 0, time: 0 }, { value: 0.4, time: 0.5 }, { value: 0, time: 1 }], isStepped: false },
          scale: { list: [{ value: 2, time: 0 }, { value: 4, time: 1 }], isStepped: false },
          color: { list: [{ value: "66aa66", time: 0 }, { value: "226622", time: 1 }], isStepped: false },
          speed: { list: [{ value: 20, time: 0 }, { value: 5, time: 1 }], isStepped: false },
    ...
    const renderWalls = () => {
      ...
      wg.stroke({ width: 4, color: wallColor, alpha: wallAlpha, join: 'miter', cap: 'square' });
      ...
      if (!isGM) {
        ...
        if (visionSources.length > 0) {
          ...
          for (const source of visionSources) {
            ...
            const polygon = computeVisibilityPolygon(vision, walls || [], gridSize * zoom, 72);
          ...
          darkness.rect(0, 0, (scene?.width || 100) * step, (scene?.height || 100) * step);
    ...
    const renderLighting = async () => {
      ...
      for (const light of lightSources) {
        ...
        lightG.fill({ color: Number(light.color.replace('#', '0x')), alpha: light.intensity * 0.3 });

        // Draw bright light
        lightG.circle(cx, cy, rBright);
        lightG.fill({ color: Number(light.color.replace('#', '0x')), alpha: light.intensity * 0.7 });
    ...
    const renderTokens = async () => {
      ...
      for (const token of visible) {
        ...
        if (!isOverlayToken && typeof token.hp === 'number' && typeof token.maxHp === 'number' && token.maxHp > 0) {
          ...
          hpFill.rect(size / 2 - barWidth / 2, size - 8, barWidth * hpPercent, barHeight);
          const barColor = hpPercent > 0.5 ? 0x22c55e : (hpPercent > 0.2 ? 0xeab308 : 0xef4444);
        ...
        if (!isOverlayToken && token.conditions && token.conditions.length > 0) {
          const conditionText = new Text({
            text: token.conditions.map(c => c.substring(0, 2).toUpperCase()).join(' '),
            style: {
              fill: 0xffffff,
              fontSize: 10,
              fontWeight: 'bold',
  ...
  useEffect(() => {
    ...
    const renderDrawingTemplate = (x1: number, y1: number, x2: number, y2: number) => {
      ...
      if (drawMode === 'ruler') {
        ...
        const label = new Text({ text: `${distFeet}ft`, style: { fill: 0xffffff, fontSize: 16, stroke: { color: 0x000000, width: 3 } } });
        label.x = x2 + 10;
        label.y = y2 + 10;
        g.addChild(label);
        ...
        const label = new Text({ text: `${distFeet}ft Cone`, style: { fill: 0xffffff, fontSize: 16, stroke: { color: 0x000000, width: 3 } } });
        label.x = x2 + 10;
        label.y = y2 + 10;
        for (let i = g.children.length - 1; i >= 0; i--) g.removeChild(g.children[i]);
        g.addChild(label);
        ...
        const label = new Text({ text: `${distFeet}ft Radius`, style: { fill: 0xffffff, fontSize: 16, stroke: { color: 0x000000, width: 3 } } });
        label.x = x2 + 10;
        label.y = y2 + 10;
        for (let i = g.children.length - 1; i >= 0; i--) g.removeChild(g.children[i]);
        g.addChild(label);
        ...
        const label = new Text({ text: `${sizeFeet}ft Cube`, style: { fill: 0xffffff, fontSize: 16, stroke: { color: 0x000000, width: 3 } } });
        label.x = x2 + 10;
        label.y = y2 + 10;
        for (let i = g.children.length - 1; i >= 0; i--) g.removeChild(g.children[i]);
        g.addChild(label);
