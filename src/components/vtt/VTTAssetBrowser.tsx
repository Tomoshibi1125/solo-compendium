...
export function VTTAssetBrowser({ onUseAsMap, onUseAsToken, onUseAsEffect, onShareHandout, campaignId, readOnly }: VTTAssetBrowserProps) {
  const { toast } = useToast();
  const { user } = useAuth();
  const [search, setSearch] = useState('');
  const [debouncedSearch, setDebouncedSearch] = useState('');
  const [activeCategory, setActiveCategory] = useState<VTTAssetCategory | null>(null);
  ...
  useEffect(() => { setVisibleCount(PAGE_SIZE); }, [debouncedSearch, activeCategory]);
  ...
  const recentAssets = useMemo(() => {
    ...
    return ids.map((id) => VTT_ASSET_LIBRARY.find((a) => a.id === id)).filter(Boolean) as VTTAsset[];
  ...
  const useAsset = useCallback((asset: VTTAsset, action: 'map' | 'token' | 'effect' | 'handout') => {
    ...
    else if (action === 'handout') onShareHandout?.(asset.imageUrl, asset.name);
  ...
  const handleDragStart = useCallback((e: React.DragEvent, asset: VTTAsset) => {
    e.dataTransfer.setData('application/vtt-asset', JSON.stringify({ imageUrl: asset.imageUrl, name: asset.name, category: asset.category }));
    e.dataTransfer.effectAllowed = 'copy';
  }, []);

  const handleUpload = useCallback(async (file: File, usage: 'map' | 'token') => {
    ...
    try {
      ...
      if (isSupabaseConfigured && user?.id) {
        ...
        const compressed = await compressImage(file, { maxWidth: maxSize, maxHeight: maxSize, quality: usage === 'map' ? 0.85 : 0.8, format: 'webp' });
        ...
        const { error: uploadError } = await supabase.storage
          ...
          .upload(fileName, compressed, { cacheControl: '3600', upsert: false, contentType: 'image/webp' });
      ...
      toast({ title: 'Uploaded!', description: `Image ready to use as ${usage}.` });
    } catch (err) {
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1">
        <button
          ...
          className={cn(
            ...
            !activeCategory ? 'bg-amber-500/20 border-amber-500/50 text-amber-200' : 'border-border/50 text-muted-foreground hover:bg-muted/30'
          )}
        >
          All ({TOTAL_ASSETS})
        </button>
        {VTT_ASSET_CATEGORIES.map((cat) => (
          <button
            ...
            className={cn(
              ...
              activeCategory === cat.id ? 'bg-amber-500/20 border-amber-500/50 text-amber-200' : 'border-border/50 text-muted-foreground hover:bg-muted/30'
            )}
          >
      ...
      {recentAssets.length > 0 && (
        <div>
          ...
          >
            ...
            <ChevronDown className={cn('w-3 h-3 ml-auto transition-transform', showRecent && 'rotate-180')} />
          </button>
          {showRecent && (
              ...
              {recentAssets.map((asset) => (
                ...
                >
                  <div className="aspect-square bg-muted/20 overflow-hidden">
                    <OptimizedImage src={asset.thumbnailUrl || asset.imageUrl} alt={asset.name} className="w-full h-full object-cover" size="thumbnail" />
                  </div>
                </button>
              ))}
            </div>
      ...
      {!readOnly && (
        <div className="flex gap-1.5">
          <input ref={uploadRef} type="file" accept="image/*" className="hidden" title="Upload image" onChange={(e) => {
      ...
      <div className="text-[10px] text-muted-foreground px-1">
        ...
        {activeCategory && ` in ${VTT_ASSET_CATEGORIES.find((c) => c.id === activeCategory)?.label}`}
        {hasMore && ` (showing ${results.length})`}
      </div>

      {/* Thumbnail grid */}
      <div ref={gridRef} className="grid grid-cols-3 gap-1.5 max-h-[400px] overflow-y-auto pr-0.5">
        {results.map((asset) => (
          <button
            key={asset.id}
            onClick={() => trackAndPreview(asset)}
            ...
            className={cn(
              ...
              previewAsset?.id === asset.id ? 'border-amber-500 ring-1 ring-amber-500/30' : 'border-border/40 hover:border-border'
            )}
          >
            <div className="aspect-square bg-muted/20 relative overflow-hidden">
              ...
              <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/60 to-transparent p-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-[8px] text-white/80">{CATEGORY_ICONS[asset.category]}</span>
              </div>
            </div>
            <div className="p-1 bg-background/80">
              <div className="text-[9px] truncate font-medium">{asset.name}</div>
              {asset.rank && (
        ...
        <SystemWindow title={previewAsset.name} compact>
          <div className="space-y-2">
            ...
            {previewAsset.description && (
              <p className="text-[10px] text-muted-foreground line-clamp-2">{previewAsset.description}</p>
            )}
            <div className="flex flex-wrap gap-1">
              {previewAsset.tags.map((tag) => (
                <span key={tag} className="text-[8px] px-1.5 py-0.5 rounded-full bg-muted/30 border border-border/30 text-muted-foreground">
                  {tag}
                </span>
              ))}
            </div>
            <div className="flex gap-1.5 flex-wrap">
              ...
              {(previewAsset.category === 'map' || previewAsset.category === 'location') && onUseAsMap && (
                <Button variant="default" size="sm" className="flex-1 text-[10px] h-7" onClick={() => useAsset(previewAsset, 'map')}>
                  <MapPin className="w-3 h-3 mr-1" /> Use as Map
                </Button>
              )}
              {(previewAsset.category === 'monster' || previewAsset.category === 'token' || previewAsset.category === 'portrait') && onUseAsToken && (
                <Button variant="default" size="sm" className="flex-1 text-[10px] h-7" onClick={() => useAsset(previewAsset, 'token')}>
                  <Image className="w-3 h-3 mr-1" /> Place Token
                </Button>
              )}
              {(previewAsset.category === 'effect' || previewAsset.category === 'condition' || previewAsset.category === 'technique') && onUseAsEffect && (
                <Button variant="default" size="sm" className="flex-1 text-[10px] h-7" onClick={() => useAsset(previewAsset, 'effect')}>
              ...
              {(previewAsset.category === 'handout') && onShareHandout && (
                <Button variant="default" size="sm" className="flex-1 text-[10px] h-7" onClick={() => useAsset(previewAsset, 'handout')}>
              ...
              {previewAsset.category !== 'map' && previewAsset.category !== 'location' && onUseAsMap && (
                <Button variant="outline" size="sm" className="text-[10px] h-7 px-2" onClick={() => useAsset(previewAsset, 'map')} title="Use as map background">
                  <MapPin className="w-3 h-3" />
                </Button>
              )}
              {previewAsset.category !== 'monster' && previewAsset.category !== 'token' && previewAsset.category !== 'portrait' && onUseAsToken && (
                <Button variant="outline" size="sm" className="text-[10px] h-7 px-2" onClick={() => useAsset(previewAsset, 'token')} title="Place as token">
                  <Image className="w-3 h-3" />
                </Button>
              )}
              {previewAsset.category !== 'effect' && previewAsset.category !== 'condition' && previewAsset.category !== 'technique' && previewAsset.category !== 'spell' && previewAsset.category !== 'prop' && onUseAsEffect && (
                <Button variant="outline" size="sm" className="text-[10px] h-7 px-2" onClick={() => useAsset(previewAsset, 'effect')} title="Place as effect overlay">
                  <Sparkles className="w-3 h-3" />
                </Button>
              )}
              {onShareHandout && previewAsset.category !== 'handout' && (
                <Button variant="outline" size="sm" className="text-[10px] h-7 px-2" onClick={() => useAsset(previewAsset, 'handout')} title="Share as handout">
