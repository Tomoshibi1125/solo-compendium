...
export function MarketplaceWorkbench() {
  ...
  const parsedTags = useMemo(
    () => tagsText.split(',').map((tag) => tag.trim()).filter((tag, index, all) => tag && all.indexOf(tag) === index),
    [tagsText]
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Could not parse content JSON.',
        variant: 'destructive',
      });
      return;
    }
