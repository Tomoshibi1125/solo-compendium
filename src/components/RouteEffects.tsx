...
export function RouteEffects() {
	...
	useEffect(() => {
		...
		const updateTitle = async () => {
			...
			const label = formatRegentVernacular(
				COMPENDIUM_LABELS[type] ?? "Compendium",
			);
			const resolvedName =
				(entry as { display_name?: string | null; title?: string | null })
					.display_name ||
				(entry as { title?: string | null }).title ||
				entry.name;
			setDynamicTitle(
				`${BASE_TITLE} - ${label}: ${formatRegentVernacular(resolvedName)}`,
			);
		};
