...
function _transformRune(
	...
	return {
		...
		rune_level: rune.rune_level ?? rune.power ?? null,
...
export const staticDataProvider: StaticDataProvider = {
	...
	getShadowSoldiers: async (search?: string) => {
		const { shadowSoldiers } = await import("../shadow-soldiers");
		...
		return filtered.map((soldier: any) => ({
			...
			agi: soldier.stats.agility,
			vit: soldier.stats.vitality,
			int: soldier.stats.intelligence,
			sense: soldier.stats.sense,
			pre: soldier.stats.presence,
			saving_throws: soldier.saving_throws as Record<string, number>,
			skills: soldier.skills as Record<string, number>,
			damage_resistances: soldier.damage_resistances,
			damage_immunities: soldier.damage_immunities,
			condition_immunities: soldier.condition_immunities,
			senses: { text: soldier.senses } as unknown as Record<string, string>,
			languages: [soldier.languages],
			cr: String(soldier.challenge_rating),
			is_boss: soldier.rank === "A",
			Anomaly_traits: soldier.traits as unknown as Record<string, Json>[],
			Anomaly_actions: soldier.actions as unknown as Record<string, Json>[],
			mechanics: soldier.mechanics as unknown as Record<string, Json>,
			limitations: soldier.limitations as unknown as Record<string, Json>,
			level: undefined,
		}));
	},
