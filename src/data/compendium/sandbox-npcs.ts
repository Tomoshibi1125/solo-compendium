...
// 8 Bureau (material-side) + 64 native (Vermillion, Awoko, Independent, Domain-Touched).
...
// and the Regent's Court (the Bailiff, chamberlain, heralds, and the First Claimed).
...
const bureauSentinels: SandboxNPC[] = [
	...
	makeNPC({
		...
		recruitCondition:
			"Repair a relay inside the Hollow Way or carry a booster through the Road of Writs.",
		isRecruitable: true,
		guildAffiliation: null,
...
const vermillionGuild: SandboxNPC[] = [
	makeNPC({
		...
		hp: 40,
		ac: 14,
		description: "The Outpost's weapons-fence: salvaged edged steel, Relic-arms the Domain grew teeth into, and Essence-cells for anything that drinks them.",
		personality: "Cheerful, amoral about buyers, deadly serious about misfires.",
		motivation: "Arm anyone who can pay, then sleep at night by never selling a curse he hasn't named.",
		backstory: "Grist strips weapons from the Domain's deadliest sites and tests every one himself before it touches a shelf. He has the scars to prove which ones he sold anyway.",
		keyAbilities: ["Salvaged Arsenal", "Relic-Weapon Lore", "Essence-Cell Supply"],
		recruitCondition: "Return a Relic-weapon that turned on its buyer; he repays it in better steel.",
		isRecruitable: true,
		guildAffiliation: "The Vermillion",
		location: "Vermillion Outpost - Armoury stall",
		questHook: "Grist sold a blade that has started cutting toward the Citadel of its own accord, dragging its owner with it.",
	}),
	makeNPC({
		id: "npc-verm-202",
		name: "Quill",
		...
		hp: 30,
		ac: 13,
		description: "A trader in memory-cores harvested from the Remembering Orchard, who can sell you a skill, a language, or a stranger's happiest day — at the price of a little of your own.",
		personality: "Smooth, melancholy, scrupulous about consent in a trade everyone else cheats.",
		motivation: "Keep the memory-trade honest in a Domain that wants it predatory.",
		backstory: "Quill was a tither before he was a broker, and remembers exactly what it costs to give a piece of yourself away. He will not sell a core that was taken, only one that was given. It makes him poor and alive.",
		keyAbilities: ["Memory Appraisal", "Skill-Core Transfer", "Detects Stolen Memory"],
		recruitCondition: "Recover a stolen memory-core and let him return it instead of selling it.",
		isRecruitable: true,
		guildAffiliation: "The Vermillion",
		location: "Vermillion Outpost / Covered Market",
		questHook: "Quill has bought a memory that does not belong to anyone alive — and it is starting to want its body back.",
	}),
	makeNPC({
		id: "npc-verm-203",
		name: "Captain Doe",
		...
		hp: 46,
		ac: 15,
		description: "Runs contraband — people, Relics, and forbidden Runes — between settlements along roads the Domain has not finished closing.",
		personality: "Cool, calculating, sentimental about exactly one thing she will never name.",
		motivation: "Move what the Domain forbids, and get every passenger to the other end breathing.",
		backstory: "Doe knows which stretches of the Road of Writs the Regent's officers have not yet learned to watch. She has lost two crews to roads that learned faster than she did, and runs the routes anyway because someone has to carry the people the writs are for.",
		keyAbilities: ["Smuggling Routes", "Hidden Cargo", "Outruns the Bailiff"],
		recruitCondition: "Help her run a cargo of refugees past a Bailiff checkpoint without paying in names.",
		isRecruitable: true,
		guildAffiliation: "The Vermillion",
		location: "The roads / Vermillion Outpost",
		questHook: "Doe's current cargo is a person the Regent's court has formally requested by name. She has not decided whether to deliver or run.",
	}),
	makeNPC({
		id: "npc-verm-204",
		name: "Tallow",
		...
		hp: 24,
		ac: 12,
		description: "A peddler of wards, tokens, and anti-Regent-Marked charms — most of them comforting junk, a precious few of them genuinely real.",
		personality: "Patter-fast, superstitious in earnest, kinder than the grift suggests.",
		motivation: "Sell hope by the handful, and slip the real charms to the people who truly need them.",
		backstory: "Tallow learned which of his charms actually work the hard way, by surviving. He overcharges the comfortable to undercharge the doomed, and tells himself this evens out.",
		keyAbilities: ["Genuine Ward (rare)", "Reads the Marked", "Sells Hope"],
		recruitCondition: "Catch him slipping a real charm to a doomed family, and keep his secret.",
		isRecruitable: true,
		guildAffiliation: "The Vermillion",
		location: "Covered Market / road camps",
		questHook: "Tallow accidentally sold a genuine Anchor-ward to someone the Regent specifically wants Marked, and the court has noticed.",
	}),
	makeNPC({
		id: "npc-verm-101",
		name: "Cinder",
		...
		motivation: "Pull enough Essence to buy her crew off the Domain's rolls for good.",
		backstory:
	...
	makeNPC({
		...
		motivation: "Keep the Outpost's stock moving and its worst Relics out of the wrong hands.",
		backstory:
		...
		questHook:
			"Vell has identified one of the party's Relics as something the Regent is missing — and quietly looking for.",
	}),
	makeNPC({
		id: "npc-verm-103",
		name: "Pip",
		...
		personality: "Eager, loyal, and overconfident about the Road of Writs.",
...
const awokoCult: SandboxNPC[] = [
	...
	makeNPC({
		...
		personality: "Numb, aching, and looking for a reason that is not the Hollow Mother's.",
...
const independents: SandboxNPC[] = [
	makeNPC({
		...
		hp: 18,
		ac: 12,
		description: "The Covered Market's main provisioner — rope, oil, rations, lamp-Essence, and whatever the party forgot they needed until the road took it.",
		personality: "Brisk, fair, allergic to drama, and quietly generous to children.",
		motivation: "Keep his stall solvent and his suppliers (and their secrets) safe.",
		backstory: "Holt has outlasted three market fires and one Bailiff inspection by the simple trick of never owing anyone a favour he could not repay by dawn.",
		keyAbilities: ["Stocks Almost Anything", "Fair Prices", "Supplier Network"],
		recruitCondition: "Settle a debt a rival pinned on him; he repays loyalty in resupply at any hour.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Covered Market - Holt's Goods",
		questHook: "Holt's supplier upriver has stopped sending caravans. The last one came back empty, driving itself.",
	}),
	makeNPC({
		id: "npc-ind-202",
		...
		hp: 24,
		ac: 12,
		description: "The closest thing the Gloamreach has to a bank: she changes Essence-cores, favours, debts, and writs into one another, and never blinks at the rate.",
		personality: "Precise, unflappable, and fluent in exactly what a thing is worth to the person holding it.",
		motivation: "Stay the one ledger the Regent's court does not fully control.",
		backstory: "Esha can read the true value of anything offered across her counter — including the cost a buyer is hiding from themselves. She has refused the Regent's court twice. There will not be a third polite request.",
		keyAbilities: ["True-Value Sight", "Favour Exchange", "Reads the Hidden Cost"],
		recruitCondition: "Bring her a debt the Regent's court wants called in, and let her be the one to settle it.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Covered Market - The Counting-Table",
		questHook: "Esha is holding a writ in escrow that names a party member as collateral. She would rather not honour it.",
	}),
	makeNPC({
		id: "npc-ind-203",
		name: "Inkwright",
		...
		hp: 28,
		ac: 13,
		description: "A quiet calligrapher who sells false names, forged writs, and invitations that were never really sent — the single most useful and most dangerous trade in the Domain.",
		personality: "Meticulous, nervous, proud of work no one is supposed to admire.",
		motivation: "Stay one forgery ahead of the Domain's own handwriting.",
		backstory: "Inkwright learned that the Gloamreach's law is only as strong as the documents it trusts. He can write a name the road has no claim to — but every forgery the Domain catches teaches it to read him better.",
		keyAbilities: ["False Name", "Forged Writ", "Counterfeit Invitation"],
		recruitCondition: "Protect his workshop from a Bailiff audit; a forger who trusts you is worth an army of keys.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Writ-Bound Hamlet - back room",
		questHook: "Inkwright forged a writ that worked too well, and now the Domain is enforcing a charge that does not exist.",
	}),
	makeNPC({
		id: "npc-ind-204",
		name: "Goodman Pell",
		...
		hp: 30,
		ac: 14,
		description: "Keeper of the one rooming-house on the road that honours guest-right — meaning the one place a long rest is possible, and the one place a careless oath becomes a leash.",
		personality: "Gruff, hospitable, and rigid about the rules that keep his roof standing.",
		motivation: "Offer true shelter without his hearth becoming the Domain's trap.",
		backstory: "Pell's guest-right is real and old, older than the Regent's claim on the road, which is the only reason it still holds. He enforces its courtesies to the letter, because the letter is the wall.",
		keyAbilities: ["Guest-Right Sanctuary", "Reads a Liar", "Old Hospitality Law"],
		recruitCondition: "Honour his house's guest-right exactly once, even when breaking it would be easier.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Road of Writs - The Last Hearth",
		questHook: "A Regent courtier has accepted Pell's guest-right and will not leave, turning his sanctuary into a siege of manners.",
	}),
	makeNPC({
		id: "npc-ind-205",
		name: "Herbalist Wen",
		...
		hp: 22,
		ac: 12,
		description: "A native apothecary who brews against Rift-Rot, spore-sickness, and the slow grey numbness the Domain leaves in people who stay too long.",
		personality: "Soft-spoken, exacting, and unwilling to lie about what a cure will cost.",
		motivation: "Keep one settlement breathing against a sickness that is partly the land itself.",
		backstory: "Wen's remedies work because she learned which of the Fungal Depths' growths heal and which only pretend to. She has been wrong exactly once, and she keeps that grave tended herself.",
		keyAbilities: ["Rift-Rot Cure", "Field Apothecary", "Knows the Safe Growths"],
		recruitCondition: "Recover a rare growth from the Fungal Depths without letting it cultivate you first.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Tribute settlements - Wen's stillroom",
		questHook: "Wen's whole stock of a key remedy has started curing the wrong illness — and inflicting a new one.",
	}),
	makeNPC({
		id: "npc-ind-206",
		name: "Sexton Mort",
		...
		hp: 30,
		ac: 13,
		description: "The man who buries the settlements' dead, and the only one who knows which graves to weigh down so their occupants stay in them.",
		personality: "Calm, fatalistic, surprisingly funny, and never surprised.",
		motivation: "Make sure the dead get to be only dead, which the Domain makes harder every season.",
		backstory: "Mort keeps two ledgers: who died, and who stayed dead. The second is shorter than it should be. He marks the difference himself, with iron and salt and a name read backward.",
		keyAbilities: ["Knows the Restless Dead", "Grave-Wards", "Final Rites"],
		recruitCondition: "Help him put down something that walked home from the Fungal Depths' Nursery.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Tribute settlements - the boneyard",
		questHook: "Mort's own name has begun appearing in the second ledger, in his own handwriting, dated next week.",
	}),
	makeNPC({
		id: "npc-ind-207",
		name: "Auntie Rell",
		...
		hp: 16,
		ac: 11,
		description: "The hub of every rumour in three settlements, who hears everything because no one thinks to guard their tongue around an old woman shelling beans.",
		personality: "Warm, nosy, sharp as a tack, and loyal to whoever's kind to the children.",
		motivation: "Know everything first, and trade it only to people who deserve to know.",
		backstory: "Rell has watched the Domain take neighbours one polite invitation at a time, and remembers every name the road tried to make her forget. Her memory is a small rebellion she has never been caught at.",
		keyAbilities: ["Knows Everyone", "Rumour Network", "Remembers the Forgotten"],
		recruitCondition: "Bring her real news from a settlement that has gone silent; she pays in everything she knows.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Covered Market / settlement doorsteps",
		questHook: "Rell has noticed a name vanishing from everyone's memory but hers, and she is the next to forget it.",
	}),
	makeNPC({
		id: "npc-ind-208",
		name: "Captain Hollow",
		...
		hp: 48,
		ac: 16,
		description: "The closest thing a tribute-settlement has to a guard: a hard, scarred captain holding a wall against things no wall was built for.",
		personality: "Blunt, exhausted, fiercely protective, and out of good options.",
		motivation: "Keep his people alive without giving the Regent a reason to make an example of them.",
		backstory: "Hollow trains farmers to hold a line they cannot win, because a line held buys time and time buys lives. He knows the Bailiff could end his militia in a single night. He drills them anyway.",
		keyAbilities: ["Hold the Line", "Settlement Defence", "Improvised Tactics"],
		recruitCondition: "Win one fight for his settlement that he could not, and let him keep the credit with his people.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Writ-Bound Hamlet / tribute settlements",
		questHook: "Hollow has been served a writ ordering him to surrender three of his own militia by name. He has not told them yet.",
	}),
	makeNPC({
		id: "npc-ind-209",
		name: "Sile",
		...
		hp: 30,
		ac: 14,
		description: "A guide who can walk the Road of Writs without being rearranged by it, for a fee, and who has never once promised to bring everyone back.",
		personality: "Laconic, honest about odds, superstitious about thanking the road.",
		motivation: "Walk every road in the Gloamreach once before one of them keeps her.",
		backstory: "Sile reads the moving milestones the way sailors once read stars. She charges in advance because the road sometimes collects the guide instead of the fare, and she will not leave a debt for her sister.",
		keyAbilities: ["Reads the Moving Road", "Shortcut Sense", "Never Pays in Names"],
		recruitCondition: "Trust her route once when it makes no sense; she does not waste a second chance.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "The Road of Writs",
		questHook: "Sile's sister took a road Sile warned her against, and the milestone where she vanished has started moving toward town.",
	}),
	makeNPC({
		id: "npc-ind-210",
		...
		hp: 14,
		ac: 11,
		description: "A market cook whose hot food is genuinely restorative and genuinely the safest meal for miles — because he refuses, on principle, to serve anything that comes with a string attached.",
		personality: "Loud, big-hearted, stubborn about feeding people for free when he can afford it.",
		motivation: "Make one place in the Gloamreach where a meal is just a meal.",
		backstory: "Bram learned to cook in a settlement that paid its tribute in food, and swore that whatever he served would never be part of a bargain. The Domain finds this quietly offensive.",
		keyAbilities: ["Restorative Meal (1d4+1 temp HP)", "No Strings Attached", "Knows the Market"],
		recruitCondition: "Defend his stall from an Awoko comfort-worker trying to lace his bread with grief.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "Covered Market - Bram's fire",
		questHook: "Bram's free meals have made him a problem: the Domain prefers its people to owe for their food.",
	}),
	makeNPC({
		id: "npc-ind-101",
		name: "Elder Saph",
		...
		motivation: "Keep her people off the Bailiff's roster for one more season.",
		backstory:
	...
	makeNPC({
		...
		ac: 13,
		description:
			"The man who rings the tribute-bells because someone must, and because the Domain chose his hands.",
		personality: "Hollowed out, precise, and kinder than the work should allow.",
		motivation: "Ring only what he must, and warn whom he can with how he rings it.",
		backstory:
			"Every settlement bell answers another. Tomas has learned to hide a half-second of warning inside an obligatory peal, and lives in terror of the day the Domain notices the difference.",
	...
	makeNPC({
		...
		motivation: "Stay uncatalogued, get paid, keep her little brother off the rolls.",
		backstory:
			"Needle has memorised which milestones move and which stay put. The Domain has not written her name yet, and she means to keep it that way for as long as a child can.",
	...
	makeNPC({
		...
		ac: 16,
		description:
			"A native tithe-collector in the Regent's livery who hates the work and softens it wherever the Domain is not looking.",
		personality:
		...
		keyAbilities: ["Domain Livery (passes writs)", "Knows the Rolls", "Reluctant Authority"],
		recruitCondition:
			"Give him a way to fail his quota that the Domain cannot trace back to him.",
		isRecruitable: true,
		guildAffiliation: null,
	...
	makeNPC({
		...
		backstory:
			"Yu-jin tended the settlements' beasts long before the Domain's mana taught her that some of its monsters respond to care before command.",
...
const anomalyAdjacent: SandboxNPC[] = [
	// --- The Regent's Court: named antagonists, bound to the Domain (not recruitable). ---
	makeNPC({
		id: "npc-court-001",
		name: "The Regent's Bailiff",
		...
		hp: 140,
		ac: 18,
		description: "The Regent's will given a body: a towering figure of black iron and counted chains that arrives before the violence, speaks your names in the voices of your dead, and collects what the Domain has marked.",
		personality: "Implacable, unhurried, and utterly certain — it has all the time there is, and it is already on its way.",
		motivation: "Collect what the Domain has marked, and never stop until it has.",
		backstory: "The Bailiff is not cruel; it is inevitable. It can be hidden from, outrun, warded off, or turned aside by a Claim or an older power — but never bribed, never bargained with, and never made to forget. Every settlement knows the sound of its chains being counted up the road. (See Warden Secret 4.)",
		keyAbilities: ["Speaks Your Names", "Counted Chains", "Cannot Be Bargained With", "Always Coming"],
		recruitCondition: "Cannot be recruited. It can be hidden from, outrun, warded against, or turned aside with a Claim — never befriended, never bargained with.",
		isRecruitable: false,
		guildAffiliation: "The Regent's Court",
		location: "Anywhere the Domain has marked someone",
		questHook: "The Bailiff has spoken a party member's name, and will keep coming — patient, certain, unhurried — until that mark is lifted or the Anchor that sent it is broken.",
	}),
	makeNPC({
		id: "npc-court-002",
		name: "Chamberlain Vire",
		title: "Keeper of Invitations",
		faction: "anomaly_adjacent",
		level: 9,
		job: "Herald",
		hp: 70,
		ac: 15,
		description: "The court official who writes the Regent's invitations in a beautiful hand, addresses them to people who have not done anything yet, and is never, ever in a hurry.",
		personality: "Elegant, patient, delighted by good manners and by their absence.",
		motivation: "See every name in the Domain properly invited, and properly answered.",
		backstory: "Vire decides who the Regent notices. An invitation from Vire is a courtesy and a sentence; refusing one twice is how the Bailiff learns your name. Vire has been doing this since long before the road learned to move.",
		keyAbilities: ["Compelling Invitation", "Court Etiquette", "Knows Who Refused"],
		recruitCondition: "Cannot be recruited. Vire can be out-mannered, given a false name to enter, or tricked into inviting the wrong guest.",
		isRecruitable: false,
		guildAffiliation: "The Regent's Court",
		location: "The roads / the Regent's Citadel",
		questHook: "Vire has sent the party a gilded invitation to dine at the Citadel. The date is already in the past.",
	}),
	makeNPC({
		id: "npc-court-003",
		name: "The Mute Herald",
		title: "Voice of the Throne",
		faction: "anomaly_adjacent",
		level: 7,
		job: "Esper",
		hp: 52,
		ac: 14,
		description: "A claimed echo with no mouth that speaks the Regent's words directly into the minds of those it is sent to, in their own remembered voices.",
		personality: "None of its own left; it is a mouthpiece wearing the cadence of whoever it quotes.",
		motivation: "Deliver the Regent's word, exactly, and watch it land.",
		backstory: "The Herald was a town crier once, in a settlement the Domain dissolved. The Regent kept the voice and discarded the rest. It now announces collections, verdicts, and invitations in the voices of the listeners' own dead.",
		keyAbilities: ["Speaks the Regent's Will", "Voice of the Lost", "Mind-Cast Decree"],
		recruitCondition: "Cannot be recruited. Restoring even one of the voices it carries can briefly silence it.",
		isRecruitable: false,
		guildAffiliation: "The Regent's Court",
		location: "Wherever the Regent has something to say",
		questHook: "The Herald has begun speaking to a party member in the voice of someone they lost — and the message is an offer.",
	}),
	makeNPC({
		id: "npc-court-004",
		...
		hp: 60,
		ac: 14,
		description: "The desiccated keeper who remembers every soul the Gloamreach has ever taken — name, face, and the manner of its taking — and who tends the heart of the fire at the Ashen Counting-House.",
		personality: "Fussy, proud, terrified of a name forgotten, and incapable of mercy that leaves a soul uncounted.",
		motivation: "Keep the tally complete, because a soul remembered is a soul the Domain still holds.",
		backstory: "Ont has named the claimed dead of every settlement for longer than any of them have stood. He knows precisely who the Gloamreach has taken and who it is still owed, and a forgotten name offends him more than any cruelty.",
		keyAbilities: ["The Tally of the Dead", "Names the Claimed", "Marks the Living"],
		recruitCondition: "Cannot be recruited. Ont can be undone by a name he cannot account for — a claimed soul set free, or a false name he records as true.",
		isRecruitable: false,
		guildAffiliation: "The Regent's Court",
		location: "Ashen Counting-House / the Citadel",
		questHook: "Ont has found a name missing from his tally — Reeve Dunn's settlement is short a soul it owes — and he is coming, in person, to collect the difference in flesh.",
	}),
	makeNPC({
		id: "npc-court-005",
		name: "Lord Sered",
		...
		hp: 110,
		ac: 16,
		description: "The oldest of the Regent's bound servants — a courtly, hollow-eyed lord who was a man before the Domain, and remembers being one the way one remembers a country one can never return to.",
		personality: "Gracious, grieving, utterly loyal, and the closest thing the Regent has to a friend.",
		motivation: "Serve the Regent, because the alternative is to admit what serving has cost.",
		backstory: "Sered was the first person the Gloamreach claimed and kept whole, and the Regent has never let him go. He hosts the party at the Citadel with real warmth and real warning, and he cannot, will not, be turned — unless shown the one thing he has spent an age refusing to look at.",
		keyAbilities: ["Courtly Authority", "Citadel Command", "Almost Human"],
		recruitCondition: "Cannot be recruited normally. A Claim contradiction, or proof of who he was, can make him hesitate at the worst moment for the Regent.",
		isRecruitable: false,
		guildAffiliation: "The Regent's Court",
		location: "The Regent's Citadel",
		questHook: "Lord Sered keeps inviting the party to leave the Gloamreach while they still can. He means it. He cannot follow his own advice.",
	}),
	makeNPC({
		id: "npc-court-006",
		name: "The Doorward",
		title: "Keeper of the Outer Court",
		faction: "anomaly_adjacent",
		level: 9,
		job: "Destroyer",
		hp: 96,
		ac: 17,
		description: "The thing that checks invitations at the Citadel's threshold — part golem, part custom, entirely certain of who is and is not permitted.",
		personality: "Literal, ceremonial, and without a flicker of doubt.",
		motivation: "Admit the invited, turn away the rest, and remember every face that tries the second.",
		backstory: "The Doorward has stood at the Outer Court since the Citadel first cast its shadow. It recognises a false name once; the second time, it remembers. Broken promises become hostile witnesses in its presence.",
		keyAbilities: ["Checks the Invitation", "Remembers False Names", "Threshold Authority"],
		recruitCondition: "Cannot be recruited. A valid (or convincingly forged) invitation gets the party past it; a broken promise gets them remembered.",
		isRecruitable: false,
		guildAffiliation: "The Regent's Court",
		location: "The Regent's Citadel - Outer Court",
		questHook: "The Doorward has the party's true names among those to be turned away — unless they arrive carrying something the threshold itself cannot refuse.",
	}),
	makeNPC({
		id: "npc-anom-101",
		name: "Lantern",
		...
		motivation: "Lead the lost to somewhere the Domain is not watching.",
		backstory:
			"Lantern was a settlement child once, claimed young and kept as a guide-light by a Domain that found it useful. Enough of the child remains to want to help — and to flinch at the dark it came from.",
		keyAbilities: ["Safe-Path Light", "Senses the Bailiff", "Cannot Speak"],
		recruitCondition:
			"Follow its light once without exploiting it, and never let the dark take it back.",
		isRecruitable: true,
		guildAffiliation: null,
		location: "The Hollow Way / dark roads",
		questHook: "Lantern is leading the party somewhere — and something older is leading Lantern.",
	}),
	makeNPC({
		id: "npc-anom-102",
		name: "The Counted",
		...
		ac: 14,
		description:
			"A native who was tithed to the Domain and came back — not dead, not alive, and not quite one person anymore.",
		personality: "Speaks in the plural; calm; unbearably sad; honest to a fault.",
		motivation: "Be counted as people again, not as a balance paid.",
		backstory:
			"The Domain took the Counted as tribute and returned what it did not need: a body, a voice, and the arithmetic of everyone it was made from. They remember being loved. They cannot remember by whom.",
		keyAbilities: ["Speaks for the Claimed", "Ledger-Sense", "Unkillable by Halves"],
		recruitCondition:
		...
		questHook:
			"The Counted can testify at the Citadel to exactly what tribute costs — if they survive being remembered.",
	}),
	makeNPC({
		id: "npc-anom-001",
		name: "Echo-7",
	...
	makeNPC({
		...
		isRecruitable: true,
		guildAffiliation: null,
		location: "Hollow Way or Road of Writs",
		questHook:
