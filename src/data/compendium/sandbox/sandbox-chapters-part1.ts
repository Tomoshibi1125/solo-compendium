import type { SandboxChapter } from "../ascendant-sandbox-module";

// ============================================================================
// THE SHADOW OF THE REGENT - CHAPTERS 1-8
// Part I: Cordon, Entry, Domain Travel, Factions, and First Clears
// ============================================================================

export const chaptersPart1: SandboxChapter[] = [
	{
		title: "Chapter 1: Warden's Briefing - Entering the Gloamreach",
		content: `# The Shadow of the Regent

## Campaign Overview

**The Shadow of the Regent** is a non-linear horror sandbox for 3-6 Ascendants, taking characters from Level 1 to Level 15. It is set inside **the Gloamreach**, an S-Rank Gate Domain behind one stabilized threshold.

This is not a city-district crisis and not a cluster of unrelated Gates. The material world is the staging ground. The campaign space is the Domain.

The Gloamreach contains roads, settlements, predator woods, tithe sites, drowned archives, ruined bastions, black-market shelters, forbidden vaults, and a citadel visible from almost everywhere. The Regent is the Anchor. His rule is expressed through weather, law, hospitality, ledgers, servants, Relics, and invitation.

## Opening Situation

No Ascendant or Bureau team has gone inside yet, so the Bureau can only rate the Gate from its threshold instruments — a provisional low/E classification that reads like a routine high-density clear. The reading is honest and wrong. Once the party is deep enough inside, the truth becomes undeniable: the Gate opens into a coherent interior realm large enough to swallow teams, distort distance, and produce settlements under foreign law. As containment slips toward Red Phase, the threshold turns unreliable behind them.

The party enters as the first available Ascendant team. Their mission is to identify the Anchor, recover the Three Claims, prevent Red Phase collapse, and survive long enough to decide what kind of ending the Gloamreach deserves.

## Running This Module

This campaign is built at Curse of Strahd scale:

1. The Domain is the adventure location.
2. The Regent recurs throughout the campaign.
3. Travel is dangerous and meaningful.
4. Settlements are unsafe compromises, not rest stops.
5. Key Relics are placed through oracle logic.
6. Faction choices matter.
7. The finale is a citadel and Anchor-throne confrontation.

## Session 0 Checklist

- Establish mature horror expectations.
- Confirm hard limits and safety tools.
- Tie every Ascendant to a reason to enter the Gloamreach.
- Explain that some threats are meant to be survived, bargained with, escaped, or legally contradicted rather than killed.
- Establish that the Regent is not merely a monster. He is Domain authority.

## Ascendant Creation Hooks

| Hook | Description |
|---|---|
| Bureau Deployed | You were ordered into the Domain under emergency clearance. |
| Guild Raider | Your Guild sees salvage, cores, Relics, and leverage. |
| Missing Team | Someone you know vanished during the first response wave. |
| Name in the Ledger | Your name appears in a recovered Regent writ. |
| Researcher | You need to prove how Regent Domains work before the Bureau's caution seals what is really inside this one. |
| Grief-Bound | The Gloamreach contains a voice, face, or memory you thought was gone. |

## Milestone Leveling Guide

| Milestone | Level |
|---|---|
| Complete Day Zero or first threshold clear | 2 |
| Survive the first Domain road crisis | 3 |
| Earn a faction's trust or save a settlement | 4 |
| Clear the first major Domain site | 5 |
| Survive or legally repel the Regent's Bailiff | 7 |
| Resolve the first settlement's tribute crisis | 8 |
| Obtain a Claim or Regent Relic | 10 |
| Reach the Forbidden Vault or fallen bastion | 12 |
| Enter the Regent's Citadel | 13 |
| Resolve the Anchor (campaign climax) | 15 |
`,
	},
	{
		title: "Chapter 2: The Three Claims",
		content: `# The Three Claims

## Overview

The Three Claims are Relic-level contradictions in the Regent's rule. They are not simple loot drops. Each one proves that the Gloamreach does not fully belong to him.

Without the Claims, the Regent's Anchor structure is nearly impossible to resolve. With them, the party can weaken his laws, force contradictions in his court, and survive the final confrontation.

## The Claims

### The Void Claim

A sphere of absolute darkness that devours light, sound, and false testimony. It weakens the Regent's ability to hide truth or erase routes.

### The Abyss Claim

A crown, chain, or black iron writ that whispers promises of authority. It weakens the Regent's command over roads, invitations, and compelled obedience.

### The Blood Claim

A blade, vessel, or living seal that remembers who paid for the Domain before the Regent claimed it. It weakens the Regent's ability to treat people as property.

## Placement

Use Chapter 24's Oracle Reading to place the Claims. Each Claim should be in a different major Domain region unless the Warden intentionally wants a compressed or brutal campaign.

Default possible locations:

1. The Gate Threshold.
2. The Road of Writs.
3. The Tithe Mill.
4. The Predator Woods.
5. The Drowned Ledgerfen.
6. The Remembering Orchard.
7. Bastion Golemfall.
8. The Vermillion Outpost.
9. The Forbidden Vault of the Unseated Law.
10. The Regent's Citadel.

## Final Confrontation Effects

| Claims Held | Effect |
|---|---|
| 0 | The Regent's Anchor law is complete. He regenerates, ignores most contradiction, and controls the battlefield freely. |
| 1 | One Domain law can be suspended for one round during the finale. |
| 2 | Two major Regent abilities are weakened or delayed. |
| 3 | The party can force the Anchor-throne to answer: break, seal, transform, or inherit. |

## Warden Guidance

Do not present the Claims as mandatory keycards. Present them as dangerous evidence. Each one should help the party, but also teach the Regent more about them.
`,
	},
	{
		title: "Chapter 3: The Gloamreach Region Guide",
		content: `# The Gloamreach Region Guide

## Overview

The Gloamreach is a country-sized interior realm behind one S-Rank Gate. The Domain is large enough for travel to matter, settlements to differ, and the Regent's citadel to dominate the horizon without being immediately reachable.

## Core Regions

### The Gate Threshold

Where the party enters. Bureau instruments briefly function here, then begin reporting impossible distances, duplicate life signs, and old team telemetry.

### The Road of Writs

The main road through the Domain. It changes distance based on invitation, debt, and names entered into ledgers. The road is not neutral terrain. It is a legal instrument.

### Tribute Settlements

Each settlement survives through a lie. One claims tribute is voluntary. One claims the missing are honored. One survives by forgetting names. None are safe for free.

### The Predator Woods

An adaptive hunting ground. It remembers tactics, blood, campfire smoke, and fear.

### The Tithe Mill

A mill, workhouse, or counting-yard where tribute is processed and sent toward the citadel.

### Bastion Golemfall

A fallen defensive fortress. This is the campaign's ruin of failed protection, last stands, and old oaths.

### The Vermillion Outpost

A black-market shelter inside the Domain. Vermillion saves people and exploits them. Both facts are true.

### The Forbidden Vault

The seat of the Unseated Law, the sealed residue of the Domain's prior Anchor. This is the campaign's forbidden bargain location.

### The Regent's Citadel

The final castle-scale dungeon and visible symbol of the Regent's ownership.

## Travel Rules

Roll once for every major journey or whenever the party refuses an invitation, travels without tribute, or carries a Claim openly.

| d6 | Road Event |
|---|---|
| 1 | The road lengthens. Lose time and supplies. |
| 2 | A toll-house appears where none stood before. |
| 3 | A messenger delivers an invitation from the Regent. |
| 4 | A settlement bell rings in the distance. Someone is being collected. |
| 5 | The Regent's Bailiff crosses the road ahead. |
| 6 | The party finds a safe-looking shelter. It has a ledger by the door. |

## Mana Exposure

Long rests outside protected settlements require a DC 10 Vitality save. On a failure, the character dreams of the citadel and wakes with disadvantage on the first Sense check of the day.
`,
	},
	{
		title: "Chapter 4: Factions and Reputation",
		content: `# Factions and Reputation

## Overview

The Gloamreach is shaped by factions trying to survive, profit, rule, worship, or escape. Reputation affects supplies, shelter, intelligence, betrayal, and final-act support.

## Reputation Tiers

| Tier | Name | Effect |
|---|---|---|
| -2 | Hostile | Faction attacks, sabotages, or reports the party. |
| -1 | Distrusted | No services, higher prices, guarded speech. |
| 0 | Neutral | Basic interaction. |
| +1 | Trusted | Discounted services, side quests, better information. |
| +2 | Allied | Safe shelter, faction support, possible final-act aid. |

## The Ascendant Bureau

**Leader**: Commander Park Jae-won  
**Base**: Bureau Domain Response Annex outside the Gate  
**Goal**: Maintain containment, recover teams, prevent Red Phase collapse, preserve official control.

The Bureau is useful, disciplined, underinformed, and politically afraid. It can save lives, but it will also classify inconvenient truths.

## The Vermillion Guild

**Leader**: Guildmaster Orin  
**Base**: Vermillion Outpost inside the Gloamreach  
**Goal**: Extract salvage, rescue useful survivors, profit, and prove the Bureau is too slow to lead.

Vermillion moves fast and does not wait for permission. Their help is real. Their invoices are also real.

## The Awoko Cult

**Leader**: The Hollow Mother  
**Base**: Hidden sanctum, moving through grief-dense sites  
**Goal**: Use the Regent's law to force an ascension beyond Rank, then inherit the Domain.

The Awoko preach remembrance, but their rituals turn grief into fuel.

## The Regent's Court

**Leader**: The Regent  
**Base**: The Citadel  
**Goal**: Preserve ownership, collect tribute, test intruders, and make the Domain's claim permanent.

The Court is not a normal faction, but reputation still matters. Courtesy, defiance, betrayal, guest-right, and broken promises are all tracked.
`,
	},
	{
		title: "Chapter 5: Bureau Domain Response Annex",
		content: `# Bureau Domain Response Annex

## Overview

The Bureau annex is the material-side command post outside the S-Rank Gate. It is the party's last reliable taste of the modern world: floodlights, barricades, medics, drones, tablet maps, exhausted analysts, and a Gate that refuses to behave like a door.

## Key Areas

### Operations Center

Commander Park briefs teams here. The main display shows the Gate Threshold, then static, then maps that update with impossible roads.

### Quartermaster Station

Basic gear, mana rations, glow rods, smoke grenades, medkits, and Bureau tactical armor. Higher-tier stock requires trust.

### Medical Bay

Dr. Serin Hayashi treats survivors and studies Domain-touched tissue. She is one of the first Bureau researchers willing to say the quiet part aloud: killing the Regent may not be the only possible Anchor resolution.

### Communications Room

Relay operators attempt contact with teams inside the Gloamreach. Messages arrive out of order. Some are from tomorrow. Some are from dead teams.

## Key NPCs

### Commander Park Jae-won

A disciplined Bureau officer trying to keep panic from becoming policy. He wants containment, but he is not blind to the Bureau's failures.

### Quartermaster Lin Mei-hua

Controls equipment access. Practical, dry, and more generous when no one is watching.

### Dr. Serin Hayashi

Researcher, physician, and dangerous thinker. Believes Regent Domains are legal/metaphysical structures, not just monster lairs.

### Agent Kira Blackwood

Intelligence operative with classified orders involving pre-threshold Relic activity. Her secrecy can become either betrayal or confession.
`,
	},
	{
		title: "Chapter 6: The Vermillion Outpost",
		content: `# The Vermillion Outpost

## Overview

The Vermillion Outpost is a black-market shelter inside the Gloamreach. It exists because Vermillion entered early, moved fast, and paid costs the Bureau would still be debating.

The outpost has food, heat, illegal Runes, wounded civilians, salvage stalls, smuggled batteries, and more guns than official policy allows.

## Areas

### Core Exchange

Buy and sell Anomaly cores at better rates than Bureau standard. Vermillion asks fewer questions and remembers more favors.

### Rune Stall

Contraband Runes and Sigils. Expensive, useful, and sometimes cursed by local Domain law.

### Field Kitchen

A hot meal grants 1d4 temporary HP for one hour, but food is scarce. If the party eats free, someone else does not.

### Bounty Board

Bounties include missing teams, dangerous roads, unpaid toll-houses, Domain beasts, and rumors of Claim locations.

### Orin's Office

Guildmaster Orin keeps maps on the walls. Every map disagrees. He trusts none of them and all of them.
`,
	},
	{
		title: "Chapter 7: Downtime in a Hostile Domain",
		content: `# Downtime in a Hostile Domain

Downtime inside the Gloamreach is never fully safe. It is time bought from hunger, tribute, weather, and the Regent's attention.

## Downtime Actions

### Rest Under Guest-Right

Requires a settlement, outpost, or protected camp. Safe unless the party has violated local law.

### Gather Rumors

Learn one clue about a Claim, settlement crisis, road law, or the Regent's next invitation.

### Repair and Refit

Restore damaged gear, reload consumables, and prepare for travel.

### Study Domain Law

Make an Intelligence or Sense check. On success, learn one way to contradict a local law.

### Treat Trauma

A character may clear one minor fear, hallucination, or memory distortion if another character spends the downtime helping them.

## Downtime Complications

| d6 | Complication |
|---|---|
| 1 | A messenger arrives with an invitation. |
| 2 | A settlement demands tribute for shelter. |
| 3 | Food spoils overnight. |
| 4 | Someone dreams of the Citadel and wakes speaking the Regent's words. |
| 5 | Vermillion calls in a favor. |
| 6 | The road outside has moved. |
`,
	},
	{
		title: "Chapter 8: First Domain Site - The Hollow Subway",
		content: `# First Domain Site - The Hollow Subway

## Overview

The Hollow Subway is a low-rank Domain site near the Gate Threshold. It looks like a modern transit station folded into old stone. Trains arrive with no tracks. Announcements name passengers who have not entered yet.

## Purpose

This site teaches the party three truths:

1. Modern infrastructure cannot be trusted inside a Domain.
2. The Regent's law can use familiar spaces as masks.
3. Leaving is harder than entering.

## Key Scenes

### The Platform

A train arrives. Its doors open onto candlelit courtrooms, hospital halls, flooded tunnels, or the same platform ten years older.

### The Ticket Gate

The turnstiles require a fare. Money works poorly. Names work too well.

### The Engine Car

A minor Relic, Claim clue, or dead team AFA can be found here. If the Oracle placed the Void Claim in this site, it is hidden in the firebox of a train that has never burned fuel.

## Exit Condition

The party exits when they understand what the station is asking for and refuse to pay with identity. Clever alternatives include false tickets, shared fare, destroying the ledger mechanism, or invoking Bureau jurisdiction with a successful Presence or Intelligence check.
`,
	},
];
