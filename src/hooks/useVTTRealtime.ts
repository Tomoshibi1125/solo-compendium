...
export interface VTTInitiativeState {
  order: { tokenId: string; name: string; initiative: number; hp?: number; maxHp?: number }[];
...
export type VTTBroadcastEvent =
  ...
  | { type: 'token_add'; payload: { token: Record<string, unknown>; addedBy: string } }
  ...
  | { type: 'scene_sync'; payload: { scenes: unknown[]; currentSceneId: string | null; syncedBy: string } }
  | { type: 'drawing_update'; payload: { drawings: unknown[]; sceneId: string; updatedBy: string } }
...
function parseDiceTerm(term: string): DiceTermResult {
  // Exploding: NdS!
  const explodingMatch = term.match(/^(\d+)d(\d+)(!)?(?:kh(\d+))?(?:kl(\d+))?(?:ro<(\d+))?$/i);
  if (!explodingMatch) {
  ...
  const keepHighest = explodingMatch[4] ? parseInt(explodingMatch[4]) : undefined;
  const keepLowest = explodingMatch[5] ? parseInt(explodingMatch[5]) : undefined;
  const rerollOnceBelow = explodingMatch[6] ? parseInt(explodingMatch[6]) : undefined;
...
export function rollDiceFormulaDetailed(rawFormula: string): VTTDiceRollDetailed {
  ...
  const tokenRegex = /([+-]?)(\d+d\d+(?:!)?(?:kh\d+)?(?:kl\d+)?(?:ro<\d+)?|\d+)/gi;
  ...
  for (const token of tokens) {
    if (token.isDice) {
      ...
      const prefix = token.sign < 0 ? '- ' : displayParts.length > 0 ? '+ ' : '';
...
export function parseChatCommand(input: string): ChatCommand {
  ...
  if (whisperMatch) return { type: 'whisper', target: whisperMatch[1].trim(), message: whisperMatch[2].trim() };
...
export function useVTTRealtime({ campaignId, sessionId, isDM = false }: UseVTTRealtimeOptions) {
  const { user } = useAuth();
  ...
  const eventHandlersRef = useRef<Map<string, Set<(payload: any) => void>>>(new Map());
  ...
  const on = useCallback(<T extends VTTBroadcastEvent['type']>(
    eventType: T,
    handler: (payload: Extract<VTTBroadcastEvent, { type: T }>['payload']) => void,
  ) => {
  ...
  const broadcastTokenMove = useCallback(
    (tokenId: string, x: number, y: number) => {
      broadcast({ type: 'token_move', payload: { tokenId, x, y, movedBy: userId } });
    },
    [broadcast, userId],
  );
  ...
  const broadcastTokenUpdate = useCallback(
    (tokenId: string, updates: Record<string, unknown>) => {
      broadcast({ type: 'token_update', payload: { tokenId, updates, updatedBy: userId } });
    },
    [broadcast, userId],
  );
  ...
  const broadcastTokenRemove = useCallback(
    (tokenId: string) => {
      broadcast({ type: 'token_remove', payload: { tokenId, removedBy: userId } });
    },
    [broadcast, userId],
  );
  ...
  const broadcastSceneChange = useCallback(
    (sceneId: string) => {
      broadcast({ type: 'scene_change', payload: { sceneId, changedBy: userId } });
    },
    [broadcast, userId],
  );
  ...
  const broadcastSceneSync = useCallback(
    (scenes: unknown[], currentSceneId: string | null) => {
      broadcast({ type: 'scene_sync', payload: { scenes, currentSceneId, syncedBy: userId } });
    },
    [broadcast, userId],
  );
  ...
  const broadcastFogUpdate = useCallback(
    (sceneId: string, fogData: boolean[][]) => {
      broadcast({ type: 'fog_update', payload: { sceneId, fogData, updatedBy: userId } });
    },
    [broadcast, userId],
  );
  ...
  const broadcastDrawingUpdate = useCallback(
    (drawings: unknown[], sceneId: string) => {
      broadcast({ type: 'drawing_update', payload: { drawings, sceneId, updatedBy: userId } });
    },
    [broadcast, userId],
  );
  ...
  const sendChatMessage = useCallback(
    (message: string, type: VTTChatMessage['type'] = 'chat', whisperTo?: string) => {
  ...
  const rollAndBroadcast = useCallback(
    (formula: string, msgType: 'dice' | 'gmroll' = 'dice'): VTTDiceRollDetailed => {
  ...
  const endCombat = useCallback(() => {
    const next: VTTInitiativeState = { order: [], currentTurnIndex: 0, round: 1, active: false };
  ...
  const broadcastRuler = useCallback(
    (fromX: number, fromY: number, toX: number, toY: number, distance: number) => {
      const segment: VTTRulerSegment = { fromX, fromY, toX, toY, distance, userId, color: userColor };
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('broadcast', { event: 'vtt' }, ({ payload }: { payload: VTTBroadcastEvent }) => {
        ...
        switch (payload.type) {
          case 'chat_message':
          case 'dice_roll':
            setChatMessages((prev) => [...prev, payload.payload as VTTChatMessage].slice(-200));
            break;
          ...
          case 'ruler_clear': {
            const { userId: clearUid } = payload.payload as { userId: string };
      ...
      .on('presence', { event: 'sync' }, () => {
        ...
        for (const [_key, presences] of Object.entries(state)) {
          for (const presence of presences as (PresencePayload & { presence_ref?: string })[]) {
      ...
      void supabase
        ...
        .then(({ data }: { data: any[] | null }) => {
          if (data && data.length > 0) {
            const msgs: VTTChatMessage[] = data
              .reverse()
              .map((row: any) => ({
                id: row.id,
  ...
  const activeUsers = useMemo(() => Array.from(presenceUsers.values()), [presenceUsers]);
