	...
	const { data: backups = [], refetch } = useQuery({
		...
		queryFn: async () => {
			...
			const { data, error } = await supabase
				...
				.select(
					"id, character_id, backup_data, backup_name, created_at, version",
				)
		...
		mutationFn: async ({
			...
			const { data, error } = await supabase
				...
				.insert({
					user_id: user.id,
					character_id: character.id,
					backup_data: JSON.parse(
						JSON.stringify(character),
					) as Database["public"]["Tables"]["character_backups"]["Insert"]["backup_data"],
					backup_name: backupName,
					version: 1,
				})
				.select("id")
		...
		mutationFn: async (backup: CharacterBackup) => {
			...
			const { data, error } = await supabase
				...
				.insert({
					...
					backup_data:
						backup.backup_data as Database["public"]["Tables"]["character_backups"]["Insert"]["backup_data"],
