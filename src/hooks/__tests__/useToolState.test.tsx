...
const resolveMaybeSingle = (table: string): { data: { state: ProbeState } | null; error: null } => {
  if (table === 'user_tool_states') {
    return { data: remoteUserState ? { state: remoteUserState } : null, error: null };
  }
  if (table === 'campaign_tool_states') {
    return { data: remoteCampaignState ? { state: remoteCampaignState } : null, error: null };
  }
...
const CampaignProbe = ({
  ...
  const result = useCampaignToolState<ProbeState>(campaignId, 'tool_state_campaign_probe', {
    initialState: { value: 0 },
    storageKey,
  });
...
describe('useToolState local fallback behavior', () => {
  beforeEach(() => {
    ...
    mockAuth('user-1');
    fromMock.mockImplementation((table: string) => createFromChain(table) as never);
  });
  ...
  it('mirrors remote user state into local storage after hydration', async () => {
    ...
    const unmount = mount(<UserProbe storageKey={storageKey} onSnapshot={(snapshot) => { latest = snapshot; }} />);
    ...
    try {
      ...
      expect(JSON.parse(storage.getItem(storageKey) || 'null')).toEqual({ value: 42 });
    } finally {
      unmount();
    }
  });
  ...
  it('writes local user mirror before remote upsert when authenticated', async () => {
    ...
    try {
      ...
      expect(JSON.parse(storage.getItem(storageKey) || 'null')).toEqual({ value: 7 });

      const userUpserts = upsertCalls.filter((call) => call.table === 'user_tool_states');
      expect(userUpserts).toHaveLength(1);
      expect((userUpserts[0].payload as { state: ProbeState }).state).toEqual({ value: 7 });
    } finally {
      unmount();
    }
  });
  ...
  it('loads and saves user state locally when unauthenticated', async () => {
    ...
    try {
      ...
      expect(JSON.parse(storage.getItem(storageKey) || 'null')).toEqual({ value: 6 });
      expect(upsertCalls.filter((call) => call.table === 'user_tool_states')).toHaveLength(0);
    } finally {
      unmount();
    }
  });
  ...
  it('mirrors remote campaign state into local storage after hydration', async () => {
    ...
    try {
      ...
      expect(JSON.parse(storage.getItem(storageKey) || 'null')).toEqual({ value: 12 });
    } finally {
      unmount();
    }
  });
  ...
  it('loads and saves campaign state locally when unauthenticated', async () => {
    ...
    try {
      ...
      expect(JSON.parse(storage.getItem(storageKey) || 'null')).toEqual({ value: 9 });
      expect(upsertCalls.filter((call) => call.table === 'campaign_tool_states')).toHaveLength(0);
    } finally {
      unmount();
    }
  });
  ...
  it('writes local campaign mirror before remote upsert when authenticated', async () => {
    ...
    try {
      ...
      expect(JSON.parse(storage.getItem(storageKey) || 'null')).toEqual({ value: 15 });
