...
import { useCharacterJournal, type JournalEntry } from '@/hooks/useCharacterJournal';
...
const buildStorageKey = (characterId: string) => `solo-compendium.character-journal.${characterId}.v1`;
  ...
  it('mirrors remote entries into localStorage after hydration', async () => {
    ...
    const unmount = mount(
      <QueryClientProvider client={queryClient}>
        <JournalProbe characterId={characterId} onSnapshot={(snapshot) => {
          latest = snapshot;
        }} />
      </QueryClientProvider>
    );
