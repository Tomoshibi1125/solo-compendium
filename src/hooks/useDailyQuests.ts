...
type QuestInstanceRow =
	Database["public"]["Tables"]["daily_quest_instances"]["Row"];
type QuestConfigRow =
	Database["public"]["Tables"]["daily_quest_configs"]["Row"];
...
const getLocalKey = (characterId: string) =>
	`${LOCAL_KEY_PREFIX}:${characterId}`;
	...
	try {
		...
		return {
			config: parsed.config
				? { ...buildDefaultConfig(characterId), ...parsed.config }
				: buildDefaultConfig(characterId),
...
const normalizeReward = (reward: Record<string, unknown>): QuestReward => ({
	...
	experience:
		typeof reward.experience === "number" ? reward.experience : undefined,
	description:
		typeof reward.description === "string"
			? reward.description
			: "Quest reward",
	custom_rewards:
		typeof reward.custom_rewards === "object" && reward.custom_rewards !== null
...
const buildLocalInstances = (
	...
	const maxTierIndex = Math.min(
		4,
		Math.max(1, baseTierIndex + difficultyAdjust),
	);
	const tierLabels = ["I", "II", "III", "IV"];
	const allowedTiers = tierLabels.slice(0, maxTierIndex);

	const availableTemplates = templates.filter(
		(template) => template.is_active && allowedTiers.includes(template.tier),
	);
	const sortedTemplates = [...availableTemplates].sort(
		(a, b) => hashString(a.id + questSeed) - hashString(b.id + questSeed),
	);
	const selectedTemplates = sortedTemplates.slice(
		0,
		Math.max(1, config.max_active_quests || 3),
	);

	return selectedTemplates.map((template, index) => {
		const customScaling = QuestScalingSchema.safeParse(config.custom_scaling);
		const scaling = customScaling.success
			? customScaling.data
			: template.default_scaling;
...
export function useDailyQuests(
	...
	const templatesQuery = useQuery({
		...
		queryFn: async () => {
			...
			try {
				...
				logger.warn(
					"Failed to load daily quest templates, using defaults:",
					error,
				);
				return DEFAULT_DAILY_QUEST_TEMPLATES;
			}
			...
			if (existing?.id) {
				const { data, error } = await supabase
					...
					.update({
						...
						updated_at: new Date().toISOString(),
						custom_scaling:
							updates.custom_scaling as QuestConfigRow["custom_scaling"],
					})
					.eq("id", existing.id)
					.select()
					.single();
	...
	const assignQuests = useMutation({
		mutationFn: async ({ forceNew = false }: { forceNew?: boolean } = {}) => {
			...
			if (isLocal) {
				...
				const newInstances = buildLocalInstances(
					characterId,
					level,
					state.config,
					templatesQuery.data || [],
	...
	const updateQuest = useMutation({
		mutationFn: async ({
			...
			questId: string;
			updates: Partial<QuestInstanceRow>;
		}) => {
			if (!characterId) return null;
			if (isLocal) {
				const state = loadLocalState(characterId);
				...
				saveLocalState(characterId, {
					...state,
					instances: updatedInstances as DailyQuestInstance[],
				});
				return (
					updatedInstances.find((instance) => instance.id === questId) || null
				);
			}
	...
	const normalizeQuestReward = useCallback(
		(quest: DailyQuestInstance, template?: DailyQuestTemplate) => {
			const scalingReward = (quest.scaling_applied as Record<string, unknown>)
				?.reward;
			...
			if (quest.rewards_granted && typeof quest.rewards_granted === "object") {
				return normalizeReward(
					quest.rewards_granted as Record<string, unknown>,
				);
			}
