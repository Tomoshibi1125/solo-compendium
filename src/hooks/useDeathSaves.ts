...
export function useDeathSaves(
  ...
  const takeDamageAtZero = useCallback(
    (damage: number, hpMax: number): DeathSaveState => {
      ...
      if (damage >= hpMax) {
        const dead: DeathSaveState = { successes: 0, failures: 3, isStable: false, isDead: true };
        setState(dead);
        return dead;
      }
      ...
      const newFailures = Math.min(3, state.failures + 1);
      const isDead = newFailures >= 3;
      const next: DeathSaveState = { ...state, failures: newFailures, isDead, isStable: false };
      setState(next);
      return next;
    },
    [state]
  ...
  return { state, rollDeathSave, takeDamageAtZero, receiveHealing, stabilize, reset, persist };
}
