...
const sanitizeBundle = (bundle: Json | null) => {
  ...
  if (sanitized.campaign && typeof sanitized.campaign === 'object' && !Array.isArray(sanitized.campaign)) {
  ...
  if (Array.isArray(sanitized.invites)) {
    sanitized.invites = sanitized.invites.map((invite) => {
      if (!invite || typeof invite !== 'object' || Array.isArray(invite)) return invite;
...
export const useCampaignExport = () => {
  ...
  return useMutation({
    mutationFn: async ({ campaignId, filename }: { campaignId: string; filename?: string }) => {
      ...
      const { data, error } = await supabase.rpc('export_campaign_bundle', { p_campaign_id: campaignId });
      if (error) throw error;
      const sanitized = sanitizeBundle(data as Json);
      downloadJson(sanitized ?? {}, filename ?? `campaign-${campaignId}-export.json`);
      return sanitized;
    },
    onSuccess: () => {
      toast({
