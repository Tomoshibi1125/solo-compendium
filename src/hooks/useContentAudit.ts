...
function generateRecommendations(report: ContentAuditReport): string[] {
	...
	if (summary.averageCompleteness < 50) {
		recommendations.push(
			"⚠️ Overall content completeness is below 50%. Consider adding missing descriptions and images.",
		);
	} else if (summary.averageCompleteness < 75) {
		recommendations.push(
			"ℹ️ Content completeness is good but could be improved. Focus on adding missing images.",
		);
	}
	...
	if (imageCoverage < 30) {
		recommendations.push(
			"🖼️ Less than 30% of entries have images. Consider generating or adding images for better visual appeal.",
		);
	}

	// Description coverage
	const descriptionCoverage =
		(summary.totalWithDescriptions / summary.totalEntries) * 100;
	if (descriptionCoverage < 80) {
		recommendations.push(
			"📝 Some entries are missing descriptions. Add descriptions to improve content quality.",
		);
	}
	...
	tables.forEach((table) => {
		if (table.totalCount === 0) {
			recommendations.push(
				`📋 ${table.tableName} table is empty. Consider adding content.`,
			);
		} else if (table.completeness < 50) {
			recommendations.push(
				`⚠️ ${table.tableName} has low completeness (${table.completeness}%). Review and improve entries.`,
			);
		}
	});

	// Source book coverage
	const tablesMissingSource = tables.filter(
		(t) => t.missingSourceBook > t.totalCount * 0.1,
	);
	if (tablesMissingSource.length > 0) {
		recommendations.push(
			"📚 Some entries are missing source book information. Add source_book for better provenance tracking.",
		);
	}
...
export function useContentAudit() {
	return useQuery({
		...
		queryFn: async (): Promise<ContentAuditReport> => {
			...
			// Calculate summary
			const totalEntries = validAudits.reduce(
				(sum, audit) => sum + audit.totalCount,
				0,
			);
			const totalWithImages = validAudits.reduce(
				(sum, audit) => sum + audit.withImage,
				0,
			);
			const totalWithDescriptions = validAudits.reduce(
				(sum, audit) => sum + audit.withDescription,
				0,
			);
			const averageCompleteness =
				validAudits.length > 0
					? Math.round(
							validAudits.reduce((sum, audit) => sum + audit.completeness, 0) /
								validAudits.length,
						)
					: 0;
