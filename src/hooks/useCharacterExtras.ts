...
export type CharacterExtraWithMonster = CharacterExtra & {
    monster?: (Database['public']['Tables']['compendium_monsters']['Row'] & {
...
export function useCharacterExtras(characterId: string) {
    ...
    const addExtra = useMutation({
        mutationFn: async (newExtra: Omit<CharacterExtra, 'id' | 'created_at' | 'updated_at'>) => {
            const { data, error } = await supabase
                .from('character_extras')
                .insert(newExtra)
                .select()
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['character_extras', characterId] });
    ...
    const updateExtra = useMutation({
        mutationFn: async ({ id, data }: { id: string; data: Partial<CharacterExtra> }) => {
