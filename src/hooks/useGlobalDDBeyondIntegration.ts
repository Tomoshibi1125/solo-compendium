...
interface CharacterSheetEnhancements {
  ...
  clearCache: () => void;
  getCacheSize: () => number;
  roll: (rollKey: string, modifier: number, kind: 'ability' | 'save' | 'skill' | 'attack' | 'damage', label?: string, campaignId?: string, advantage?: 'advantage' | 'disadvantage' | 'normal') => Promise<unknown>;
}

interface DMToolsEnhancements {
  endCombatSession: (sessionId: string, options?: Record<string, unknown>) => Promise<unknown>;
  generateSessionReport: (sessionId: string) => Promise<string | null>;
  distributeRewards: (sessionId: string, rewards: unknown, options?: unknown) => Promise<boolean>;
  ...
  uploadVTTAsset: (campaignId: string, file: File, type: string) => Promise<unknown>;
  isDM: boolean;
...
export function useGlobalDDBeyondIntegration() {
  ...
  const useDMToolsEnhancements = (campaignId?: string): DMToolsEnhancements => {
    ...
    return {
      endCombatSession: async (sessionId: string, options: Record<string, unknown> = {}) => {
      ...
      distributeRewards: async (_sessionId: string, rewards: unknown, options: unknown = []) => {
        return await distributeRewards(campaignId || '', rewards as never, (options ?? []) as string[]);
      },
