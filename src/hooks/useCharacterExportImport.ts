...
export function useCharacterExport() {
  ...
  const exportCharacterJson = useCallback(async (characterId: string) => {
    try {
      ...
      // Fetch related data
      const [abilitiesResult, equipmentResult, featuresResult] = await Promise.all([
        supabase.from('character_abilities').select('*').eq('character_id', characterId),
  ...
  const exportCharacterPdf = useCallback(async (characterId: string) => {
    try {
...
Generated: ${new Date().toLocaleDateString()}
  ...
  const importCharacterJson = useCallback(async (file: File) => {
    try {
      if (!isSupabaseConfigured) {
        throw new Error('Backend not configured');
      }
