...
const getNotesKey = (campaignId: string) =>
	...
	return useQuery({
		...
		queryFn: async (): Promise<CampaignNote[]> => {
			if (!isSupabaseConfigured || import.meta.env.VITE_E2E === "true") {
				return loadLocalNotes(campaignId).sort((a, b) =>
			...
			const notes = (data ||
				[]) as Database["public"]["Tables"]["campaign_notes"]["Row"][];
	...
	return useMutation({
		...
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: ["campaigns", variables.campaignId, "notes"],
			});
			toast({
				title: "Note created",
...
export const useUpdateCampaignNote = () => {
	...
	return useMutation({
		mutationFn: async ({
			...
			if (error) throw error;
			const note =
				data as Database["public"]["Tables"]["campaign_notes"]["Row"];
			return {
				...note,
				category: note.category ?? "general",
			};
