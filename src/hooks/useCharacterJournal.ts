...
const buildJournalStorageKey = (characterId: string) =>
...
const writeLocalJournalEntries = (
	characterId: string,
	...
	return useMutation({
		...
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: ["character-journal", variables.character_id],
			});
		},
	});
};
...
const useUpdateJournalEntry = () => {
	...
	return useMutation({
		mutationFn: async ({
			...
			...updates
		}: { id: string; characterId: string } & Partial<JournalEntry>) => {
		...
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: ["character-journal", variables.characterId],
			});
		},
	});
};
