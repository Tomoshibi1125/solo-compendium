...
import { deriveCampaignInviteStatus, normalizeInviteAccessKey } from '@/lib/campaignInviteUtils';
...
const normalizeInviteRecord = (invite: CampaignInviteRecord): CampaignInviteRecord => {
...
const createInviteViaApi = async ({
  ...
  if (!response.ok) {
    ...
    throw new AppError(payload.error || 'Failed to create invite via API', 'UNKNOWN');
  }
...
export const useCampaignInvites = (campaignId: string) => {
  return useQuery({
    ...
    queryFn: async (): Promise<CampaignInviteRecord[]> => {
      ...
      const invites = ((data || []) as unknown as CampaignInviteRecord[]).map(normalizeInviteRecord);
      return invites;
    },
    enabled: !!campaignId,
  });
...
export const useCreateCampaignInvite = () => {
  ...
  return useMutation({
    mutationFn: async ({ campaignId, role = 'hunter', expiresAt, maxUses = 1, inviteEmail }: CreateInviteArgs) => {
      if (!isSupabaseConfigured) {
        throw new AppError('Supabase not configured', 'CONFIG');
      }

      const normalizedInviteEmail = inviteEmail?.trim().toLowerCase() || undefined;
    ...
    onSuccess: (invite, variables) => {
      queryClient.invalidateQueries({ queryKey: ['campaigns', variables.campaignId, 'invites'] });
      queryClient.invalidateQueries({ queryKey: ['campaigns', variables.campaignId, 'invite-audit'] });
      toast({
...
export const useAddPlayerCharacterToCampaign = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data, error } = await supabaseAny.rpc('add_player_character_to_campaign', {
        p_campaign_id: campaignId,
...
export const useDeleteCampaignInvite = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (error) throw error;
      if (!data) {
        throw new AppError('Invite is already revoked or unavailable', 'NOT_FOUND');
      }
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['campaigns', variables.campaignId, 'invites'] });
...
export const useRedeemCampaignInvite = () => {
  ...
  return useMutation({
    mutationFn: async ({ token, characterId }: { token: string; characterId?: string }) => {
      if (!isSupabaseConfigured) {
        throw new AppError('Supabase not configured', 'CONFIG');
      }
      const accessKey = normalizeInviteAccessKey(token);
