...
export function useCombatSessionManager() {
  ...
  const endCombatSession = useCallback(async (
    sessionId: string,
    options: CombatSessionEndOptions = {}
  ) => {
  ...
  const resumeCombatSession = useCallback(async (sessionId: string) => {
    if (!user || !isSupabaseConfigured) {
      throw new Error('Must be logged in to manage combat sessions');
    }
  ...
  const pauseCombatSession = useCallback(async (sessionId: string) => {
    if (!user || !isSupabaseConfigured) {
      throw new Error('Must be logged in to manage combat sessions');
    }
