...
import { REGENT_LABEL, formatRegentVernacular } from '@/lib/vernacular';
import {
...
export function useRegentUnlocks(characterId: string) {
  ...
  const unlockRegentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['regent-unlocks', characterId] });
      toast({
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || 'An error occurred while unlocking the regent.',
        variant: 'destructive',
      });
    },
  });
  ...
  const removeUnlockMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || 'An error occurred while removing the unlock.',
        variant: 'destructive',
      });
    },
  });
  ...
  const updateUnlockMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || 'An error occurred while updating the unlock.',
        variant: 'destructive',
      });
    },
  });
  ...
  const {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        `)
        .filter('character_id', 'in', `(select id from characters where campaign_id = '${campaignId}')`)
        .order('unlocked_at', { ascending: false });
