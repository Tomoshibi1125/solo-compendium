...
type CampaignInventory = Database['public']['Tables']['campaign_inventory']['Row'];
type CampaignInventoryInsert = Database['public']['Tables']['campaign_inventory']['Insert'];
type CampaignInventoryUpdate = Database['public']['Tables']['campaign_inventory']['Update'];
...
export const useCampaignInventory = (campaignId: string | null, characterId?: string) => {
    ...
    const addItem = useMutation({
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['campaign_inventory', campaignId] });
            toast({ title: 'Item added to Party Stash' });
            if (characterId) {
                const qtyStr = data.quantity && data.quantity > 1 ? `(${data.quantity}x) ` : '';
                playerTools.trackInventoryChange(
                    characterId,
    ...
    const updateItem = useMutation({
        mutationFn: async ({ id, updates }: { id: string; updates: CampaignInventoryUpdate }) => {
            const { data, error } = await supabase
                .from('campaign_inventory')
                .update(updates)
                .eq('id', id)
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['campaign_inventory', campaignId] });
            if (characterId) {
                const qtyStr = data.quantity && data.quantity > 1 ? `(now ${data.quantity}x) ` : '';
