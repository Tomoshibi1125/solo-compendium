...
export function useBackgroundSync() {
  ...
  // Handle sync events
  const handleSyncEvent = useCallback(async (event: any) => {
    ...
    try {
      ...
      toast({
        title: 'Sync Failed',
        description: 'Background sync encountered an error. Some data may not be synchronized.',
        variant: 'destructive'
      });
    } finally {
    ...
    for (const item of queue) {
      try {
        ...
        if (failedItem.retryCount < 3) {
          ...
          console.error(`Item ${item.id} exceeded max retry count, dropping from queue`);
        }
      }
    }

    // Update queue in localStorage
    const updatedQueue = queue.filter(item => !processedItems.includes(item.id)).concat(failedItems);
  ...
  const addToSyncQueue = useCallback((type: string, action: 'create' | 'update' | 'delete', data: Record<string, any>) => {
    if (!user) return;
