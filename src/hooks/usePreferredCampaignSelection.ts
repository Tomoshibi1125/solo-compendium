...
export function usePreferredCampaignSelection(toolKey: string) {
  ...
  const {
    ...
    isLoading,
    saveNow,
  } = useUserToolState<PreferredCampaignSelection>(`preferred_campaign:${toolKey}`, {
...
export function useHydratedPreferredCampaignId(args: {
  ...
  const { toolKey, campaigns, urlCampaignId, isCampaignIdValid, onResolveCampaignId } = args;
  const { campaignId: preferredCampaignId, isLoading, setPreferredCampaignId } = usePreferredCampaignSelection(toolKey);
  ...
  }, [campaigns, isCampaignIdValid, isLoading, onResolveCampaignId, preferredCampaignId, setPreferredCampaignId, urlCampaignId]);
}
