...
type EncounterEntryRow = Database['public']['Tables']['campaign_encounter_entries']['Row'];
...
export const useSaveCampaignEncounter = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['campaigns', variables.campaignId, 'encounters'] });
      toast({
...
export const useDeleteCampaignEncounter = () => {
  ...
  return useMutation({
    mutationFn: async ({ campaignId, encounterId }: { campaignId: string; encounterId: string }) => {
      if (!isSupabaseConfigured) {
        throw new AppError('Supabase not configured', 'CONFIG');
      }
      const { data: { user } } = await supabase.auth.getUser();
...
export const useDeployCampaignEncounter = () => {
  ...
  return useMutation({
    ...
    onSuccess: (sessionId, variables) => {
      queryClient.invalidateQueries({ queryKey: ['campaigns', variables.campaignId, 'combat-session'] });
      toast({
