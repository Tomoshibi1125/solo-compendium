...
export const useCreateVTTAudioTrack = () => {
  ...
  return useMutation({
    mutationFn: async (track: Omit<VTTAudioTrack, 'id' | 'created_at' | 'updated_at'>): Promise<{ trackId: string }> => {
      if (!isSupabaseConfigured) {
        throw new AppError('Supabase not configured', 'CONFIG');
      }

      const { data, error } = await supabaseAny.rpc('create_vtt_audio_track', track);
      if (error) throw error;

      return { trackId: data as string };
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['vtt', 'audio', 'tracks', variables.session_id] });
      toast({
...
export const useUpdateVTTAudioTrack = () => {
  ...
  return useMutation({
    mutationFn: async (updates: { track_id: string; session_id: string } & Partial<VTTAudioTrack>): Promise<void> => {
      if (!isSupabaseConfigured) {
        throw new AppError('Supabase not configured', 'CONFIG');
      }

      const { error } = await supabaseAny.rpc('update_vtt_audio_track', updates);
      if (error) throw error;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['vtt', 'audio', 'tracks', variables.session_id] });
...
export const useDeleteVTTAudioTrack = () => {
  ...
  return useMutation({
    mutationFn: async ({ trackId, sessionId }: { trackId: string; sessionId: string }): Promise<void> => {
      if (!isSupabaseConfigured) {
        throw new AppError('Supabase not configured', 'CONFIG');
      }

      const { error } = await supabaseAny.rpc('delete_vtt_audio_track', { track_id: trackId });
      if (error) throw error;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['vtt', 'audio', 'tracks', variables.sessionId] });
      toast({
...
export const useUpdateVTTAudioSettings = () => {
  ...
  return useMutation({
    mutationFn: async (settings: Partial<VTTAudioSettings> & { session_id: string }): Promise<void> => {
      if (!isSupabaseConfigured) {
        throw new AppError('Supabase not configured', 'CONFIG');
      }

      const { error } = await supabaseAny.rpc('update_vtt_audio_settings', settings);
      if (error) throw error;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['vtt', 'audio', 'settings', variables.session_id] });
      toast({
...
export class VTTAudioManager {
  ...
  private updateAllVolumes() {
    ...
    for (const [trackId, audio] of this.audioElements) {
      ...
      audio.volume = (parseFloat(audio.dataset.volume || '1') * volumeMultiplier * this.settings.master_volume);
    }
  }
