...
import { DomainEventBus, buildCorePayload, type ItemAttuneEvent } from '@/lib/domainEvents';
...
export interface UseAttunementReturn {
  ...
  /** Persist to Supabase */
  persist: (characterId: string, characterName: string, jobName: string | null, level: number, campaignId: string | null) => Promise<void>;
}
...
export function useAttunement(
  ...
  const canAttuneItem = useCallback(
    (item: AttunableItem): { allowed: boolean; reason: string } => {
      if (!item.requiresAttunement) {
        return { allowed: false, reason: `${item.name} does not require attunement.` };
      }
      if (item.isAttuned) {
        return { allowed: false, reason: `${item.name} is already attuned.` };
      }
      if (attunedCount >= MAX_ATTUNEMENT_SLOTS) {
        return { allowed: false, reason: `Attunement limit reached (${MAX_ATTUNEMENT_SLOTS}/${MAX_ATTUNEMENT_SLOTS}).` };
      }
  ...
  const unattune = useCallback(
    (itemId: string): AttuneResult => {
      ...
      if (!item.isAttuned) {
        return { success: false, message: `${item.name} is not attuned.`, attunedCount };
      }
