...
type CampaignRuleEvent =
	Database["public"]["Tables"]["campaign_rule_events"]["Row"];
...
export const normalizeRules = (
	...
	return {
		...
		failure_injection_rate: Math.min(
			Math.max(Number(raw.failure_injection_rate ?? 0), 0),
			100,
		),
		failure_injection_note:
			(raw.failure_injection_note as string | null | undefined) ?? null,
	};
};
...
export const useCampaignRules = (campaignId: string) => {
	return useQuery({
		...
		queryFn: async (): Promise<
			CampaignRulesRow & { rules: CampaignRuleSettings }
		> => {
	...
	return useMutation({
		...
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: ["campaigns", variables.campaignId, "rule-events"],
			});
	...
	return useMutation({
		mutationFn: async ({
			...
			const payload = {
				...
				failure_injection_rate: Math.min(
					Math.max(Number(rules.failure_injection_rate || 0), 0),
					100,
				),
		...
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: ["campaigns", variables.campaignId, "rules"],
			});
			queryClient.invalidateQueries({
				queryKey: ["campaigns", variables.campaignId, "rule-events"],
			});
