    ...
    try {
      const response = await aiService.processRequest({
        ...
        context: {
          ...
          style: 'detailed TTRPG content with full lore, mechanics, rules, and flavor',
      ...
      if (!response.success) {
        toast({
          ...
          description: response.error || 'Could not reach AI service. Using base output.',
          variant: 'destructive',
        });
        return null;
      }
      ...
      const text = typeof response.data === 'string'
        ? response.data
        : response.data?.content || response.data?.text || JSON.stringify(response.data);
