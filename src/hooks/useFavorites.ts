...
export const useFavorites = () => {
	...
	const pushDebug = (payload: Record<string, unknown>) => {
		...
		const debugWindow = window as typeof window & {
			__favoritesDebug?: Array<Record<string, unknown>>;
		};
		const next = debugWindow.__favoritesDebug
			? [...debugWindow.__favoritesDebug]
			: [];
	...
	const { data: favorites = new Set<string>(), isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			if (isSupabaseConfigured && resolvedUserId) {
				...
				return new Set(
					(data || []).map((row) => `${row.entry_type}:${row.entry_id}`),
				);
			}
	...
	const isFavorite = useCallback(
		(entryType: string, entryId: string): boolean => {
