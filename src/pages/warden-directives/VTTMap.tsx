...
const VTTMap = () => {
	...
	const [drawMode, setDrawMode] = useState<
		"none" | "ruler" | "cone" | "sphere" | "cube" | "wall"
	>("none");
	const [weather, setWeather] = useState<
		"none" | "rain" | "snow" | "embers" | "gas"
	>("none");
	...
	const {
		...
		saveNow: saveMapNow,
	} = useUserToolState<VTTMapState>("vtt_map", {
	...
	useEffect(() => {
		...
		const sourceTokens =
			normalizedTokens.length > 0 ? normalizedTokens : DEFAULT_TOKENS;
	...
	useEffect(() => {
		...
		mapLoading,
		storedMapState.currentLayer,
		storedMapState.tokens,
		storedMapState.weather,
	]);
	...
	const handleTokenDrag = (e: React.MouseEvent<HTMLDivElement>) => {
		...
		setPlacedTokens((tokens) =>
			tokens.map((t) =>
				t.id === draggedToken.id ? { ...t, x: gridX, y: gridY } : t,
			),
		);
	};
	...
	const handleToggleLock = (id: string) => {
		setPlacedTokens((tokens) =>
			tokens.map((t) => (t.id === id ? { ...t, locked: !t.locked } : t)),
		);
	};
			...
			<div className="container mx-auto px-4 py-8 max-w-7xl">
				...
				<div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
					<div className="lg:col-span-1 space-y-6">
							...
							<div className="space-y-4">
								...
								<div className="space-y-2 pt-4 border-t border-border">
									...
									<div className="flex flex-wrap gap-1">
										...
										<Button
											variant={drawMode === "cone" ? "default" : "outline"}
											size="sm"
											onClick={() => setDrawMode("cone")}
											title="Cone"
										>
											<Triangle className="w-4 h-4" />
										</Button>
										<Button
											variant={drawMode === "sphere" ? "default" : "outline"}
											size="sm"
											onClick={() => setDrawMode("sphere")}
											title="Sphere/Radius"
										>
											<Circle className="w-4 h-4" />
										</Button>
										<Button
											variant={drawMode === "cube" ? "default" : "outline"}
											size="sm"
											onClick={() => setDrawMode("cube")}
											title="Cube"
										>
								...
								<div className="space-y-2 pt-4 border-t border-border">
									...
									<div className="flex flex-wrap gap-1">
										<Button
											variant={weather === "none" ? "default" : "outline"}
											size="sm"
										...
										<Button
											variant={weather === "snow" ? "default" : "outline"}
											size="sm"
										...
										<Button
											variant={weather === "embers" ? "default" : "outline"}
											size="sm"
											onClick={() => setWeather("embers")}
											title="Embers/Fire"
										>
											<Flame className="w-4 h-4" />
										</Button>
										<Button
											variant={weather === "gas" ? "default" : "outline"}
											size="sm"
											onClick={() => setWeather("gas")}
											title="Toxic Gas"
										>
								...
								{storedMapState.walls && storedMapState.walls.length > 0 && (
									<div className="flex justify-between items-center text-xs">
										<span className="text-muted-foreground">
											{storedMapState.walls.length} Walls Drawn
										</span>
										<Button
											variant="ghost"
											size="sm"
						...
						<AscendantWindow title="TOKEN LIBRARY">
												...
												<div className="flex items-center gap-2">
													<DynamicStyle
														...
														vars={{
															...
															backgroundColor: isOverlayPreview
																? "transparent"
																: token.color
																	? `${token.color}20`
																	: undefined,
							...
							<Button
								onClick={() => navigate("/warden-directives/token-library")}
								variant="outline"
						...
						<AscendantWindow title="PLACED TOKENS" variant="quest">
							<div className="space-y-2 max-h-64 overflow-y-auto text-xs">
										...
										>
											...
											<div className="flex gap-1">
												<button
													...
													aria-label={
														token.locked ? "Unlock token" : "Lock token"
													}
												>
													{token.locked ? (
								...
								{visibleTokens.length === 0 && (
									<AscendantText className="block text-muted-foreground text-center py-2 text-xs">
					...
					<div className="lg:col-span-3">
						<AscendantWindow title="MAP CANVAS">
							<DynamicStyle
								as="div"
								ref={mapRef}
                        ...
                        `,
							...
							<div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
								<div>
									Click map to place selected token. Drag tokens to move.
