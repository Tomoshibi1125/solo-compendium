...
const CharacterNew = () => {
	...
	const isPointBuyValid =
		abilityMethod !== "point-buy" || pointBuyRemaining >= 0;
	...
	const { data: jobs = [] } = useQuery({
		...
		queryFn: async () => {
			...
			const staticJobsData = staticJobs.map((job) => ({
				...
				>,
				saving_throw_proficiencies: (job.savingThrows || []).map(
					mapAbilityToSA,
				) as Array<"STR" | "AGI" | "VIT" | "INT" | "SENSE" | "PRE">,
	...
	const { data: paths = [] } = useQuery({
		...
		queryFn: async () => {
			...
			const filtered = jobName
				? staticPaths.filter(
						(p) => p.job_name === jobName || p.job_id === selectedJob,
					)
				: staticPaths;
	...
	const pathUnlockLevel = useMemo(() => {
		const levels = paths
			.map((p) =>
				typeof (p as { path_level?: number }).path_level === "number"
					? (p as { path_level?: number }).path_level
					: null,
			)
	...
	const jobData = allJobs.find((j) => j.id === selectedJob) as
		| (Job & { _homebrew?: boolean })
		| undefined;
	...
	const handleRollStats = () => {
		...
		const newRolledStats = Array.from({ length: 6 }, () => roll4d6()).sort(
			(a, b) => b - a,
		);
	...
	const handleCreate = async () => {
		...
		try {
			...
			if (
				staticJobData?.specialSenses &&
				staticJobData.specialSenses.length > 0
			) {
			...
			const character = await createCharacter.mutateAsync({
				...
				condition_immunities:
					(staticJobData?.conditionImmunities as never) ?? null,
				conditions: [],
				exhaustion_level: 0,
			} as never);
			...
			if (isLocalCharacterId(character.id)) {
				setLocalAbilities(
					character.id,
					finalAbilities as unknown as Record<DbAbilityScore, number>,
				);
			} else {
				const abilityUpdates = Object.entries(finalAbilities).map(
					([ability, score]) => ({
			...
			try {
				...
				const featureIds = (level1Features || [])
					.map((row) => row.id)
					.filter(Boolean);
				if (featureIds.length > 0) {
					...
					if (groupIds.length > 0) {
						toast({
							...
							description:
								"The System detected pending selections. Open your Ascendant sheet to bind them.",
						});
					}
				}
			} catch {
					...
					<div className="bg-card border border-border rounded-lg p-4 shadow-sm">
						...
						<div className="mt-4 text-center">
							<SystemText className="block text-sm text-muted-foreground">
								...
								{currentStep === "job" &&
									"Select your character's primary class"}
								{currentStep === "abilities" &&
									"Choose your character's ability scores"}
								{currentStep === "path" &&
									"Choose a specialization within your job"}
								{currentStep === "background" &&
									"Define your character's history and origins"}
					...
					{currentStep === "abilities" && (
						<div className="space-y-6">
							...
							<div className="flex gap-4 flex-wrap">
								...
								<Button
									variant={
										abilityMethod === "point-buy" ? "default" : "outline"
									}
							...
							{abilityMethod === "manual" && (
								<div className="space-y-4">
									<div className="flex gap-4 items-center">
										<Button variant="outline" onClick={handleRollStats}>
											Roll 4d6 (Drop Lowest)
										</Button>
										{rolledStats.length > 0 && (
											<div className="text-sm font-heading">
												Rolled: {rolledStats.join(", ")} - Assign these to
												abilities below
											</div>
										)}
									</div>
								...
								{(Object.keys(ABILITY_NAMES) as AbilityScore[]).map(
									(ability) => (
										<div key={ability}>
													...
													if (abilityMethod === "point-buy") {
														...
														if (nextSpent > POINT_BUY_TOTAL) {
															toast({
																...
																description:
																	"You only have 27 points to spend.",
																variant: "destructive",
															});
															return;
														}
											...
											<div className="text-xs text-muted-foreground mt-1">
												Modifier:{" "}
												{Math.floor((effectiveAbilities[ability] - 10) / 2) >= 0
													? "+"
													: ""}
									...
									{staticJobFeatures && staticJobFeatures.length > 0 && (
										<div className="border-t border-border/50 pt-3">
											...
											>
												...
												{showJobFeatures ? "Hide" : "Show"} Level Progression (
												{staticJobFeatures.length} features)
											</button>
											{showJobFeatures && (
								...
								<SelectContent>
									{allBackgrounds.map((bg) => (
										<SelectItem key={bg.id} value={bg.id}>
											...
											{(bg as Background & { _marketplace?: boolean })
												._marketplace && (
												<Badge variant="outline" className="ml-2 text-xs">
													Marketplace
												</Badge>
											)}
											{(bg as Background & { _homebrew?: boolean })
												._homebrew && (
												<Badge variant="outline" className="ml-2 text-xs">
													Homebrew
												</Badge>
							...
							staticJobData.startingEquipment.length === 0 ? (
								<div className="text-center py-8 text-muted-foreground">
									...
									<p className="text-xs mt-1">
										Equipment will be assigned automatically.
									</p>
								</div>
							) : (
								<>
							...
							{jobData && (
								<div>
									...
									<div className="text-sm space-y-1">
										...
										<div>
											<strong>AC:</strong>{" "}
											{10 + Math.floor((effectiveAbilities.AGI - 10) / 2)}
										</div>
										<div>
							...
							{staticJobData && (
								<div>
									...
									<div className="space-y-2 text-sm">
										...
										{typeof staticJobData.darkvision === "number" && (
											<div>
												<strong>Darkvision:</strong> {staticJobData.darkvision}{" "}
												ft
											</div>
										)}
