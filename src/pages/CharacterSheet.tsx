...
const CharacterSheet = () => {
	...
	const [customModifiersDraft, setCustomModifiersDraft] = useState<
		CustomModifier[]
	>([]);
		...
		try {
			...
			playerTools
				.trackHealthChange(
					character.id,
					delta,
					direction === "damage" ? "damage" : "healing",
				)
				.catch(console.error);
					...
					if (scope === "campaign") {
						...
						if (result.concentrationLost && result.spellName) {
							...
							toast({
								...
								description: `Failed save for ${result.spellName}`,
								variant: "destructive",
							});
						} else {
							toast({
								title: "Concentration Maintained",
								description: `Passed save for ${result.spellName}`,
							});
						}
					}
				}
	...
	const handleResourcesChange = async (
		nextResources: typeof characterResources,
	) => {
		if (!character || isReadOnly) return;
		try {
			await saveSheetState({ resources: nextResources });
		...
		if (nextTempHp !== character.hp_temp) {
			...
			playerTools
				.trackHealthChange(character.id, nextTempHp, "temp")
				.catch(console.error);
		}
		...
		if (
			nextInspiration.inspiration_points > currentInspiration.inspiration_points
		) {
			playerTools
				.trackConditionChange(character.id, "Inspiration", "add")
				.catch(console.error);
		} else if (
			nextInspiration.inspiration_points < currentInspiration.inspiration_points
		) {
			playerTools
				.trackConditionChange(character.id, "Inspiration", "remove")
				.catch(console.error);
		}
	};
	...
	const handleAddCustomCondition = () => {
		...
		// DDB Parity Integration
		playerTools
			.trackConditionChange(character.id, trimmed, "add")
			.catch(console.error);
	};
	...
	const _openProficiencyDialog = () => {
		...
		setWeaponProficienciesDraft(
			(character.weapon_proficiencies || []).join(", "),
		);
			...
			<div className="container mx-auto px-4 sm:px-6 py-4 sm:py-8 max-w-7xl">
							...
							<div className="p-4 border-b border-primary/20 flex flex-wrap gap-4 items-center justify-between bg-black/40">
								...
								<div
									className={cn(
										...
										character.level >= 17
											? "border-gate-s/50 text-gate-s shadow-[0_0_10px_hsl(var(--gate-s)/0.4)]"
											: character.level >= 13
												? "border-gate-a/50 text-gate-a shadow-[0_0_10px_hsl(var(--gate-a)/0.4)]"
												: character.level >= 9
													? "border-gate-b/50 text-gate-b shadow-[0_0_8px_hsl(var(--gate-b)/0.3)]"
													: character.level >= 5
														? "border-gate-c/50 text-gate-c shadow-[0_0_8px_hsl(var(--gate-c)/0.3)]"
														: character.level >= 2
															? "border-gate-d/50 text-gate-d shadow-[0_0_8px_hsl(var(--gate-d)/0.3)]"
								...
								<div className="p-4 border-b sm:border-b-0 sm:border-r border-primary/20 flex-1 flex flex-col justify-center gap-2 bg-black/20">
									...
									{(pathDisplayName || backgroundDisplayName) && (
										<div className="flex items-center gap-2">
											...
											<span className="font-system text-primary/80 uppercase tracking-wider text-xs">
												{pathDisplayName || "None"}
											</span>
											{backgroundDisplayName && (
												<>
													<span className="text-primary/30 mx-1">|</span>{" "}
													<span className="font-system text-primary/60 uppercase tracking-wider text-xs">
														{backgroundDisplayName}
													</span>
												</>
											)}
									...
									<div className="flex items-center gap-2 mt-1">
										...
										<span className="font-system text-white font-bold text-xl drop-shadow-[0_0_5px_currentColor]">
											{character.level}
										</span>
									</div>
								</div>
				...
				<Tabs defaultValue="overview" className="space-y-4 sm:space-y-6">
					...
					<TabsContent value="overview" className="space-y-6 mt-0">
						...
						<div className="character-sheet-stats-grid grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
								...
								<div className="pt-6 sm:pt-8 text-center">
									<DefensesModal
										acBreakdown={{
											...
											shieldBonus:
												equipmentMods.armorClass - baseStats.armorClass > 0
													? equipmentMods.armorClass - baseStats.armorClass
													: 0,
											magicalBonus:
												runeBonuses.ac - equipmentMods.armorClass > 0
													? runeBonuses.ac - equipmentMods.armorClass
													: 0,
										...
										resistances={
											(character as unknown as Record<string, unknown>)
												.damage_resistances as string[] | undefined
										}
										immunities={
											(character as unknown as Record<string, unknown>)
												.damage_immunities as string[] | undefined
										}
										vulnerabilities={
											(character as unknown as Record<string, unknown>)
												.damage_vulnerabilities as string[] | undefined
										}
										conditionImmunities={
											(character as unknown as Record<string, unknown>)
												.condition_immunities as string[] | undefined
										}
										characterId={character.id}
									/>
							...
							>
								...
								<div className="pt-6 sm:pt-8 text-center relative">
									...
									>
										<div
											className="font-display text-3xl sm:text-4xl font-bold mb-1 tracking-tighter text-white inline-flex items-center gap-2"
											data-testid="initiative-display"
										>
									...
									{initiativeAdvantage !== "normal" && (
										<Badge
											variant="outline"
											className="mt-2 text-[10px] border-amber-500/50 text-amber-400 px-1 py-0 h-4"
										>
							...
							>
								...
								<div className="pt-6 sm:pt-8 text-center">
									<div
										className="font-display text-3xl sm:text-4xl font-bold mb-1 tracking-tighter text-white"
										data-testid="speed-display"
									>
										{finalSpeed}
										<span className="text-lg ml-1 font-mono text-muted-foreground">
											FT
										</span>
									</div>
									<div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
							...
							<div className="character-sheet-ability-grid grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
								{ABILITY_KEYS.map((ability) => {
									...
									const equipmentBonus =
										equipmentMods.abilityModifiers[
											ability.toLowerCase() as keyof typeof equipmentMods.abilityModifiers
										] || 0;
									const customAbilityBonus =
										sumCustomModifiers(customModifiers, "ability", ability) +
										sumCustomModifiers(
											customModifiers,
											"ability_bonus",
									...
									return (
										<div
											key={ability}
											className="text-center group bg-card border border-border rounded-lg p-3 sm:p-4 relative"
										>
											<div className="text-xs text-muted-foreground mb-1 font-heading">
											...
											<button
												...
												onClick={() =>
													ddbEnhancements.rollAbilityCheck(ability)
												}
											...
											{isProficient && (
												<Badge
													variant="secondary"
													className="mt-1 text-xs absolute top-1 right-1"
												>
													P
												</Badge>
											)}
							...
							<div className="character-sheet-saving-throws-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
								{ABILITY_KEYS.map((ability) => {
									...
									return (
										<div
											key={ability}
											className="flex items-center justify-between p-3 sm:p-2 rounded-lg bg-card border border-border group hover:bg-muted/50 transition-colors"
										>
											<div className="flex items-center gap-2">
											...
											<div className="flex items-center gap-2">
												{isEditingSave ? (
													<Button
														...
														onClick={() =>
															handleToggleSavingThrowProficiency(ability)
														}
												...
												<Button
													...
													size="sm"
													onClick={() =>
														ddbEnhancements.rollSavingThrow(ability)
													}
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
								{allSkills.map((skill) => {
									...
									return (
										<div
											key={skill.name}
											className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-card border border-border group hover:bg-muted/50 transition-colors"
										>
											...
											<div className="flex items-center gap-1.5 flex-shrink-0">
												...
												<Button
													...
													size="sm"
													onClick={() =>
														ddbEnhancements.rollSkillCheck(skill.name)
													}
						...
						<SensesDisplay
							senses={
								(character as unknown as Record<string, unknown>).senses as
									| string[]
									| undefined
							}
						...
						<DeathSaveTracker
							...
							onRollDeathSave={() => {
								...
								if (result.isNat20) {
									...
									playerTools
										.trackHealthChange(character.id, 1, "healing")
										.catch(console.error);
								}
								...
								if (scope === "campaign") {
									...
									if (
										result.message.includes("died") ||
										result.message.includes("stable")
									) {
										playerTools
											.trackConditionChange(
												character.id,
							...
							onStabilize={() => {
								...
								toast({
									title: "Stabilized",
									description: "Character is stable.",
								});

								playerTools
									.trackConditionChange(character.id, "Stable", "add")
									.catch(console.error);
							}}
										...
										{!isReadOnly && (
											<>
												<Button
													...
													className="h-6 w-6 p-0"
													onClick={() =>
														handleResourceAdjust("system_favor_current", -1)
													}
										...
										{getAvailableFavorOptions(character.level).map((opt) => (
											<button
												...
												className={cn(
													...
													character.system_favor_current >= opt.cost &&
														!isReadOnly
									...
									<div className="space-y-3">
										<div className="grid grid-cols-2 gap-2 text-xs">
											{ALL_CONDITIONS.map((condition) => {
												const isActive = (character.conditions || []).some(
													(item) =>
														item.toLowerCase() === condition.name.toLowerCase(),
												);
												return (
													<label
														key={condition.name}
													...
													>
														<Checkbox
															checked={isActive}
															onCheckedChange={() =>
																handleToggleCondition(condition.name)
															}
										...
										<div className="flex items-center gap-2">
											<Input
												...
												onChange={(e) =>
													setCustomConditionDraft(e.target.value)
												}
						...
						{!isReadOnly && (
							<CharacterResourcesPanel
								...
								isDead={
									...
									characterResources.death_saves.death_save_failures >= 3
								}
							/>
						)}
					...
					<TabsContent value="actions" className="space-y-6 mt-0">
						...
						<CharacterRollsPanel
							...
							skills={allSkills.map((skill) => ({
								name: skill.name,
								ability: skill.ability,
								proficiency: (character.skill_expertise || []).includes(
									skill.name,
								)
									? "expertise"
								...
								<SpellPanel
									...
									onTogglePrepared={async (spellId, prepared) => {
										...
										if (isLocalCharacterId(character.id)) {
											const { updateLocalPower } = await import(
												"@/lib/guestStore"
											);
									...
									onRestoreSlot={async (level) => {
										...
										if (isLocalCharacterId(character.id)) {
											...
											const slotRow = spellSlotData.find(
												(s) => s.level === level,
											);
											if (slotRow)
												updateLocalSpellSlotRow(
													`${character.id}-slot-${level}`,
										...
										toast({
											title: "Slot Restored",
											description: `Level ${level} spell slot restored.`,
										});
									}}
								/>
							);
			...
			>
				<DialogContent>
					...
					<DialogFooter>
						<Button
							variant="outline"
							onClick={() => setProficiencyDialogOpen(false)}
						>
							Cancel
						</Button>
						<Button
				...
				<>
						...
						onFinishRest={(totalRecovered, hitDiceSpent) => {
							updateCharacter.mutate({
								...
								data: {
									...
									hit_dice_current: Math.max(
										0,
										character.hit_dice_current - hitDiceSpent,
									),
								},
							});
					...
					<LongRestDialog
						...
						onConfirmRest={() => {
							...
							playerTools
								.trackConditionChange(character.id, "Long Rest", "add")
								.catch(console.error);
							playerTools
								.trackHealthChange(character.id, character.hp_max, "healing")
								.catch(console.error);
