...
const THEME_SWATCH_CLASSES: Partial<
	Record<DiceTheme, { icon: string; bar: string }>
> = {
	"umbral-ascendant": {
...
const DiceRoller = () => {
	...
	const [pendingResolution, setPendingResolutionState] =
	...
	useEffect(() => {
		if (!show3D) return;
		void import("@/lib/three/loaders").then(({ initThreeLoaders }) =>
			initThreeLoaders(),
		);
							...
							{campaigns.length > 0 && (
								<div className="flex items-center gap-2 bg-muted/50 p-1.5 rounded-lg border border-border">
									...
									<Select
										...
										onValueChange={(value) =>
											setCampaignId(value === "none" ? null : value)
										}
									>
									...
									{resolutionOutcome && (
										<div className="text-sm">
											{resolutionOutcome.kind === "attack" && (
												<div>
													<div>
														Attack total: {resolutionOutcome.attackTotal} vs AC{" "}
											...
											{resolutionOutcome.kind === "save" && (
												<div>
													<div>
														Save total: {resolutionOutcome.saveTotal} vs DC{" "}
										...
										{Object.entries(DICE_THEMES).map(([key, theme]) => {
											...
											return (
												...
												>
													...
													<div
														className={cn(
															"w-full h-2 rounded-full opacity-30",
															swatch?.bar,
														)}
													/>
												</button>
								...
								{["1d20", "1d20+5", "2d6", "1d8+3", "1d4", "1d12"].map(
									(notation) => (
										<Button
											...
											data-testid={
												notation === "1d20" ? "quick-roll-1d20" : undefined
											}
										>
											{notation}
										</Button>
							...
							<div className="space-y-4">
								...
								<div className="flex flex-wrap gap-3">
									{diceTypes.map(({ sides, label }) => {
										const selected = selectedDice.find(
											(d) => d.sides === sides,
										);
										return (
