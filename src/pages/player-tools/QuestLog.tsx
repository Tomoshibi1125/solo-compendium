...
export function QuestLog({ characterId }: { characterId: string }) {
	...
	const [configForm, setConfigForm] = useState<QuestConfigForm>(
		buildConfigForm(),
	);
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: "Unable to save daily quest settings.",
				variant: "destructive",
			});
		}
	};
	...
	const completeQuest = async (quest: DailyQuestInstance) => {
		...
		try {
			...
			toast({
				...
				description:
					error instanceof Error ? error.message : "Unable to update quest.",
				variant: "destructive",
			});
		}
	};
		...
		try {
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: "Unable to update quest progress.",
				variant: "destructive",
			});
		}
	};
	...
	const renderQuestCard = (quest: DailyQuestInstance) => {
		...
		const progress = getQuestProgress(quest);
		const reward = normalizeQuestReward(quest, template);
		const rewardsApplied = Boolean(
			(quest.rewards_granted as Record<string, unknown> | null)?.applied,
		);
			...
			<Card key={quest.id} className="border border-muted">
				...
				<CardContent className="space-y-4">
					<div className="flex flex-wrap gap-2">
						{template?.tier && (
							<Badge variant="secondary">Tier {template.tier}</Badge>
						)}
						{template?.category && (
					...
					<div className="space-y-2">
						...
						<Progress
							value={(progress.current / Math.max(1, progress.target)) * 100}
						/>
					</div>

					<div className="text-sm text-muted-foreground">
						<strong className="text-foreground">Reward:</strong>{" "}
						{formatReward(reward, allowExperienceRewards)}
					</div>
					...
					<div className="flex flex-wrap gap-2">
						...
						{quest.status === "completed" && !rewardsApplied && (
							<Button
								size="sm"
								variant="secondary"
								onClick={() => applyRewards(quest, reward)}
							>
								Claim Rewards
							</Button>
						)}
						...
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(value) =>
									setConfigForm((prev) => ({ ...prev, enabled: value }))
								}
							/>
						</div>
							...
							<div>
								...
								>
									...
									<SelectContent>
										...
										<SelectItem value="system_fatigue">
											System Fatigue
										</SelectItem>
						...
						<div className="text-2xl font-bold">
							{
								completedQuests.filter(
									(quest) =>
										(quest.rewards_granted as Record<string, unknown> | null)
											?.applied,
								).length
							}
						</div>
