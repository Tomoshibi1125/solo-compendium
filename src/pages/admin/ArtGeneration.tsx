...
export default function ArtGenerationAdmin() {
	...
	const fetchArtAsset = async (
		entityId: string,
		variant: string,
	): Promise<ArtAssetApiRow | null> => {
	...
	const handleTestGeneration = async () => {
		...
		try {
			const cachedAsset = await fetchArtAsset(
				testRequest.entityId,
				testRequest.variant,
			);
			if (cachedAsset) {
				const pathValues =
					cachedAsset.paths && typeof cachedAsset.paths === "object"
				...
				if (result && typeof result.success === "boolean" && result.success) {
					...
					for (let attempt = 0; attempt < 4; attempt += 1) {
						fallbackAsset = await fetchArtAsset(
							testRequest.entityId,
							testRequest.variant,
						);
						if (fallbackAsset) {
					...
					if (fallbackAsset) {
						const pathValues =
							fallbackAsset.paths && typeof fallbackAsset.paths === "object"
	...
	return (
		<div className="container mx-auto p-6 space-y-6">
			...
			{!enabled && (
				<Alert>
					<AlertDescription>
						Art generation is disabled. Enable it with{" "}
						<code>VITE_FEATURE_ART_GENERATION=true</code>. You can override the
						free image backend with <code>VITE_FREE_IMAGE_API</code>.
					</AlertDescription>
				</Alert>
			)}
									...
									<div className="grid grid-cols-2 gap-4">
										<div>
											...
											<p className="text-2xl font-bold">
												{queueMonitor.running?.length || 0}
											</p>
										</div>
										<div>
											<h4 className="font-medium">Pending</h4>
											<p className="text-2xl font-bold">
												{queueMonitor.pending?.length || 0}
											</p>
										</div>
									</div>
