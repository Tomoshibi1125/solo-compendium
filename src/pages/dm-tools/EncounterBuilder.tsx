...
const EncounterBuilder = () => {
	...
	useEffect(() => {
		...
		isToolStateLoading,
		persistenceContext,
		storedState.encounterMonsters,
		storedState.hunterCount,
		storedState.hunterLevel,
	]);
			...
			const withTimeout = async <T,>(
				...
				const timeoutPromise = new Promise<T>((_, reject) => {
					timeoutId = setTimeout(
						() => reject(new Error("Monster query timed out")),
						timeoutMs,
					);
				});
				try {
	...
	const handleSaveEncounter = async () => {
		...
		if (encounterMonsters.length === 0) {
			toast({
				...
				description:
					"Include at least one monster before saving the encounter.",
				variant: "destructive",
			});
			return;
		}
			...
			try {
				...
				const description =
					error instanceof Error
						? error.message
						: "Unable to sync to campaign combat session.";
				toast({
							...
							<div className="space-y-4">
								...
								<div className="pt-4 border-t border-border">
									...
									<div className="flex items-center justify-between">
										...
										<Badge
											variant={
												difficulty === "deadly" ? "destructive" : "secondary"
											}
											className={cn(
												"font-display",
												difficultyColors[
											...
											>
												<div className="flex-1">
													...
													<div className="text-xs text-muted-foreground">
														CR {em.monster.cr || "?"} |{" "}
														{calculateXP(em.monster, em.quantity)} XP
													</div>
												</div>
												<div className="flex items-center gap-2">
													<Button
														...
														className="h-6 w-6"
														onClick={() =>
															updateQuantity(em.id, em.quantity + 1)
														}
...
function EncounterAIEnhanceButton({
	encounterMonsters,
	hunterLevel,
	hunterCount,
	difficulty,
	...
	const handleAIEnhance = async () => {
		const monsterList = encounterMonsters
			.map(
				(em) =>
					`${em.quantity}x ${em.monster.name} (CR ${em.monster.cr || "?"})`,
			)
			.join(", ");
