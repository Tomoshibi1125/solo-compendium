...
export default function ArtGeneratorDM() {
	...
	const {
		...
		isLoading,
		saveNow,
	} = useUserToolState<GeneratedContent[]>("art_generator", {
		initialState: [],
	...
	const updateCurrentItemData = useCallback(
		(updates: GeneratedContentData) => {
			if (!currentEditItem) return;
	...
	const fetchArtAsset = useCallback(
		...
		for (let attempt = 0; attempt < 5; attempt += 1) {
			for (const candidateId of entityIdCandidates) {
				try {
					...
					if (apiAsset) {
						const previewUrl =
							apiAsset.paths && typeof apiAsset.paths === "object"
								? Object.values(apiAsset.paths as Record<string, string>).find(
					...
					const assets = await artPipeline.getAssetsForEntity(
	...
	const handleArtGenerated = (assetId: string, previewUrl?: string) => {
		...
		if (targetId) {
			setGeneratedContent((prev) =>
				prev.map((item) =>
					item.id === targetId
						? { ...item, artId: assetId, artUrl: previewUrl }
						: item,
				),
			);
			if (currentEditItem?.id === targetId) {
				setCurrentEditItem((prev) =>
					prev ? { ...prev, artId: assetId, artUrl: previewUrl } : prev,
				);
			}
			setIsAIArtPending(false);
			return;
	...
	const handleSaveAsToken = async (item: GeneratedContent, artSrc: string) => {
		...
		const currentTokens = Array.isArray(storedTokens)
			? storedTokens
			: DEFAULT_TOKENS;
		...
		<Layout>
			<div className="container mx-auto p-6 space-y-6">
				...
				>
					...
					{/* Content Lists */}
					{(["monster", "npc", "item", "location"] as ContentType[]).map(
						(type) => (
							<TabsContent key={type} value={type} className="space-y-4">
								...
								{/* Empty State */}
								{generatedContent.filter((item) => item.type === type)
									.length === 0 && (
					...
					<Dialog
						...
						onOpenChange={(open) => {
							if (!open) {
								...
								if (
									currentEditItem &&
									!currentEditItem.artId &&
									!currentEditItem.artUrl
								) {
					...
					>
						<DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
							<DialogHeader>
								...
								<DialogDescription>
									Create custom System Ascendant themed art for your{" "}
									{currentEditItem.type}.
								</DialogDescription>
							</DialogHeader>
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
								<div>
									...
									<Input
										...
										onChange={(event) =>
											updateCurrentItem({ name: event.target.value })
										}
								...
								<div className="md:col-span-2">
									...
									<Textarea
										...
										onChange={(event) =>
											updateCurrentItemData({ description: event.target.value })
										}
								...
								<div>
									...
									<Input
										...
										onChange={(event) =>
											updateCurrentItemData({ rarity: event.target.value })
										}
								...
								<div>
									...
									<Input
										...
										onChange={(event) =>
											updateCurrentItemData({ environment: event.target.value })
										}
								...
								<TabsContent value="ai">
									<AIEnhancedArtGenerator
										...
										title={
											...
											`Generated ${formatContentTypeLabel(currentEditItem.type)}`
										}
										onArtGenerated={handleArtGenerated}
