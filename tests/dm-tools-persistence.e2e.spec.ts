...
test.describe.serial('DM tools persistence: reload restores state', () => {
  ...
  test('rollable tables: tab + last roll persist after reload', async () => {
    ...
    await expect(dmPage.getByText('PROTOCOL WARDEN TABLES', { exact: false }).first()).toBeVisible({ timeout: 15_000 });

    // Wait for initial tool-state hydration to settle so immediate saves are not skipped.
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => localStorage.getItem('solo-compendium.dm-tools.rollable-tables.v1'));
      })
      .not.toBeNull();
    ...
    const rollRewardBtn = dmPage.getByRole('button', { name: /Roll Reward/i });
    ...
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => {
          const raw = localStorage.getItem('solo-compendium.dm-tools.rollable-tables.v1');
          if (!raw) return null;
          try {
            return JSON.parse(raw) as { activeTab?: string; results?: Record<string, string> };
          } catch {
            return null;
          }
        });
    ...
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => {
          ...
          if (!raw) return '';
          try {
            const parsed = JSON.parse(raw) as { results?: Record<string, string> };
    ...
    await expect(dmPage.getByRole('button', { name: /Roll Reward/i })).toBeVisible({ timeout: 15_000 });

    // Rolled reward panel should still be present.
    await expect(dmPage.getByText('Reward').first()).toBeVisible({ timeout: 5_000 });
  });

  test('rift generator: selected rank + last rift persist after reload', async () => {
    await dmPage.goto('/dm-tools/gate-generator');
    await expect(dmPage.getByText('RIFT GENERATOR', { exact: false }).first()).toBeVisible({ timeout: 15_000 });

    // Wait for initial tool-state hydration to settle.
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => localStorage.getItem('solo-compendium.dm-tools.gate-generator.v1'));
      })
      .not.toBeNull();
    await dmPage.waitForTimeout(500);
    ...
    const generateBtn = dmPage.getByRole('button', { name: /Generate Rift/i });
    ...
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => {
          const raw = localStorage.getItem('solo-compendium.dm-tools.gate-generator.v1');
          if (!raw) return null;
          try {
            return JSON.parse(raw) as { selectedRank?: string; rift?: unknown };
          } catch {
            return null;
          }
        });
    ...
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => {
          ...
          if (!raw) return false;
          try {
            const parsed = JSON.parse(raw) as { rift?: { description?: string } | null };
    ...
    await expect(dmPage.getByRole('button', { name: /Copy Details/i })).toBeVisible({ timeout: 15_000 });
  });
  ...
  test('treasure generator: selected rank + last treasure persist after reload', async () => {
    ...
    await expect(dmPage.getByText('TREASURE GENERATOR', { exact: false }).first()).toBeVisible({ timeout: 15_000 });

    // Wait for initial tool-state hydration to settle.
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => localStorage.getItem('solo-compendium.dm-tools.treasure-generator.v1'));
      })
      .not.toBeNull();
    await dmPage.waitForTimeout(500);
    ...
    if (await rankBOption.isVisible({ timeout: 3_000 }).catch(() => false)) {
      ...
      await dmPage.getByText(/Rank B/i).first().click();
    }
    await dmPage.waitForTimeout(300);

    const generateBtn = dmPage.getByRole('button', { name: /Generate Treasure/i });
    await expect(generateBtn).toBeVisible({ timeout: 10_000 });
    await generateBtn.click();
    ...
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => {
          const raw = localStorage.getItem('solo-compendium.dm-tools.treasure-generator.v1');
          if (!raw) return null;
          try {
            return JSON.parse(raw) as { selectedRank?: string; treasure?: unknown };
          } catch {
            return null;
          }
        });
    ...
    await expect
      .poll(async () => {
        return await dmPage.evaluate(() => {
          ...
          if (!raw) return false;
          try {
            const parsed = JSON.parse(raw) as { treasure?: { gold?: number } | null };
