...
test.describe("Dual-context: DM + Player campaign flow", () => {
	test("DM creates campaign + homebrew, Player joins and rolls dice", async ({
		browser,
	}) => {
		...
		test.info().attach("dm-context", {
			contentType: "text/plain",
			body: "DM browser context",
		});
		test.info().attach("player-context", {
			contentType: "text/plain",
			body: "Player browser context",
		});
		...
		try {
			...
			const campaignId = await dm.createCampaign(
				campaignName,
				"E2E dual-context test campaign",
			);
			...
			test.info().attach("player-final-state", {
	...
	test("Player applies homebrew feature to character (requires compendium data)", async ({
		browser,
	}) => {
		...
		expect(
			characterId,
			"Need an existing character or a creatable character to continue dual-context test",
		).toBeTruthy();
		if (!characterId) {
