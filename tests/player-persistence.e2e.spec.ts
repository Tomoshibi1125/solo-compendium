...
test.describe.serial('Player persistence: reload restores state', () => {
  ...
  const openFirstCharacterSheet = async (page: Page) => {
    ...
    await expect(page.getByRole('button', { name: 'Back to Characters' })).toBeVisible({ timeout: 20_000 });
    await expect(page.getByText('HIT POINTS', { exact: true })).toBeVisible({ timeout: 20_000 });
  };

  const getSpellSlotSummary = async (page: Page) => {
    const slotDecreaseCount = await page.locator('[data-testid^="spell-slot-decrease-tier-"]').count();
    if (slotDecreaseCount === 0) {
      return { slotDecreaseCount, firstContainerText: null as string | null };
    }

    const slotDecreaseBtn = page.locator('[data-testid^="spell-slot-decrease-tier-"]').first();
    await expect(slotDecreaseBtn).toBeVisible({ timeout: 10_000 });
    const slotContainer = slotDecreaseBtn.locator('xpath=ancestor::div[contains(@class,"flex")]');
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    await expect(playerPage.locator('h1')).toContainText("ASCENDANT REGISTRY", { timeout: 15_000 });
  });
  ...
  test('character sheet: spell slots persist after reload', async ({ browser }) => {
    ...
    if (initial.slotDecreaseCount === 0) {
      ...
      await expect(playerPage.getByRole('button', { name: 'Back to Characters' })).toBeVisible({ timeout: 15_000 });
    ...
    const slotDecreaseBtn = playerPage.locator('[data-testid^="spell-slot-decrease-tier-"]').first();
  ...
  test('character sheet: resources/HP persist after reload', async ({ browser }) => {
    if (!playerPage.url().includes('/characters/')) {
      await openFirstCharacterSheet(playerPage);
    }

    await expect(playerPage.getByText('HIT POINTS', { exact: true })).toBeVisible({ timeout: 15_000 });
    await expect(playerPage.locator('[data-testid="hp-current-display"]').first()).toBeVisible({ timeout: 15_000 });

    const deltaInput = playerPage.locator('[data-testid="hp-delta-input"]').first();
    await expect(deltaInput).toBeVisible({ timeout: 10_000 });

    const initialDisplayed = (await playerPage.locator('[data-testid="hp-current-display"]').first().innerText()).trim();
    await deltaInput.fill('1');
    await playerPage.locator('[data-testid="hp-damage-button"]').first().click();

    await playerPage.waitForTimeout(1000);

    await playerPage.reload();
    await expect(playerPage.getByRole('button', { name: 'Back to Characters' })).toBeVisible({ timeout: 15_000 });
    const reloadedDisplayed = (await playerPage.locator('[data-testid="hp-current-display"]').first().innerText()).trim();
    ...
    try {
      ...
      await expect(newSession.page.getByRole('button', { name: 'Back to Characters' })).toBeVisible({ timeout: 15_000 });
      const freshDisplayed = (await newSession.page.locator('[data-testid="hp-current-display"]').first().innerText()).trim();
