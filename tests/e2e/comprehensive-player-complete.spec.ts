...
test.describe('Comprehensive Player Complete Test', () => {
  ...
  test('Player Complete UI/UX Journey - Every Possible Interaction', async () => {
    ...
    try {
      await authPage.signIn(PLAYER_EMAIL, PLAYER_PASSWORD, 'player');
      ...
      try {
        ...
        for (let i = 0; i < roleCount; i++) {
          ...
          if (buttonText?.toLowerCase().includes('player')) {
    ...
    try {
      ...
      for (const element of formElements) {
        try {
          ...
          console.log(`    Form element test failed: ${element} - ${(formError as Error).message}`);
        }
      }
      ...
      console.log(`  ❌ Character creation test failed: ${(error as Error).message}`);
    }
    ...
    try {
      ...
      const characterCards = page.locator('[data-testid*="character"], .character-card, .character-item');
      ...
      if (charCount > 0) {
        ...
        const sheetTabs = page.locator('[role="tab"], .tab, [data-testid*="tab"]');
      ...
      console.log(`  ❌ Character sheet test failed: ${(error as Error).message}`);
    }
    ...
    try {
      ...
      const toolCards = page.locator('[data-testid*="tool"], .tool-card, .player-tool');
      ...
      for (let i = 0; i < Math.min(toolCount, 8); i++) {
        try {
          ...
          const toolName = await toolCard.locator('h3, h4, .title, [data-testid*="name"]').first().textContent();
    ...
    for (const tool of specificTools) {
      try {
        ...
        (playerToolsTests as any)[tool.name.toLowerCase().replace(' ', '_')] = true;
      } catch (toolError) {
        console.log(`  ❌ ${tool.name} test failed: ${(toolError as Error).message}`);
      }
    }
    ...
    try {
      ...
      for (let i = 0; i < Math.min(categoryCount, 5); i++) {
        try {
          ...
          const categoryName = await category.locator('h2, h3, .title').first().textContent();
          ...
          if (itemCount > 0) {
            ...
            // Test detail view
            const detailContent = page.locator('[data-testid*="detail"], .detail, .content');
          ...
          console.log(`    Category test failed: ${(categoryError as Error).message}`);
        }
      }
    ...
    for (const viewport of viewports) {
      try {
        ...
        if (await nav.isVisible()) {
          ...
          if (navBox) {
            console.log(`    Navigation visible: ${navBox.width}x${navBox.height}`);
          }
        }
        ...
        if (await mainContent.isVisible()) {
          ...
          if (contentBox) {
            console.log(`    Content visible: ${contentBox.width}x${contentBox.height}`);
          }
        }
        
        // Test mobile menu if on mobile
        if (viewport.width <= 768) {
          const mobileMenu = page.locator('[data-testid*="menu"], .hamburger, .mobile-menu');
        ...
        console.log(`    ❌ ${viewport.name} test failed: ${(viewportError as Error).message}`);
    ...
    console.log(`  ✅ Responsive design ${responsivePassed ? 'passed' : 'failed'}`);
    ...
    try {
      ...
      for (let i = 0; i < Math.min(textCount, 5); i++) {
        try {
          ...
          console.log(`    Text color: ${styles.color}, Background: ${styles.backgroundColor}`);
        } catch (colorError) {
          // Ignore color errors
        }
      }
      ...
      console.log(`  ❌ Accessibility test failed: ${(error as Error).message}`);
    ...
    try {
      ...
      const chatElements = page.locator('[data-testid*="chat"], .chat, .chat-panel');
    ...
    try {
      ...
      for (const url of invalidUrls) {
        try {
          ...
          if (await errorElements.isVisible()) {
            ...
            console.log(`    Error handling for ${url}: ${errorText?.substring(0, 50)}...`);
          }
        } catch (urlError) {
          console.log(`    ❌ Error handling failed for ${url}: ${(urlError as Error).message}`);
      ...
      if (await submitButton.isVisible()) {
        ...
        const validationMessages = page.locator('[data-testid*="error"], .error, .validation-error');
      ...
      console.log(`  ✅ Error handling ${errorHandlingPassed ? 'passed' : 'had issues'}`);
    } catch (error) {
    ...
    try {
      ...
      for (const pagePath of pages) {
        try {
          ...
          console.log(`    ❌ Performance test failed for ${pagePath}: ${(perfError as Error).message}`);
          performancePassed = false;
        }
      }
      
      testResults.performance = performancePassed;
      console.log(`  ✅ Performance ${performancePassed ? 'passed' : 'had issues'}`);
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        console.log(`  ⚠️ Clipboard not available: ${(clipboardError as Error).message}`);
      }
    ...
    for (const [category, result] of Object.entries(results)) {
      ...
      if (typeof result === 'boolean') {
        ...
        for (const [subCategory, subResult] of Object.entries(result)) {
          ...
          console.log(`  ${subResult ? '✅' : '❌'} ${subCategory}: ${subResult ? 'PASS' : 'FAIL'}`);
        }
      }
    }
    ...
    if (passedTests === totalTests) {
      ...
      console.log('✅ MAJORITY OF TESTS PASSED - PLAYER UI/UX IS LARGELY FUNCTIONAL');
    } else {
