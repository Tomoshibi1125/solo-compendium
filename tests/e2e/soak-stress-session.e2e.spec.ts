...
const buildInitiativeTrackerUrl = (campaignIdValue: string, sessionIdValue?: string) => {
...
const dismissAnalyticsIfPresent = async (page: any) => {
  const dismissBtn = page.getByRole('button', { name: /No Thanks|Dismiss/i }).first();
...
const isOnLogin = async (page: any) => {
  const enterShadowRealm = page.getByRole('button', { name: /Enter Shadow Realm/i }).first();
  if (await enterShadowRealm.isVisible({ timeout: 1_000 }).catch(() => false)) return true;
...
const ensureCampaignVttLoaded = async (
  ...
  let visible = await vttInterface.isVisible({ timeout: 5_000 }).catch(() => false);

  if (!visible && (await isOnLogin(page))) {
    await ensureAuthed(page);
    await page.goto(`/campaigns/${campaignIdValue}/vtt`);
    await dismissAnalyticsIfPresent(page);
    visible = await vttInterface.isVisible({ timeout: 5_000 }).catch(() => false);
  }
...
const waitForCharacterSheetLoaded = async (page: any, characterId: string) => {
  ...
  await expect(page.getByRole('button', { name: /Back to Characters/i }).first()).toBeVisible({ timeout: 30_000 });
...
test.describe.serial('Soak/Stress E2E: Simulated 4-hour DM + Player session', () => {
  ...
  test('1. DM creates campaign and prepares shared handouts + notes + chat', async () => {
    ...
    campaignId = await dm.createCampaign(`Soak-Session-${timestamp}`, 'Soak stress campaign for a simulated 4-hour session.');
    ...
    await expect(dmPage.getByText(/CAMPAIGN CHAT/i).first()).toBeVisible({ timeout: 20_000 });
    await dmPage.getByPlaceholder(/Type a message/i).fill(`Soak DM message ${timestamp}`);
    await dmPage.locator('form').locator('button[type="submit"]').click({ timeout: 20_000 });
    await expect(dmPage.getByText(new RegExp(`Soak DM message ${timestamp}`, 'i')).first()).toBeVisible({ timeout: 20_000 });

    // Notes
    await dmPage.getByRole('tab', { name: /Notes/i }).click();
    await dismissAnalyticsIfPresent(dmPage);
    const addNoteBtn = dmPage.getByRole('button', { name: /Add Note|New Note|Create Note|\+|Add/i }).first();
    if (await addNoteBtn.isVisible({ timeout: 2_000 }).catch(() => false)) {
      ...
      const sharedSwitch = dmPage.getByRole('switch', { name: /Shared/i }).first();
      if (await sharedSwitch.isVisible({ timeout: 1_000 }).catch(() => false)) {
    ...
    await dismissAnalyticsIfPresent(dmPage);
    await expect(dmPage.getByText(/JOURNAL & NOTES/i).first()).toBeVisible({ timeout: 20_000 });
    ...
    await expect(dmPage.getByText(new RegExp(`Soak Handout ${timestamp}`, 'i')).first()).toBeVisible({ timeout: 20_000 });
  });
  ...
  test('2. Player joins campaign, creates character, verifies chat + handouts', async () => {
    ...
    await expect(playerPage.getByText(/CAMPAIGN CHAT/i).first()).toBeVisible({ timeout: 20_000 });

    await playerPage.getByRole('tab', { name: /Handouts/i }).click();
    await expect(playerPage.getByText(/HANDOUTS & LORE/i).first()).toBeVisible({ timeout: 20_000 });
    await expect(playerPage.getByText(/Soak Handout/i).first()).toBeVisible({ timeout: 20_000 });
  });
  ...
  test('3. Soak loop: travel/VTT interactions + multi-encounter combat loops + rewards + rests', async () => {
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      await ensureCampaignVttLoaded(playerPage, campaignId, ensurePlayerAuthed);
      const playerCanvas = playerPage.locator('canvas').first();
      if (await playerCanvas.isVisible({ timeout: 5_000 }).catch(() => false)) {
        const box = await playerCanvas.boundingBox();
        if (box) {
          await playerPage.mouse.click(box.x + 60 + i * 10, box.y + 60 + i * 10);
        }
      }
      ...
      if (!(await vttInterface.isVisible({ timeout: 2_000 }).catch(() => false))) {
        await dmPage.goto(`/campaigns/${campaignId}/vtt`);
        await dismissAnalyticsIfPresent(dmPage);
      }
    ...
    for (let encounterIndex = 0; encounterIndex < 4; encounterIndex += 1) {
      ...
      await dmPage.getByTestId('encounter-name').fill(`Soak Encounter ${encounterIndex}`);
      await dmPage.getByTestId('encounter-notes').fill('Simulated combat in a 4-hour session soak test.');
      ...
      await dmPage.waitForURL(/\/dm-tools\/initiative-tracker/i, { timeout: 30_000 });
      ...
      if (sessionId) {
        ...
        await expect(playerPage.getByTestId('campaign-session-play')).toBeVisible({ timeout: 30_000 });
        await expect(playerPage.getByTestId('session-initiative-list')).toBeVisible({ timeout: 30_000 });
      }
      ...
      await dismissAnalyticsIfPresent(dmPage);
      await expect(dmPage.getByText(/LOOT & RELIC DISTRIBUTION/i).first()).toBeVisible({ timeout: 20_000 });
      await expect(dmPage.getByText(/Assign Loot Drop/i).first()).toBeVisible({ timeout: 20_000 });
      await expect(dmPage.locator('#loot-item')).toBeVisible({ timeout: 20_000 });

      // Assign a loot drop.
      await dmPage.fill('#loot-item', `Loot-${encounterIndex}`);
      await dmPage.fill('#loot-qty', '1');
      await dmPage.fill('#loot-value', '10');
      await dmPage.getByTestId('campaign-loot-assign').click({ timeout: 20_000 });

      // Assign a relic.
      await expect(dmPage.locator('#relic-name')).toBeVisible({ timeout: 20_000 });
      ...
      await dmPage.getByTestId('campaign-relic-assign').click({ timeout: 20_000 });
      ...
      await expect(dmPage.getByTestId('initiative-tracker')).toBeVisible({ timeout: 30_000 });
    }
  });

  test('4. Persistence validation: new contexts re-open campaign and verify chat + handouts still visible', async ({ browser }) => {
    test.setTimeout(30 * 60 * 1000);
    ...
    try {
      ...
      await expect(playerNewPage.getByText(/HANDOUTS & LORE/i).first()).toBeVisible({ timeout: 20_000 });
      await expect(playerNewPage.getByText(/Soak Handout/i).first()).toBeVisible({ timeout: 20_000 });
    } finally {
