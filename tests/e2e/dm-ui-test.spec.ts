...
test.describe('Focused DM UI Test', () => {
  ...
  test('Complete DM UI Interaction Journey', async ({ page }) => {
    ...
    const loginButton = page.locator('button').filter({ hasText: /login|sign in|get started/i }).first();
    ...
    const dmRoleButton = page.locator('button').filter({ hasText: /protocol warden|dm|dungeon master/i }).first();
    if (await dmRoleButton.isVisible()) {
      await dmRoleButton.click();
    }

    // Fill credentials if we find email/password fields
    ...
    // Submit if we find a submit button
    ...
    for (const containerSelector of toolsContainers) {
      try {
        ...
        if (await container.isVisible()) {
          ...
          const toolElements = await container.locator('button, a, [role="button"]').count();
          if (toolElements > 0) {
            ...
            for (let i = 0; i < toolsToTest; i++) {
              try {
                await container.locator('button, a, [role="button"]').nth(i).click();
      ...
      try {
        ...
        if (pageTitle && !pageTitle.includes('404') && !pageTitle.includes('Error')) {
          ...
          try {
            ...
            console.log(`  Interaction test failed: ${(error as Error).message}`);
          }
    ...
    try {
      ...
      const categoryButtons = page.locator('button:has-text("spells"), button:has-text("monsters"), button:has-text("items")');
    ...
    try {
      ...
      const errorElements = page.locator('h1:has-text("404"), h1:has-text("Error"), [data-testid*="error"]');
