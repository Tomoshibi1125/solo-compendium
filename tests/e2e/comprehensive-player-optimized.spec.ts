...
test.describe('Optimized Player Complete Test', () => {
  ...
  async function safeNavigate(url: string, description: string): Promise<boolean> {
    try {
      ...
      console.log(`❌ Failed to navigate to ${description}: ${(error as Error).message}`);
      return false;
    }
  }

  // Helper function for safe element interaction
  async function safeInteraction(selector: string, action: string, description: string): Promise<boolean> {
    try {
    ...
    try {
      ...
      if (await safeNavigate('http://localhost:8080', 'Landing Page')) {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            if (currentUrl.includes('/player-tools') || currentUrl.includes('/characters') || currentUrl !== 'http://localhost:8080/') {
              authSuccess = true;
            ...
            console.log(`❌ Auth attempt ${attempt} failed: ${(error as Error).message}`);
            if (attempt < 3) {
              await page.waitForTimeout(2000);
            }
          }
      ...
      for (const navTest of navigationTests) {
        try {
          ...
          if (await nav.isVisible({ timeout: 5000 })) {
            ...
            if (linkCount > 0) {
              ...
              for (let i = 0; i < Math.min(3, linkCount); i++) {
                await safeInteraction(`nth=${i}`, 'hover', `Navigation link ${i + 1}`);
              }
          ...
          console.log(`⚠️ ${navTest.name} test failed: ${(error as Error).message}`);
        }
      }
      ...
      if (await safeNavigate('http://localhost:8080/player-tools', 'Player Tools Dashboard')) {
        ...
        for (const tool of playerTools) {
          if (await safeNavigate(`http://localhost:8080${tool.path}`, tool.name)) {
            ...
            await page.goto('http://localhost:8080/player-tools', { timeout: 15000 });
        ...
        if (toolsTested >= 2) {
          ...
          console.log(`✅ Player Tools tested: ${toolsTested}/${playerTools.length}`);
        }
      }
