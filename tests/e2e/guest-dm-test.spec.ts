	...
	test("DM Features as Guest User", async ({ page }) => {
		...
		try {
			...
			try {
				...
				if (hasContent) {
					...
					const errorElements = page.locator(
						'[data-testid*="error"], .error, .login-required, .auth-required',
					);
					const hasError = (await errorElements.count()) > 0;

					if (hasError || currentUrl === "http://localhost:8080/") {
						console.log(
							"✅ DM tools properly restricted (redirected to home or error shown)",
						);
						testResults.dmToolsAccess = true;
					} else {
						// Check if any DM tools are visible (guest access)
						const dmToolElements = page.locator(
							'[data-testid*="tool"], .dm-tool, .tool-card',
						);
			...
			for (const pageTest of publicPages) {
				try {
					await page.goto(`http://localhost:8080${pageTest.path}`, {
						timeout: 15000,
					});
					...
					console.log(
						`❌ ${pageTest.name} page failed: ${(error as Error).message}`,
					);
				}
			}
			...
			if (publicFeaturesWorking >= 2) {
				...
				console.log(
					`✅ ${publicFeaturesWorking}/${publicPages.length} public features working`,
				);
			}
			...
			if (errorHandlingWorking >= 2) {
				...
				console.log(
					`✅ Error handling working: ${errorHandlingWorking}/${invalidUrls.length} tests`,
				);
			}
			...
			for (const viewport of viewports) {
				try {
					...
					if (isVisible) {
						...
						if (viewport.width <= 768) {
							const mobileMenu = page.locator(
								'[data-testid*="menu"], .hamburger, .mobile-menu, button:has-text("menu")',
							);
					...
					console.log(
						`❌ ${viewport.name} viewport failed: ${(error as Error).message}`,
					);
				}
			}
			...
			if (responsiveWorking >= 2) {
				...
				console.log(
					`✅ UI responsiveness working: ${responsiveWorking}/${viewports.length} viewports`,
				);
			}
