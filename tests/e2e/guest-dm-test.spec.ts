    ...
    try {
      ...
      try {
        ...
        if (hasContent) {
          ...
          const loginElements = page.locator('[data-testid*="login"], .login, .sign-in');
          
          const hasError = await errorElements.count() > 0;
          const hasLogin = await loginElements.count() > 0;
          
          if (hasError) {
            console.log('✅ DM tools properly restricted (error message shown)');
            testResults.dmToolsAccess = true;
          } else if (hasLogin) {
            console.log('✅ DM tools properly restricted (login required)');
            testResults.dmToolsAccess = true;
          } else {
