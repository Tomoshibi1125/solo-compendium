...
test.describe("Comprehensive DM UI/UX Test", () => {
	...
	test("DM Complete UI/UX Journey - All Possible Interactions", async () => {
		...
		await expect(
			page.locator('[data-testid="dashboard-welcome"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-testid="campaigns-section"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-testid="dm-tools-section"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-testid="compendium-section"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-testid="player-tools-section"]'),
		).toBeVisible();
		...
		await page.fill(
			...
			"Testing all DM UI interactions",
		);
		...
		for (const tab of campaignTabs) {
			...
			await expect(
				page.locator(`[data-testid="${tab}-tab-content"]`),
			).toBeVisible();
		}
		...
		await expect(
			page.locator('[data-testid="encounter-builder"]'),
		).toBeVisible();
		...
		await expect(
			page.locator('[data-testid="saved-encounters"]'),
		).toBeVisible();
		...
		await expect(
			page.locator('[data-testid="initiative-tracker"]'),
		).toBeVisible();
		...
		for (const tool of vttTools) {
			...
			await expect(
				page.locator(`[data-testid="vtt-tool-${tool}"]`),
			).toHaveClass(/active/);
		}
		...
		await expect(
			page.locator('[data-testid="asset-thumbnails"]'),
		).toBeVisible();
		...
		for (const die of diceTypes) {
			...
			await expect(page.locator(`[data-testid="die-${die}"]`)).toHaveClass(
				/selected/,
			);
		}
		...
		await expect(
			page.locator('[data-testid="campaign-options"]'),
		).toBeVisible();
		...
		await expect(
			page.locator('[data-testid="ai-enhancement-loading"]'),
		).toBeVisible();
		...
		await expect(
			page.locator('[data-testid="quest-objectives"]'),
		).toBeVisible();
		...
		await expect(
			page.locator('[data-testid="treasure-generator"]'),
		).toBeVisible();
		...
		await expect(
			page.locator('[data-testid="relic-properties"]'),
		).toBeVisible();
		...
		await expect(
			page.locator('[data-testid="compendium-categories"]'),
		).toBeVisible();
		...
		for (const category of categories) {
			...
			// Test detail view
			if (
				await page.locator(`[data-testid="${category}-item-0"]`).isVisible()
			) {
		...
		await expect(
			page.locator('[data-testid="nav-player-tools"]'),
		).toBeVisible();
