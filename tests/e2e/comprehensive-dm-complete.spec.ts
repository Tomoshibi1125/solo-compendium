  ...
  test('DM Complete UI/UX Journey - Every Possible Interaction', async () => {
    ...
    try {
      ...
      try {
        ...
        for (let i = 0; i < roleCount; i++) {
          const buttonText = await roleButtons.nth(i).textContent();
          if (buttonText?.toLowerCase().includes('dm') || buttonText?.toLowerCase().includes('protocol')) {
        ...
        if (await emailInput.isVisible()) {
          await emailInput.fill(DM_EMAIL);
        }
        if (await passwordInput.isVisible()) {
          await passwordInput.fill(DM_PASSWORD);
        }
        ...
        console.log(`  ❌ Alternative auth failed: ${(altError as Error).message}`);
      }
    }
        ...
        if (await nav.isVisible()) {
          ...
          for (let i = 0; i < Math.min(linkCount, 10); i++) {
            try {
              ...
              console.log(`    Link test failed: ${(linkError as Error).message}`);
            }
          }
          break;
        }
    ...
    for (const url of campaignUrls) {
      try {
            ...
            try {
              ...
              if (await createButton.isVisible()) {
                ...
                for (let i = 0; i < Math.min(inputCount, 5); i++) {
                  ...
                  } else if (await input.evaluate(el => el.tagName) === 'textarea') {
              ...
              console.log(`    Campaign form test failed: ${(formError as Error).message}`);
            }
            break;
          }
        }
        ...
        console.log(`  Campaign page ${url} failed: ${(error as Error).message}`);
      }
    }
    ...
    for (const toolUrl of dmToolUrls) {
      ...
      try {
        ...
        if (pageTitle && !pageTitle.includes('404') && !pageTitle.includes('Error') && hasContent.length > 100) {
    ...
    try {
      ...
      const categories = ['spells', 'monsters', 'items', 'feats', 'backgrounds', 'jobs', 'paths', 'runes', 'relics', 'conditions', 'locations', 'techniques'];
      ...
      for (const category of categories) {
        ...
        try {
          ...
          const categoryButton = page.locator(`button:has-text("${category}"), a:has-text("${category}")`).first();
          ...
          // Test detail views
          const itemElements = page.locator('button, a, [role="button"]').filter({ hasText: /\w+/ });
          ...
          console.log(`    ❌ ${category} category failed: ${(categoryError as Error).message}`);
        }
      }
    ...
    try {
      ...
      // Test VTT canvas
      const vttCanvas = page.locator('canvas, [data-testid*="vtt-canvas"], .vtt-canvas').first();
      if (await vttCanvas.isVisible()) {
        console.log('  ✅ VTT canvas found');
        
        // Test all VTT tools
        const vttTools = ['select', 'pan', 'draw', 'fog', 'measure', 'token', 'effect'];
        
        for (const tool of vttTools) {
          try {
            const toolButton = page.locator(`button:has-text("${tool}"), [data-testid*="${tool}"], .tool-${tool}`).first();
            ...
            console.log(`    ❌ VTT tool ${tool} failed: ${(toolError as Error).message}`);
          }
        }
        ...
        for (const panel of vttPanels) {
          try {
            const panelTab = page.locator(`button:has-text("${panel}"), [data-testid*="${panel}"]`).first();
            ...
            console.log(`    ❌ VTT panel ${panel} failed: ${(panelError as Error).message}`);
          }
        }
    ...
    for (const viewport of viewports) {
      ...
      try {
        ...
        const navigationVisible = await page.locator('nav, [data-testid*="nav"]').isVisible();
        console.log(`    Navigation visible: ${navigationVisible}`);
        
        // Test mobile menu if applicable
        if (viewport.width <= 768) {
          const mobileMenu = page.locator('button[aria-label*="menu"], button:has-text("menu"), .hamburger').first();
          if (await mobileMenu.isVisible()) {
            await mobileMenu.click();
            await page.waitForTimeout(1000);
            
            const mobileNavVisible = await page.locator('.mobile-menu, [data-testid*="mobile-menu"]').isVisible();
        ...
        console.log(`    ❌ ${viewport.name} viewport failed: ${(viewportError as Error).message}`);
      }
    }
    ...
    const interactiveElements = page.locator('button, a, input, select, textarea, [role="button"], [tabindex]');
    ...
    for (let i = 0; i < Math.min(elementCount, 20); i++) {
      try {
        ...
        console.log(`    Element ${i} focus failed: ${(focusError as Error).message}`);
      }
    }
    
    // Test ARIA attributes
    console.log('  Testing ARIA attributes...');
    const ariaElements = page.locator('[aria-label], [role], [aria-expanded], [aria-describedby], [aria-hidden]');
    ...
    const contrastIssues = await page.evaluate(() => {
      ...
      elements.forEach(el => {
        ...
        if (color !== 'rgba(0, 0, 0, 0)' && bg !== 'rgba(0, 0, 0, 0)') {
          ...
          if (colorRgb && bgRgb) {
            const contrast = Math.abs(parseInt(colorRgb[0]) - parseInt(bgRgb[0])) +
    ...
    try {
      ...
      // Test real-time chat
      const chatInput = page.locator('input[type="text"], textarea[placeholder*="message"], [data-testid*="chat-input"]').first();
      if (await chatInput.isVisible()) {
        await chatInput.fill('Test message for real-time testing');
        await page.waitForTimeout(1000);
        
        const sendButton = page.locator('button:has-text("send"), button:has-text("chat")').first();
      ...
      const diceButtons = page.locator('button:has-text("d20"), button:has-text("roll")');
      ...
      console.log(`  ❌ Real-time features failed: ${(error as Error).message}`);
    }
    ...
    for (const invalidUrl of invalidUrls) {
      try {
        ...
        const errorElements = page.locator('h1:has-text("404"), h1:has-text("Error"), [data-testid*="error"], .error-page');
        const errorVisible = await errorElements.first().isVisible();
        
        console.log(`    ${invalidUrl} error page: ${errorVisible ? '✅' : '❌'}`);
        
        // Test recovery options
        const backButton = page.locator('button:has-text("back"), a:has-text("home")').first();
        ...
        console.log(`    ${invalidUrl} test failed: ${(error as Error).message}`);
      }
    }
    ...
    try {
      ...
      if (await saveButton.isVisible()) {
        ...
        const validationMessages = page.locator('.error, [data-testid*="error"], .validation-message');
        const messageCount = await validationMessages.count();
        console.log(`    Validation messages shown: ${messageCount}`);
      }
      
    } catch (error) {
      console.log(`    Form validation test failed: ${(error as Error).message}`);
    }
    ...
    for (const url of performanceUrls) {
      try {
        ...
        console.log(`    ${url} performance test failed: ${(error as Error).message}`);
      }
    }
    ...
    try {
      ...
      const aiButton = page.locator('button:has-text("AI"), button:has-text("enhance"), [data-testid*="ai"]').first();
    ...
    try {
      ...
      console.log(`  ❌ Clipboard integration failed: ${(error as Error).message}`);
    }
    ...
    const passedTests = Object.values(testResults).filter(value => typeof value === 'boolean' ? value : Object.values(value).some(Boolean)).length;
    ...
    console.log(`  Authentication: ${typeof testResults.authentication === 'boolean' ? (testResults.authentication ? '✅' : '❌') : '❌'}`);
    console.log(`  Dashboard: ${typeof testResults.dashboard === 'boolean' ? (testResults.dashboard ? '✅' : '❌') : '❌'}`);
    console.log(`  Campaigns: ${typeof testResults.campaigns === 'boolean' ? (testResults.campaigns ? '✅' : '❌') : '❌'}`);
    console.log(`  Compendium: ${typeof testResults.compendium === 'boolean' ? (testResults.compendium ? '✅' : '❌') : '❌'}`);
    console.log(`  VTT: ${typeof testResults.vtt === 'boolean' ? (testResults.vtt ? '✅' : '❌') : '❌'}`);
    console.log(`  Responsive: ${typeof testResults.responsive === 'boolean' ? (testResults.responsive ? '✅' : '❌') : '❌'}`);
    console.log(`  Accessibility: ${typeof testResults.accessibility === 'boolean' ? (testResults.accessibility ? '✅' : '❌') : '❌'}`);
    console.log(`  Real-time: ${typeof testResults.realTime === 'boolean' ? (testResults.realTime ? '✅' : '❌') : '❌'}`);
    console.log(`  Error Handling: ${typeof testResults.errorHandling === 'boolean' ? (testResults.errorHandling ? '✅' : '❌') : '❌'}`);
    console.log(`  Performance: ${typeof testResults.performance === 'boolean' ? (testResults.performance ? '✅' : '❌') : '❌'}`);
    console.log(`  Integrations: ${typeof testResults.integrations === 'boolean' ? (testResults.integrations ? '✅' : '❌') : '❌'}`);
    
    console.log('');
    console.log('DM Tools Results:');
    const workingTools = Object.entries(testResults.dmTools as Record<string, boolean>).filter(([_, working]) => working).length;
    const totalTools = Object.keys(testResults.dmTools as Record<string, boolean>).length;
    console.log(`  Working Tools: ${workingTools}/${totalTools} (${Math.round((workingTools / totalTools) * 100)}%)`);
    
    Object.entries(testResults.dmTools as Record<string, boolean>).forEach(([tool, working]) => {
 ...
 */
async function testPageInteractions(page: Page, pageName: string): Promise<void> {
  ...
  for (let i = 0; i < buttonsToTest; i++) {
    try {
      ...
      if (isVisible && buttonText && !buttonText.includes('delete') && !buttonText.includes('remove')) {
        await buttons.nth(i).click();
        await page.waitForTimeout(500);
        
        // Check for modals or overlays
        const modal = page.locator('.modal, [role="dialog"], [data-testid*="modal"]').first();
 ...
 */
async function testResponsiveDesign(page: Page, pageName: string): Promise<void> {
  ...
  for (const viewport of viewports) {
    ...
    const navigationAdapted = await page.locator('.mobile-menu, .hamburger, [data-testid*="mobile"]').isVisible();
    
    console.log(`      ${viewport.width}x${viewport.height}: Scroll=${hasHorizontalScroll}, Mobile=${navigationAdapted}`);
  }
 ...
 */
async function testAccessibilityFeatures(page: Page, pageName: string): Promise<void> {
