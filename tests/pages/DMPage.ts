...
export class DMPage {
  ...
  async createDetailedCampaign(opts: {
    ...
    if (opts.settings?.difficulty) {
      const difficultySelect = this.page.locator('#campaign-difficulty, #difficulty').first();
      if (await difficultySelect.isVisible({ timeout: 3_000 }).catch(() => false)) {
        await difficultySelect.click();
        await this.page.getByRole('option', { name: new RegExp(opts.settings.difficulty, 'i') }).click();
      }
    }

    // Fill player level
    if (opts.settings?.playerLevel) {
      const levelInput = this.page.locator('#starting-level, #player-level').first();
    ...
    if (opts.settings?.maxPlayers) {
      const maxPlayersInput = this.page.locator('#max-players, #player-limit').first();
      if (await maxPlayersInput.isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    if (opts.settings?.houseRules) {
      const houseRulesInput = this.page.locator('#house-rules, #custom-rules').first();
      if (await houseRulesInput.isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    // Submit
    await this.page.getByRole('button', { name: /Establish Guild|Create Campaign/i }).click();
  ...
  async createSession(opts: {
    ...
    const sessionCreateBtn = this.page.getByRole('button', { name: /Create Session|New Session|Start Session/i }).first();

    if (await sessionCreateBtn.isVisible({ timeout: 10_000 }).catch(() => false)) {
    ...
    const descInput = this.page.locator('#session-description, #description').first();
    ...
    const saveBtn = this.page.getByRole('button', { name: /Create Session|Save Session|Start/i }).first();
    ...
    const url = new URL(this.page.url());
    const sessionIdMatch = url.pathname.match(/\/sessions\/([a-z0-9-]+)/i) || url.pathname.match(/\/campaigns\/[^\/]+\/sessions\/([a-z0-9-]+)/i);
  ...
  private async configureSessionMap(map: {
    ...
    // Map type selection
    if (map.type) {
      const mapTypeSelect = this.page.locator('#map-type, #dungeon-type').first();
      if (await mapTypeSelect.isVisible({ timeout: 3_000 }).catch(() => false)) {
        await mapTypeSelect.click();
        await this.page.getByRole('option', { name: new RegExp(map.type, 'i') }).click();
      }
    }
    ...
    if (map.size) {
      ...
      if (await sizeSelect.isVisible({ timeout: 3_000 }).catch(() => false)) {
        ...
        await this.page.getByRole('option', { name: new RegExp(map.size, 'i') }).click();
      }
    }
    ...
    if (map.features) {
      for (const feature of map.features) {
        const featureCheckbox = this.page.getByRole('checkbox', { name: new RegExp(feature, 'i') }).first();
        if (await featureCheckbox.isVisible({ timeout: 2_000 }).catch(() => false)) {
  ...
  private async configureSessionCombat(combat: {
    ...
    if (combat.initiative) {
      const initiativeSelect = this.page.locator('#initiative-system, #combat-initiative').first();
      if (await initiativeSelect.isVisible({ timeout: 3_000 }).catch(() => false)) {
        await initiativeSelect.click();
        await this.page.getByRole('option', { name: new RegExp(combat.initiative, 'i') }).click();
      }
    }
    ...
    if (combat.encounters && combat.encounters.length > 0) {
      ...
      const addEncounterBtn = this.page.getByRole('button', { name: /Add Encounter|New Encounter/i }).first();
      if (await addEncounterBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
        ...
        // Select difficulty
        const difficultySelect = this.page.locator('#encounter-difficulty').last();
        if (await difficultySelect.isVisible({ timeout: 2_000 }).catch(() => false)) {
          await difficultySelect.click();
          await this.page.getByRole('option', { name: new RegExp(encounter.difficulty, 'i') }).click();
        }
      }
    }
  }
   ...
   */
  private async configureSessionQuests(quests: Array<{
    title: string;
    description: string;
    objectives: string[];
    rewards: string[];
    ...
    if (quests.length > 0) {
      ...
      const addQuestBtn = this.page.getByRole('button', { name: /Add Quest|New Quest/i }).first();
  ...
  private async configureSessionRewards(rewards: {
    ...
    if (rewards.gold) {
      const goldInput = this.page.locator('#session-gold, #gold-reward').first();
    ...
    if (rewards.items && rewards.items.length > 0) {
      const itemsInput = this.page.locator('#session-items, #item-rewards').first();
   ...
   */
  async initiateCombat(sessionId: string, opts: {
    ...
    const combatBtn = this.page.getByRole('button', { name: /Start Combat|Initiate Combat|Begin Fight/i }).first();
    ...
    for (const combatant of opts.combatants) {
      const addCombatantBtn = this.page.getByRole('button', { name: /Add Combatant|New Combatant/i }).first();
      if (await addCombatantBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
        ...
        const initiativeInput = this.page.locator('#combatant-initiative').last();
        if (await initiativeInput.isVisible({ timeout: 2_000 }).catch(() => false)) {
        ...
        if (combatant.type === 'monster') {
          ...
          if (await typeSelect.isVisible({ timeout: 2_000 }).catch(() => false)) {
            ...
            await this.page.getByRole('option', { name: /Monster|NPC|Enemy/i }).click();
          }
        }
      }
    }
    ...
    const beginCombatBtn = this.page.getByRole('button', { name: /Begin Combat|Start Initiative|Roll Initiative/i }).first();
   ...
   */
  async executeCombatRound(sessionId: string, roundNumber: number): Promise<void> {
    ...
    if (await nextTurnBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      const altNextBtn = this.page.getByRole('button', { name: /Next Turn|Next|Advance/i }).first();
   ...
   */
  async performMonsterAction(sessionId: string, opts: {
    monster: string;
    action: string;
    target: string;
    description: string;
    ...
    const monsterRow = this.page.getByText(opts.monster, { exact: false }).first();
    ...
    const actionBtn = this.page.getByRole('button', { name: new RegExp(opts.action, 'i') }).first();
    if (await actionBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await actionBtn.click();
    }

    // Log action description
    const actionLog = this.page.locator('#combat-log, #action-log, textarea').first();
   ...
   */
  async rollForMonster(sessionId: string, rollType: string, modifier: number): Promise<void> {
    // Use dice roller for monster rolls
    const rollBtn = this.page.getByRole('button', { name: new RegExp(`${rollType}|Roll.*${rollType}`, 'i') }).first();
   ...
   */
  async continueCombatUntilResolution(sessionId: string): Promise<'victory' | 'defeat' | 'ongoing'> {
    // Monitor combat state - look for victory/defeat indicators
    const victoryIndicator = this.page.getByText(/Victory|Defeated|Won|Success/i).first();
    const defeatIndicator = this.page.getByText(/Defeat|Defeated|Lost|Failure/i).first();
    ...
    while (rounds < maxRounds) {
      ...
      const hasVictory = await victoryIndicator.isVisible({ timeout: 1_000 }).catch(() => false);
      const hasDefeat = await defeatIndicator.isVisible({ timeout: 1_000 }).catch(() => false);
   ...
   */
  async completeQuest(sessionId: string, opts: {
    questId: string;
    completion: string;
    rewards: {
      xp?: number;
    ...
    const questElement = this.page.getByText(opts.questId, { exact: false }).first();
    if (await questElement.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await questElement.click();
    }

    // Mark as complete
    const completeBtn = this.page.getByRole('button', { name: /Complete Quest|Mark Complete|Finish/i }).first();
    if (await completeBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await completeBtn.click();
    }

    // Add completion notes
    const notesInput = this.page.locator('#quest-notes, #completion-notes, textarea').first();
    if (await notesInput.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await notesInput.fill(opts.completion);
    }

    // Confirm rewards distribution
    const distributeBtn = this.page.getByRole('button', { name: /Distribute Rewards|Award|Give/i }).first();
   ...
   */
  async endSession(sessionId: string, summary: {
    summary: string;
    notes: string;
    nextSession?: string;
  }): Promise<void> {
    // End session
    const endBtn = this.page.getByRole('button', { name: /End Session|Finish|Conclude/i }).first();
    ...
    const summaryInput = this.page.locator('#session-summary, #summary, textarea').first();
    if (await summaryInput.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await summaryInput.fill(summary.summary);
    }

    // Fill session notes
    const notesInput = this.page.locator('#session-notes, #notes, textarea').first();
    if (await notesInput.isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    if (summary.nextSession) {
      const nextSessionInput = this.page.locator('#next-session, #plans, textarea').last();
      if (await nextSessionInput.isVisible({ timeout: 3_000 }).catch(() => false)) {
        await nextSessionInput.fill(summary.nextSession);
      }
    }

    // Save and finalize
    const saveBtn = this.page.getByRole('button', { name: /Save Session|Finalize|Complete/i }).first();
    if (await saveBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await saveBtn.click();
    }
  }
  ...
  async gotoCampaignDetail(campaignId: string) {
    try {
      ...
      await this.page.goto(`/campaigns/${campaignId}`, { waitUntil: 'domcontentloaded' });
      ...
      if (!currentUrl.includes(`/campaigns/${campaignId}`)) {
        throw new Error(`Navigation failed - expected /campaigns/${campaignId}, got ${currentUrl}`);
      }
