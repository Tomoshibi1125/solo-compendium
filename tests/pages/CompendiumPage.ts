...
export class CompendiumPage {
  ...
  async verifyEntriesVisible() {
    const entry = this.page.locator('.compendium-entry, [data-testid="compendium-item"]').first();
  ...
  async verifyDetailLoads() {
    await expect(this.page.locator('.compendium-detail, [data-testid="compendium-detail"]')).toBeVisible({ timeout: 10_000 });
  }
  ...
  async deepExercise() {
    ...
    for (const cat of categories) {
      const btn = this.page.getByRole('button', { name: new RegExp(cat, 'i') }).first();
