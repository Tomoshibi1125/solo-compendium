...
export class DMToolsPage {
  ...
  async verifyHub() {
    ...
    await expect(this.page.getByTestId('dm-tools')).toBeVisible({ timeout: 15_000 });
  ...
  async testEncounterBuilder() {
    ...
    // Page heading
    await expect(this.page.getByText('ENCOUNTER BUILDER', { exact: false }).first())
    ...
    // Back button
    await expect(this.page.getByText('Back to System Tools', { exact: false }).first()).toBeVisible();
  }
  ...
  async testInitiativeTracker() {
    ...
    await expect(this.page.getByTestId('initiative-tracker'))
      .toBeVisible({ timeout: 15_000 });
    ...
    await expect(this.page.getByText('Test Combatant', { exact: true })).toBeVisible({ timeout: 5_000 });
  ...
  async testRollableTables() {
    ...
    await expect(this.page.getByText('PROTOCOL WARDEN TABLES', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });

    // Verify tabs
    await expect(this.page.getByRole('tab', { name: /Rifts/i })).toBeVisible();
    await expect(this.page.getByRole('tab', { name: /Rewards/i })).toBeVisible();
    await expect(this.page.getByRole('tab', { name: /NPCs/i })).toBeVisible();
    await expect(this.page.getByRole('tab', { name: /Treasure/i })).toBeVisible();

    // Roll on Rifts tab (default)
    const rollComplicationBtn = this.page.getByRole('button', { name: /Roll Complication/i });
    await expect(rollComplicationBtn).toBeVisible({ timeout: 5_000 });
    await rollComplicationBtn.click();
    await expect(this.page.getByText('Complication').first()).toBeVisible({ timeout: 5_000 });

    // Switch to Rewards tab
    await this.page.getByRole('tab', { name: /Rewards/i }).click();
    const rollRewardBtn = this.page.getByRole('button', { name: /Roll Reward/i });
    await expect(rollRewardBtn).toBeVisible({ timeout: 5_000 });
    await rollRewardBtn.click();
    await expect(this.page.getByText('Reward').first()).toBeVisible({ timeout: 5_000 });

    // Switch to NPCs tab
    await this.page.getByRole('tab', { name: /NPCs/i }).click();
    const rollMotivationBtn = this.page.getByRole('button', { name: /Roll Motivation/i });
    ...
    await expect(this.page.getByText('TREASURE BY RIFT RANK', { exact: false }).first())
  ...
  async testRiftGenerator() {
    ...
    await expect(this.page.getByText('RIFT GENERATOR', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });

    // Generate button
    const generateBtn = this.page.getByRole('button', { name: /Generate/i }).first();
  ...
  async testNPCGenerator() {
    ...
    await expect(this.page.getByText('NPC GENERATOR', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
  ...
  async testTreasureGenerator() {
    ...
    await expect(this.page.getByText('TREASURE GENERATOR', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
  ...
  async testQuestGenerator() {
    ...
    await expect(this.page.getByText('QUEST GENERATOR', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
  ...
  async testSessionPlanner() {
    ...
    await expect(this.page.getByText('SESSION PLANNER', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
    ...
    const campaignSelect = this.page.locator('#session-planner-campaign').first();
    const noCampaigns = this.page.getByText(/NO CAMPAIGNS|Create or join/i).first();
    const loading = this.page.getByText(/Loading campaigns/i).first();
    const hasCampaignSelect = await campaignSelect.isVisible({ timeout: 8_000 }).catch(() => false);
    const hasNoCampaigns = await noCampaigns.isVisible({ timeout: 3_000 }).catch(() => false);
  ...
  async testRandomEventGenerator() {
    ...
    await expect(this.page.getByText('RANDOM EVENT', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });

    // Generate world event
    const worldBtn = this.page.getByRole('button', { name: /World Event|Generate World/i }).first();
    ...
    const npcBtn = this.page.getByRole('button', { name: /NPC|Generate NPC/i }).first();
  ...
  async testRelicWorkshop() {
    ...
    await expect(this.page.getByText('RELIC WORKSHOP', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
    ...
    if (await textarea.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await textarea.fill('A powerful test relic forged in the crucible of E2E testing.');
    }
    ...
    // Add property button
    const addPropertyBtn = this.page.getByRole('button', { name: /Add Property/i }).first();
  ...
  async testPartyTracker() {
    ...
    await expect(this.page.getByText('PARTY TRACKER', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });

    // Add member button
    const addMemberBtn = this.page.getByRole('button', { name: /Add|New Member/i }).first();
  ...
  async testDungeonMapGenerator() {
    ...
    await expect(this.page.getByText('DUNGEON MAP', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
  ...
  async testTokenLibrary() {
    ...
    await expect(this.page.getByText('TOKEN LIBRARY', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
    ...
    // Create token button
    const createBtn = this.page.getByRole('button', { name: /Create|New Token/i }).first();
    ...
    // Category filter
    const categories = this.page.locator('button').filter({ hasText: /All|Custom|Monsters|NPCs/i });
  ...
  async testArtGenerator() {
    ...
    await expect(this.page.getByText('ART GENERATOR', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
  ...
  async testAudioManager() {
    ...
    await expect(this.page.getByText('AUDIO', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });
  ...
  async testVTTMap() {
    ...
    await expect(this.page.getByText('VTT', { exact: false }).first())
      .toBeVisible({ timeout: 15_000 });

    // Zoom buttons
    const zoomInBtn = this.page.getByRole('button', { name: /Zoom In|\+/i }).first();
  ...
  async testContentAudit() {
    ...
    await this.page.waitForTimeout(2_000);
    const heading = this.page.getByText(/CONTENT AUDIT|AUDIT|DATABASE/i).first();
    await expect(heading).toBeVisible({ timeout: 15_000 });
  }
  ...
  async testVTTEnhanced(campaignId: string) {
    ...
    const heading = this.page.getByText(/VTT|VIRTUAL TABLETOP|MAP|TOKEN/i).first();
    const errorBoundary = this.page.getByText(/Connection Error|Error|Try Again|Reload/i).first();
    const headingVisible = await heading.isVisible({ timeout: 10_000 }).catch(() => false);
    const errorVisible = await errorBoundary.isVisible({ timeout: 5_000 }).catch(() => false);
   ...
   */
  async setupSessionMap(sessionId: string, opts: {
    gridSize?: number;
    tokens?: string[];
    fogOfWar?: boolean;
    ...
    if (opts.gridSize) {
      const gridSizeInput = this.page.locator('#grid-size, input[name*="grid"]').first();
      if (await gridSizeInput.isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    if (opts.tokens && opts.tokens.length > 0) {
      for (const token of opts.tokens) {
        const addTokenBtn = this.page.getByRole('button', { name: /Add Token|New Token/i }).first();
        if (await addTokenBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
          await addTokenBtn.click();

          // Select token type
          const tokenSelect = this.page.locator('#token-type, select[name*="token"]').last();
          if (await tokenSelect.isVisible({ timeout: 2_000 }).catch(() => false)) {
            await tokenSelect.click();
            await this.page.getByRole('option', { name: new RegExp(token, 'i') }).click();
          }
        }
      }
    }
    ...
    if (opts.fogOfWar !== undefined) {
      const fogToggle = this.page.getByRole('checkbox', { name: /Fog|Fog of War/i }).first();
    ...
    if (opts.dynamicLighting !== undefined) {
      const lightingToggle = this.page.getByRole('checkbox', { name: /Lighting|Dynamic Lighting/i }).first();
      if (await lightingToggle.isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    const saveBtn = this.page.getByRole('button', { name: /Save|Apply/i }).first();
