...
const dupes = ['Harmonic Shield', 'Shadow Step', 'Perfect Kill', 'Nerve Strike', 'Pressure Point Cascade', "Predator's Mark"];
const files = {
...
for (const d of dupes) {
  const locs = [];
  for (const [k, f] of Object.entries(files)) { if (findName(f, d)) locs.push(k); }
