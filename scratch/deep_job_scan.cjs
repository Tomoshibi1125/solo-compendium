...
for (const block of jobBlocks) {
  ...
  const hasFightingStyle = block.includes('Fighting Style') || block.includes('fighting-style');
  ...
  const spellAbilityM = block.match(/spellcasting:\s*\{[^}]*ability:\s*["'`]([^"'`]+)["'`]/s);

  // powersKnown max
  const powersKnownM = block.match(/powersKnown:\s*\[([^\]]+)\]/);
  let maxPowers = 0;
  if (powersKnownM) {
    maxPowers = Math.max(...powersKnownM[1].split(',').map(v => parseInt(v.trim())));
  }
  const techniquesKnownM = block.match(/techniquesKnown:\s*\[([^\]]+)\]/);
  let maxTech = 0;
  if (techniquesKnownM) {
    maxTech = Math.max(...techniquesKnownM[1].split(',').map(v => parseInt(v.trim())));
  }
  ...
  console.log(`${name.padEnd(16)} | ${(hitDieM?hitDieM[1]:'?').padEnd(4)} | primary=${(primaryM?primaryM[1]:'?').padEnd(14)} | saves=${savesM?savesM[1].replace(/"/g,'').trim():''}`);
  console.log(`${''.padEnd(16)} | spells=${hasSpellcasting?'YES':'no '} (${spellAbility.padEnd(13)}) | powers=${hasPowersKnown?`YES(max ${maxPowers})`:' no      '} | tech=${hasTechniquesKnown?`YES(max ${maxTech})`:' no      '} | fightStyle=${hasFightingStyle?'YES':'no '} | spellbook=${hasSpellbook?'YES':'no '}`);
