...
function abilityForClasses(cls) {
	...
	if (
		cls.includes("Holy Knight") ||
		cls.includes("Idol") ||
		cls.includes("Contractor")
	)
		return "Presence";
...
function diceForLevel(lv) {
	return (
		["", "2d6", "3d6", "4d6", "5d6", "6d6", "7d6", "8d6", "9d6", "10d6"][lv] ||
		"4d6"
	);
}
...
function higherForLevel(lv, dt) {
	if (lv >= 7)
		return `At 9th level, damage increases by 2d6 and the rider effect's duration doubles.`;
	if (lv >= 5)
		return `Damage increases by 1d6 per level above ${lv}th. At ${lv + 2}th level, the secondary effect also intensifies.`;
...
L.push(`// ═══════════════════════════════════════════════════════════════`);
L.push(
	`// ${allPowers.length} Unique Powers — Fully Fleshed, Zero Template Recycling`,
);
...
for (const p of allPowers) {
	...
	const hasSave =
		!!p.save || ["guard", "overload", "radiant", "tracking"].includes(p.mode);
	...
	L.push(
		`\t\ttags: ${JSON.stringify(["awakened", "power", p.mode, p.sch, ...p.cls])},`,
	);
	...
	L.push(
		`\t\teffects: { primary: ${JSON.stringify(eff)}, secondary: ${JSON.stringify(higher)} },`,
	);
	...
	L.push(
		`\t\tduration: ${p.lv >= 5 ? '"Concentration, up to 1 minute"' : '"Instantaneous"'},`,
	);
	...
	L.push(
		`\t\t\tlattice_interaction: ${JSON.stringify(`${p.lv}-tier ${p.mode} stabilized through gate-lattice resonance`)},`,
	);
	if (hasAttack) {
		L.push(
			`\t\t\tattack: { type: ${JSON.stringify(dt)}, mode: ${JSON.stringify(isMelee ? "melee" : "ranged")}, resolution: "martial_attack", modifier: ${JSON.stringify(ability)}, damage: ${JSON.stringify(dice)}, damage_type: ${JSON.stringify(dt)} },`,
		);
	}
	L.push(
	...
	L.push(`\t\tlimitations: {`);
	L.push(
		`\t\t\tuses: ${JSON.stringify(p.lv >= 7 ? "1/long rest" : p.lv >= 5 ? "2/long rest" : "3/long rest")},`,
	);
...
console.log(
	`Wrote ${allPowers.length} powers (${(output.length / 1024).toFixed(1)} KB)`,
);
