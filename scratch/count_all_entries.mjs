...
function countSpellSupplemental() {
  const content = fs.readFileSync(path.join(dataDir, 'spells', 'supplemental.ts'), 'utf-8');
  ...
  for (let i = 0; i < familyBlocks.length; i++) {
    ...
    console.log(`  Spell family ${i+1}: ${prefixCount} prefixes × ${formCount} forms = ${prefixCount * formCount}`);
  }
  return total;
}
...
function countPowerSupplemental() {
  const content = fs.readFileSync(path.join(dataDir, 'powers-supplemental.ts'), 'utf-8');
  ...
  for (let i = 0; i < familyBlocks.length; i++) {
    ...
    const formCount = (formBlocks[i].match(/"/g) || []).length / 2;
    total += prefixCount * formCount;
    console.log(`  Power family ${i+1}: ${prefixCount} prefixes × ${formCount} forms = ${prefixCount * formCount}`);
  }
  return total;
}
...
function countTechniqueSupplemental() {
  const content = fs.readFileSync(path.join(dataDir, 'techniques-supplemental.ts'), 'utf-8');
  ...
  for (let i = 0; i < familyBlocks.length; i++) {
    ...
    const formCount = (formBlocks[i].match(/"/g) || []).length / 2;
    total += prefixCount * formCount;
    console.log(`  Technique family ${i+1}: ${prefixCount} prefixes × ${formCount} forms = ${prefixCount * formCount}`);
  }
  return total;
}
...
// Spells
console.log('\n--- SPELLS ---');
const spellCoreFiles = ['rank-a.ts', 'rank-b.ts', 'rank-c.ts', 'rank-d.ts', 'rank-s.ts'];
let spellCore = 0;
...
const spellIndex = fs.readFileSync(path.join(dataDir, 'spells', 'index.ts'), 'utf-8');
console.log('\nspells/index.ts:');
console.log(spellIndex);

// Check staticDataProvider for how powers/techniques are exported
const provider = fs.readFileSync(path.join(dataDir, 'staticDataProvider.ts'), 'utf-8');
// Just search for relevant exports
const powerLines = provider.split('\n').filter(l => /power/i.test(l) && (/import|export|concat|spread|\.\.\.|push|combine/i.test(l)));
const techLines = provider.split('\n').filter(l => /technique/i.test(l) && (/import|export|concat|spread|\.\.\.|push|combine/i.test(l)));
const spellLines = provider.split('\n').filter(l => /spell/i.test(l) && (/import|export|concat|spread|\.\.\.|push|combine/i.test(l)));
const featLines = provider.split('\n').filter(l => /feat/i.test(l) && (/import|export|concat|spread|\.\.\.|push|combine/i.test(l)));

console.log('\nstaticDataProvider.ts references:');
console.log('  Spells:', spellLines.map(l => l.trim()));
console.log('  Powers:', powerLines.map(l => l.trim()));
console.log('  Techniques:', techLines.map(l => l.trim()));
console.log('  Feats:', featLines.map(l => l.trim()));

console.log('\n' + '='.repeat(80));
console.log('GRAND TOTAL');
console.log('='.repeat(80));
console.log(`  Spells:     ${spellCore} core + ${spellSupp} supplemental = ${spellCore + spellSupp}`);
console.log(`  Powers:     ${powerCore} core + ${powerSupp} supplemental = ${powerCore + powerSupp}`);
console.log(`  Techniques: ${techCore} core + ${techSupp} supplemental = ${techCore + techSupp}`);
console.log(`  Feats:      ${featCount} (no supplemental)`);
console.log(`  COMBINED:   ${spellCore + spellSupp + powerCore + powerSupp + techCore + techSupp + featCount}`);
