...
import("../src/data/compendium/feats-comprehensive.js").then(m => {
  ...
  const descCounts = {};
  for (const d of descs) { descCounts[d] = (descCounts[d] || 0) + 1; }
  const dupes = Object.entries(descCounts).filter(([_, c]) => c > 1);
  if (dupes.length > 0) {
    console.log(`  !! ${dupes.length} duplicate descriptions found:`);
    dupes.forEach(([desc, count]) => console.log(`     ${count}x: "${desc.substring(0,60)}..."`));
  } else {
