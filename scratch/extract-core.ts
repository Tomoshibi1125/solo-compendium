...
const isUniquePower = (p) =>
	!p.description.startsWith("Strikes for") &&
	!p.description.startsWith("Channels");
const isUniqueTech = (t) =>
	!t.description.startsWith("Strikes for") &&
	!t.description.startsWith("Channels");
...
const uniquePowers = powers.filter(
	(p) => !p.id.includes("parity") && isUniquePower(p),
);
const uniqueTechs = techniques.filter(
	(t) => !t.id.includes("parity") && isUniqueTech(t),
);
