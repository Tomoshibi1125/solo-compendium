...
function findDescriptionTemplate(descs) {
  ...
  for (const desc of descs) {
    ...
    const template = desc
      ...
      .replace(/(Strength|Agility|Intelligence|Sense|Presence|Vitality|Wisdom|Charisma|Constitution|Dexterity)/g, '{ABILITY}')
      .replace(/(Prone|Blinded|Frightened|Stunned|Restrained|Deafened|Incapacitated|Charmed|Paralyzed|Poisoned)/g, '{CONDITION}')
      .replace(/\d+/g, '{N}');
...
function findLimitations(content) {
  ...
  return { uses: countOccurrences(uses), recharges: countOccurrences(recharges) };
}
...
function findDamageDescriptionMismatches(content) {
  ...
  const blockRegex = /id:\s*"([^"]+)"[\s\S]*?name:\s*"([^"]+)"[\s\S]*?description:\s*\n?\s*"([^"]*(?:\n[^"]*)*)"[\s\S]*?(?:damage_profile:\s*"([^"]*)")?/g;
...
function findHomogeneousFields(content, type) {
  ...
  const fields = ['rarity', 'source_book', 'school', 'power_type', 'power_level', 'type', 'style'];
  ...
  for (const field of fields) {
    ...
    if (dominant && dominant[1] / total > 0.8 && total > 5) {
      results[field] = { value: dominant[0], count: dominant[1], total, percentage: Math.round(dominant[1] / total * 100) };
    }
  }
...
const fileSets = {
  ...
  feats: [
    path.join(dataDir, 'feats-comprehensive.ts'),
  ],
};
...
for (const [category, files] of Object.entries(fileSets)) {
  ...
  for (const file of files) {
    ...
    console.log(`  [READ] ${path.basename(file)} - ${data.ids.length} IDs found`);
  }
  ...
  if (dupIds.length > 0) {
    ...
    dupIds.forEach(([id, c]) => console.log(`    - "${id}" appears ${c} times`));
  ...
  if (dupDescs.length > 0) {
    ...
    if (dupDescs.length > 10) console.log(`    ... and ${dupDescs.length - 10} more`);
    categoryReport.issues.duplicateDescriptions = { uniqueRepeated: dupDescs.length, totalDuplicateEntries: totalDups };
  }
  ...
  if (templateGroups.length > 0) {
    ...
    templateGroups.forEach(([template, descs]) => {
      console.log(`    Template (${descs.length} entries): "${template.substring(0, 100)}..."`);
      console.log(`      Examples:`);
      descs.slice(0, 3).forEach(d => console.log(`        - "${d.substring(0, 80)}..."`));
    });
    categoryReport.issues.templatedDescriptions = templateGroups.map(([t, d]) => ({
  ...
  const significantBoilerplate = Object.entries(boilerplate).filter(([_, c]) => c > 5);
  if (significantBoilerplate.length > 0) {
    console.log(`\n  [ISSUE] BOILERPLATE STRINGS (used >5 times):`);
    significantBoilerplate.sort((a, b) => b[1] - a[1]).forEach(([str, c]) => {
      console.log(`    - (${c}x) "${str}"`);
    });
  ...
  if (Object.keys(homogeneous).length > 0) {
    ...
    Object.entries(homogeneous).forEach(([field, info]) => {
      console.log(`    - ${field}: "${info.value}" in ${info.count}/${info.total} entries (${info.percentage}%)`);
    });
  ...
  if (originEntries.length > 0 && originEntries.length < 10) {
    ...
    originEntries.forEach(([origin, c]) => console.log(`    - (${c}x) "${origin}"`));
    categoryReport.issues.loreOrigins = originEntries;
  }
  
  // 7. Repeated flavor text
  const flavors = findFlavors(combinedContent);
  const repeatedFlavors = Object.entries(flavors).filter(([_, c]) => c > 2).sort((a, b) => b[1] - a[1]);
  ...
  const repeatedDiscovery = Object.entries(discovery).filter(([_, c]) => c > 2).sort((a, b) => b[1] - a[1]);
  if (repeatedDiscovery.length > 0) {
    console.log(`\n  [ISSUE] REPEATED DISCOVERY LORE (used >2 times):`);
    repeatedDiscovery.forEach(([d, c]) => console.log(`    - (${c}x) "${d.substring(0, 80)}..."`));
  ...
  const usesEntries = Object.entries(limitations.uses).sort((a, b) => b[1] - a[1]);
  if (usesEntries.length > 0 && usesEntries[0][1] / totalIds.length > 0.5) {
    console.log(`\n  [ISSUE] LIMITATIONS UNIFORMITY:`);
    console.log(`    Uses: "${usesEntries[0][0]}" in ${usesEntries[0][1]}/${totalIds.length} entries (${Math.round(usesEntries[0][1]/totalIds.length*100)}%)`);
    categoryReport.issues.limitationsUniformity = { uses: usesEntries, recharges: Object.entries(limitations.recharges) };
  }
  ...
  categoryReport.descriptionUniqueness = { unique: uniqueDescs.size, total: totalDescs.length, percentage: Math.round(uniqueRatio * 100) };
