...
function analyzePowerSupplemental() {
  ...
  const effectBlocks = content.match(/primary:[\s\S]*?secondary:[\s\S]*?}/g) || [];
...
function analyzeCoreSpells() {
  ...
  return { totalEntries, madLibCount: totalMadLib, uniqueDescriptions: totalUnique };
}
...
function analyzeCoreFeats() {
  ...
  const boilerplateDesc = (content.match(/A subtle manipulation of the magical weave/g) || []).length;
...
function checkSupplementalQuality(filename, label) {
  ...
  const boilerplateChecks = [
    ...
    { pattern: 'A subtle manipulation of the magical weave', label: 'Subtle manipulation' },
  ...
  for (const check of boilerplateChecks) {
    const regex = new RegExp(check.pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
...
console.log(`  Mad-lib template descriptions: ${coreSpells.madLibCount}/${coreSpells.totalEntries} (${Math.round(coreSpells.madLibCount/coreSpells.totalEntries*100)}%)`);
...
console.log(`  Mad-lib template descriptions: ${corePowers.madLibCount}/${corePowers.totalCore} (${Math.round(corePowers.madLibCount/corePowers.totalCore*100)}%)`);
...
console.log(`  Mad-lib template descriptions: ${coreTechs.madLibCount}/${coreTechs.totalCore} (${Math.round(coreTechs.madLibCount/coreTechs.totalCore*100)}%)`);
...
console.log(`  Boilerplate descriptions: ${coreFeats.boilerplateDescCount}/${coreFeats.totalEntries}`);
...
console.log(`  Description mode variants: ${spellSupp.hasMultipleDescriptionTemplates} (11 spell modes × unique templates)`);
console.log(`  Total unique string literals: ${spellSupp.uniqueStrings}`);
const spellSuppQuality = checkSupplementalQuality('spells/supplemental.ts', 'Spell Suppl');
console.log(`  Core boilerplate found:`, Object.keys(spellSuppQuality.boilerplate).length === 0 ? 'NONE ✅' : spellSuppQuality.boilerplate);
console.log(`  Description template count: ${spellSuppQuality.descriptionTemplates}`);
...
const powerSuppQuality = checkSupplementalQuality('powers-supplemental.ts', 'Power Suppl');
console.log(`  Core boilerplate found:`, Object.keys(powerSuppQuality.boilerplate).length === 0 ? 'NONE ✅' : powerSuppQuality.boilerplate);
...
const techSuppQuality = checkSupplementalQuality('techniques-supplemental.ts', 'Tech Suppl');
console.log(`  Core boilerplate found:`, Object.keys(techSuppQuality.boilerplate).length === 0 ? 'NONE ✅' : techSuppQuality.boilerplate);
...
const boilerplateCore = coreSpells.madLibCount + corePowers.madLibCount + coreTechs.madLibCount + coreFeats.boilerplateDescCount;
const uniqueCore = (coreSpells.uniqueDescriptions || 0) + corePowers.uniqueDescriptions + coreTechs.uniqueDescriptions + coreFeats.uniqueDescriptions;
...
console.log(`  Supplemental: ${totalSuppl} entries use mode-specific templates with proper variation`);
console.log(`\n  Overall: ${boilerplateCore} boilerplate + ${uniqueCore} unique-core + ${totalSuppl} supplemental = ${totalAll}`);
console.log(`  Boilerplate ratio of entire compendium: ${Math.round(boilerplateCore/totalAll*100)}%`);
