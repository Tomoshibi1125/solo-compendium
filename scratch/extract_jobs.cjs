...
for (const block of jobBlocks) {
  ...
  if (idM && nameM) {
    const category = spellcastingM ? (powersM ? 'hybrid' : 'caster') : (powersM ? 'martial' : 'unknown');
    console.log(`${nameM[1].padEnd(20)} id=${idM[1].padEnd(20)} hitDie=${(hitDieM?hitDieM[1]:'?').padEnd(6)} primary=${(primaryM?primaryM[1]:'?').padEnd(15)} spells=${spellcastingM?'YES':'no '}  powers=${powersM?'YES':'no '}  techniques=${techM?'YES':'no '}  => ${category}`);
  }
}

// Also check for class references in spells/powers/techniques supplemental files
console.log('\n=== Classes referenced in spell supplemental ===');
const spellRaw = fs.readFileSync('src/data/compendium/spells/supplemental.ts', 'utf-8');
...
const powersRaw = fs.readFileSync('src/data/compendium/powers-supplemental.ts', 'utf-8');
...
const techRaw = fs.readFileSync('src/data/compendium/techniques-supplemental.ts', 'utf-8');
