...
if (dupFlav.length > 0) {
	console.error("DUPLICATE FLAVORS:", dupFlav);
}
...
function rarityForRank(r) {
	return { S: "legendary", A: "epic", B: "rare", C: "uncommon", D: "common" }[
		r
	];
}
...
for (const s of allSpells) {
	...
	if (
		!mechAbility &&
		cls.some((c) => ["Mage", "Revenant", "Technomancer"].includes(c))
	)
		mechAbility = "Intelligence";
	else if (!mechAbility && cls.some((c) => ["Summoner", "Stalker"].includes(c)))
		mechAbility = "Sense";
	...
	lines.push(
		`\t\ttags: ${JSON.stringify(["awakened", "magic", rank, ...tags])},`,
	);
	lines.push(`\t\trarity: ${JSON.stringify(rarity)},`);
	lines.push(`\t\tsource_book: "Rift Ascendant Canon",`);
	lines.push(
		`\t\teffects: { primary: ${JSON.stringify(effPri)}, secondary: ${JSON.stringify(effSec)} },`,
	);
	lines.push(`\t\tlevel: ${lv},`);
	...
	lines.push(
		`\t\tcomponents: { verbal: ${comp.v ?? comp.verbal ?? true}, somatic: ${comp.s ?? comp.somatic ?? true}, material: ${comp.m ? JSON.stringify(comp.m) : comp.material ? JSON.stringify(comp.material) : "false"} },`,
	);
	...
	if (atk) {
		lines.push(
			`\t\tattack: { type: ${JSON.stringify(atk)}, ability: ${JSON.stringify(atkAbl)}, damage: ${JSON.stringify(dmg)} },`,
		);
	} else {
	...
	if (saveAbl && saveDC > 0) {
		lines.push(
			`\t\tsaving_throw: { ability: ${JSON.stringify(saveAbl)}, dc: ${saveDC}, success: ${JSON.stringify(saveSuccess)}, failure: ${JSON.stringify(saveFailure)} },`,
		);
	} else {
	...
	if (dmg && dt)
		lines.push(`\t\t\tdamage_profile: ${JSON.stringify(`${dmg} ${dt}`)},`);
