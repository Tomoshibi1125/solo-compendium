...
for (const e of allEntries) {
  if (e.school) {
    report.schoolDistribution[e.school] = (report.schoolDistribution[e.school] || 0) + 1;
  }
  if (e.damage_profile) {
    report.damageProfileDistribution[e.damage_profile] = (report.damageProfileDistribution[e.damage_profile] || 0) + 1;
  }
  if (e.range) {
    report.rangeDistribution[e.range] = (report.rangeDistribution[e.range] || 0) + 1;
  }
  if (e.duration) {
    report.durationDistribution[e.duration] = (report.durationDistribution[e.duration] || 0) + 1;
  }
  if (e.uses) {
    report.usesDistribution[e.uses] = (report.usesDistribution[e.uses] || 0) + 1;
  }
  if (e.casting_time) {
    report.castingTimeDistribution[e.casting_time] = (report.castingTimeDistribution[e.casting_time] || 0) + 1;
  }
}

// Count how many entries are "Transmutation" school
const transCount = allEntries.filter(e => e.school === 'Transmutation').length;
report.mechanicalHomogeneity.allSameSchool = `${transCount}/${allEntries.length} are Transmutation`;

const instCount = allEntries.filter(e => e.duration === 'Instantaneous').length;
...
const dmgDash = allEntries.filter(e => e.damage_roll === '—' || e.damage_roll === '-').length;
