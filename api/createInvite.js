...
export default async function handler(req, res) {
	...
	const maxUses = Number.isFinite(maxUsesRaw)
		? Math.max(1, Math.floor(maxUsesRaw))
		: 1;
	const inviteEmail =
		typeof body.inviteEmail === "string"
			? body.inviteEmail.trim().toLowerCase()
			: "";
	...
	if (!invite) {
		return res
			.status(500)
			.json({ error: "Invite was not returned by create_campaign_invite RPC" });
	}
