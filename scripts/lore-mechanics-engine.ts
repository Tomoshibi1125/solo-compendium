...
function enrichSpell(spell: any) {
...
function enrichPowerOrTechnique(pt: any, isPower: boolean) {
    const obj = sanitizeObject(pt);
    if (!obj.lore) {
        obj.lore = generateLore(isPower ? "Awakened Power" : "Hunter Technique", obj.name);
    }
...
function processFile(filePath: string, type: 'spell' | 'power' | 'technique' | 'feat' | 'monster') {
...
async function superRegexEnrichment() {
    ...
    for (const f of filesToEnrich) {
        ...
        content = content.replace(nameRegex, (match, prefix, name, suffix) => {
            ...
            if (content.substring(content.indexOf(match), content.indexOf(match) + 150).includes('lore:')) {
                return match; 
            }
...
function regenerateItems() {
    const targetCount = Math.floor(913 - 186); // To rebuild parts 6 to 9 (approx 650-700 items)
    
    // We will use generate-items.ts logic but injected into this engine with massive enhancements
    console.log(`[INFO] Re-generating bulk items to meet 100% lore/mechanics deep requirement...`);
