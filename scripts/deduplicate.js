...
async function main() {
    ...
    for (const filePath of allFiles) {
        ...
        if (content.includes("import ") && content.includes("from ")) {
            ...
            if (content.includes("...")) {
                console.log(`[SKIP] Aggregate file skipped: ${path.basename(filePath)}`);
                continue;
            }
        }
        ...
        const exportKey = Object.keys(moduleData).find((k) => Array.isArray(moduleData[k]));
        if (!exportKey)
            continue;
        ...
        if (originalArray.length !== deduplicatedArray.length) {
            ...
            console.log(`[DEDUPE] Cleared duplicates in ${path.basename(filePath)}. Reduced from ${originalArray.length} to ${deduplicatedArray.length}.`);
        }
    }
    if (removedCount > 0) {
        console.log(`=> Removed a total of ${removedCount} duplicates across the compendium.`);
    }
    console.log("Done!");
}
