...
async function generateUniqueContent(batch: any[], type: string): Promise<any[]> {
...
async function processFile(filePath: string, exportName: string, typeDesc: string, header: string) {
    ...
    console.log(`Found ${dataArray.length} items. Processing in batches of 20...`);
    const finalArray = [];
    ...
    console.log(`[SUCCESS] Rewrote ${filePath} with ${finalArray.length} ultra-unique entries.`);
}

async function runAll() {
    console.log("=== STARTING EXPERIMENTAL SYSTEM ASCENDANT AI GENERATION ENGINE ===");
    
    const targets = [
        { file: 'src/data/compendium/spells/rank-s.ts', name: 'spells_s', type: 'S-Rank Spell Protocols', header: '// S-Rank Spells\n// Massive system protocols requiring immense mana resources.' },
        { file: 'src/data/compendium/powers.ts', name: 'powers', type: 'Awakened Powers', header: '// Awakened Powers\n// Core abilities utilized by advanced Hunters.' }
    ];
