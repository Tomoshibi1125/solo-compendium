...
function tagAt(txt, idx) {
	...
	for (let i = idx; i < txt.length; i++) {
		...
		if (c === "{") depth++;
			...
			return { tag: txt.slice(idx, i + 1), end: i + 1 };
	}
...
for (const f of files) {
	...
	let scan = "";
	let cursor = 0;
	let index = txt.indexOf("<Button", cursor);
	while (index !== -1) {
		const { tag, end } = tagAt(txt, index);
		...
		if (isIcon && !hasName) {
			...
			const newTag = tag.replace(
				/size=(["'])icon\1/,
				`size=$1icon$1 aria-label="${label}"`,
			);
			scan += txt.slice(cursor, index) + newTag;
			changed = true;
			count++;
		...
		cursor = end;
		index = txt.indexOf("<Button", cursor);
	}
