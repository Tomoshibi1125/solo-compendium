...
const OUTPUT = resolve(__dirname, "..", "docs", "adventure-protocol-zero-sandbox.md");
...
function formatAnomaly(anomaly) {
    ...
    const formatStat = (val) => `${val} (${mod(val) >= 0 ? "+" : ""}${mod(val)})`;
...
console.log(`Included ${selectedLocations.length} locations, ${selectedRelics.length} relics, and ${selectedAnomalies.length} anomalies.`);
