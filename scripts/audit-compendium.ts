...
type AuditReport = {
	summary: {
		categories: Record<
			string,
			{ entries: number; errors: number; warnings: number }
		>;
...
const auditSpell = (entry: StaticCompendiumEntry, issues: AuditIssue[]) => {
	...
	if (
		!isRecord(entry.activation) ||
		!hasNonEmptyString(entry.activation.type)
	) {
		pushIssue(issues, {
			type: "spells",
			id: entry.id,
	...
	if (!isRecord(entry.effects) || !hasNonEmptyString(entry.effects.primary)) {
		pushIssue(issues, {
			...
			severity: "warn",
			details:
				"Spell has no structured primary effect (falls back to legacy effect field).",
		});
	}
	...
	if (!hasAttack && !hasSave && !hasHealing) {
		pushIssue(issues, {
			...
			severity: "warn",
			details:
				"Spell has no mechanics resolution; add mechanics.attack, mechanics.saving_throw, or mechanics.healing.",
		});
	}
};
...
const auditMonster = (entry: StaticCompendiumEntry, issues: AuditIssue[]) => {
	...
	if (
		!Array.isArray(entry.monster_actions) ||
		entry.monster_actions.length === 0
	) {
		pushIssue(issues, {
			type: "monsters",
			id: entry.id,
			...
			severity: "warn",
			details:
				"Monster has no embedded actions; if not using Supabase, add actions to static pack.",
		});
	}

	if (
		!Array.isArray(entry.monster_traits) ||
		entry.monster_traits.length === 0
	) {
		pushIssue(issues, {
			type: "monsters",
			id: entry.id,
			...
			severity: "warn",
			details:
				"Monster has no embedded traits; if not using Supabase, add traits to static pack.",
		});
	}
};
...
async function main() {
	...
	for (const category of categories) {
		...
		if (category === "monsters")
			entries = await staticDataProvider.getMonsters("");
	...
	const report: AuditReport = {
		summary: {
			...
			total: {
				entries: Object.values(categoryStats).reduce(
					(sum, s) => sum + s.entries,
					0,
				),
				errors: Object.values(categoryStats).reduce(
					(sum, s) => sum + s.errors,
					0,
				),
				warnings: Object.values(categoryStats).reduce(
