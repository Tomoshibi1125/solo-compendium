...
const allItems = [
    ...monsters.filter(x => x.id.startsWith('companion-') || x.id.startsWith('mount-') || x.id.startsWith('pet-'))
        .map(x => ({ name: x.name, img: (x as any).image, desc: x.description })),
    ...vehicles.filter(x => x.item_type === 'vehicle')
        .map(x => ({ name: x.name, img: (x as any).image, desc: x.description }))
].filter(x => x.img);
...
async function generateToken(item: any) {
    ...
    try {
        ...
        console.error(` -> Error generating ${item.name}: ${e.message?.substring(0, 100)}`);
    }
}
