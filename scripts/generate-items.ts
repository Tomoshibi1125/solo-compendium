...
const weaponMechanicsMap: Record<string, {dmg: string, prop: string}> = {
    ...
    "Bow": {dmg: "1d8 piercing", prop: "ammunition (range 150/600), heavy, two-handed"},
    "Crossbow": {dmg: "1d10 piercing", prop: "ammunition (range 100/400), heavy, loading, two-handed"},
...
const armorMechanicsMap: Record<string, {ac: string, prop: string}> = {
    ...
    "Robes": {ac: "10 + Dex modifier", prop: "Unarmored defense. Mana flow increased."},
    ...
    "Mantle": {ac: "None", prop: "Advantage on saving throws against environmental hazards."}
};
...
function generateNameAndType(): { name: string, type: string, rarity: string, base: string } {
...
function generateDetailedDescription(name: string, type: string, base: string, rarity: string): string {
...
function generateEffect(rarity: string, type: string) {
    ...
    let effect: any = { passive: passives };
    if (Math.random() > 0.5 && rarity !== "common") {
        const cost = rarity === "uncommon" ? "10 MP" : rarity === "rare" ? "25 MP" : "50 MP";
        effect.active = [{
...
for (let i = 0; i < items.length; i += chunkSize) {
    ...
    let jsonStr = JSON.stringify(chunk, null, '\t')
        .replace(/"([^"]+)":/g, '$1:');
    ...
    fs.writeFileSync(path.join(process.cwd(), 'src', 'data', 'compendium', filename), content);
    console.log(`Generated ${filename} with ${chunk.length} items.`);
    partNum++;
}

console.log("Successfully overhauled and generated", items.length, "high-detail items.");
