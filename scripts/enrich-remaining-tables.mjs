...
async function enrichTable(tableName, category, opts = {}) {
	...
	for (let i = 0; i < rows.length; i++) {
		...
		if (!row.tags || row.tags.length === 0)
			u.tags = generateTags(row.name || "", row.description || "", category, r);
		if (hasThemeTags && (!row.theme_tags || row.theme_tags.length === 0))
			u.theme_tags = pickN(THEME_TAGS, 2 + Math.floor(r() * 2), r);
		...
		if (
			...
			u.mechanics = generateMechanics(row.name || "", row.description || "", r);
		...
		if ((i + 1) % 50 === 0 || i === rows.length - 1)
			process.stdout.write(
				`\r  ${i + 1}/${rows.length} (${updated} upd, ${skipped} skip)`,
			);
	}
