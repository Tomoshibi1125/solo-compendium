...
const vehicles = rawItems.filter((x) => x.item_type === "vehicle");
...
async function generatePlaceholder(name, outPath) {
    const initials = name
        .split(" ")
        .map((n) => n[0])
        .join("")
...
async function searchWikimedia(query) {
    ...
    try {
        const res = await axios.get(`https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(`${coreTerm} art|drawing|illustration`)}&gsrnamespace=6&gsrlimit=1&prop=imageinfo&iiprop=url&format=json`);
...
async function processEntry(entryName, imagePathStr) {
    if (!imagePathStr)
        return;
