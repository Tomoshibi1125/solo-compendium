...
const themeFlavorMap = {
	...
	Absolute: {
		...
		stat: "Presence",
		dmg: "force",
		lore: "a sovereign mandate from the Architect",
	},
};
...
function processEntry(entry, rank, filePath) {
	...
	if (entry.includes('description: "') || entry.includes("description: ''")) {
		...
		const isSigil =
			filePath.includes("sigils") || entry.includes("can_inscribe_on:");
		const isTattoo =
			filePath.includes("tattoos") || entry.includes("body_part:");

		let desc = "";
		if (isRune)
			desc = `This sovereign skill-stone teaches the target how to ${flavor.verbs[0]} the local ${flavor.noun}. For those without an innate mana-heart, the stone provides a fixed usage lattice, allowing the technique to be manifested through the stone's residual resonance.`;
		else if (isSigil)
			desc = `Forged from ${flavor.lore}, this sigil acts as a dimensional socket for high-tier equipment. Upon inscription, it allows the wielder to ${flavor.verbs[1]} incoming essence, converting it into a stable ${flavor.adj} field.`;
		else if (isTattoo)
			desc = `A permanent manifold inscribed using ${flavor.adj} dimensional ink. The tattoo grafts directly onto the hunter's soul-lattice, allowing them to ${flavor.verbs[0]} their own essence into a concentrated burst of ${flavor.noun}.`;
		else
			desc = `This ${rank}-Rank artifact vibrates with the power to ${flavor.verbs[0]} reality. It allows the wielder to manifest a ${flavor.adj} field that can ${flavor.verbs[1]} any proximity-based dimensional distortion.`;
	...
	const mech = {
		stat_bonuses: {
			[flavor.stat]:
				rank === "Divine" || rank === "Mythic" || rank === "S" ? 2 : 1,
		},
	...
	entry = entry.replace(
		/^\s*(?:requires_level|level_requirement|gate_rank):\s*\d+,?\s*$/gm,
		"",
	);
...
for (let i = 1; i <= 9; i++)
	processFile(path.join(baseDir, `items-part${i}.ts`), "B");
	...
	for (const rf of runeFiles) {
		if (rf.endsWith(".ts") && rf !== "index.ts") {
			processFile(
				...
				rf.includes("-s") ? "S" : rf.includes("-a") ? "A" : "B",
			);
		}
	}
}
