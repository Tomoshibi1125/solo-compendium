...
const CATEGORY_THEMES = {
	...
	compendium_job_paths: {
		icon: "🛤",
		colors: ["#1a1a2e", "#0d2137", "#162447"],
		accent: "#1f4068",
	},
	compendium_job_features: {
		icon: "✨",
		colors: ["#1b1b2f", "#1a1a3e", "#162447"],
		accent: "#e43f5a",
	},
	compendium_powers: {
		icon: "🔥",
		colors: ["#200122", "#2d0037", "#6f0000"],
		accent: "#ff6b35",
	},
	compendium_relics: {
		icon: "💎",
		colors: ["#0c0c1d", "#1a0a2e", "#2d1b69"],
		accent: "#b721ff",
	},
	compendium_equipment: {
		icon: "🛡",
		colors: ["#1a1a2e", "#16213e", "#1f4068"],
		accent: "#4ecdc4",
	},
	compendium_Anomalies: {
		icon: "👹",
		colors: ["#1a0000", "#2d0000", "#4a0000"],
		accent: "#ff2e63",
	},
	compendium_backgrounds: {
		icon: "📜",
		colors: ["#1a1510", "#2d2215", "#4a3728"],
		accent: "#daa520",
	},
	compendium_conditions: {
		icon: "💀",
		colors: ["#0d0d0d", "#1a0a1a", "#2d0a2d"],
		accent: "#9b59b6",
	},
	compendium_feats: {
		icon: "🏆",
		colors: ["#1a1a00", "#2d2d00", "#4a4a00"],
		accent: "#f1c40f",
	},
	compendium_skills: {
		icon: "📊",
		colors: ["#001a1a", "#002d2d", "#004a4a"],
		accent: "#1abc9c",
	},
	compendium_regents: {
		icon: "👑",
		colors: ["#1a0a2e", "#2d0037", "#4a0060"],
		accent: "#8e44ad",
	},
	compendium_regent_features: {
		icon: "🌑",
		colors: ["#0a0a1a", "#0d0d2d", "#10104a"],
		accent: "#6c5ce7",
	},
	compendium_sovereigns: {
		icon: "⚡",
		colors: ["#1a0000", "#2d0a00", "#4a1500"],
		accent: "#e17055",
	},
	compendium_sovereign_features: {
		icon: "🌟",
		colors: ["#1a1000", "#2d1a00", "#4a2d00"],
		accent: "#fdcb6e",
	},
	compendium_runes: {
		icon: "᛭",
		colors: ["#0a1a2e", "#0d2d4a", "#104a6e"],
		accent: "#00cec9",
	},
	compendium_shadow_soldiers: {
		icon: "⚔",
		colors: ["#050510", "#0a0a20", "#0f0f30"],
		accent: "#6c5ce7",
	},
};
...
async function enrichTable(tableName) {
	...
	for (let i = 0; i < data.length; i++) {
		...
		if (hasImageUrl && (!row.image_url || row.image_url === "")) {
			u.image_url = generateSvgDataUri(
				row.display_name || row.name || row.title || "",
				tableName,
				r,
			);
		}
		...
		if (hasLore && row.lore && typeof row.lore === "string") {
			u.lore = {
				origin: row.lore,
				history: "Its full history remains undocumented.",
			};
		}
		...
		if ((i + 1) % 100 === 0 || i === data.length - 1) {
			process.stdout.write(
				`\r  ${i + 1}/${data.length} (${updated} upd, ${skipped} skip)`,
			);
		}
	}
