...
function castToStringArray(v) {
    if (Array.isArray(v)) {
        return v.filter((item) => typeof item === "string");
    }
    return null;
}
function mapAbility(s) {
    if (!s)
        return "STR";
    const upper = s.toUpperCase();
    if (upper === "STRENGTH" || upper === "STR")
        return "STR";
    if (upper === "AGILITY" || upper === "AGI")
        return "AGI";
    if (upper === "VITALITY" || upper === "VIT")
        return "VIT";
    if (upper === "INTELLIGENCE" || upper === "INT")
        return "INT";
    if (upper === "SENSE")
        return "SENSE";
    if (upper === "PRESENCE" || upper === "PRE")
        return "PRE";
    return "STR";
}
...
function mapRarity(r) {
    ...
    if (norm === "mythic")
        return "legendary"; // Fallback for out-of-spec rarities
    return "common";
}
// ---------------------------------------------------------------------------
...
async function syncSpells() {
    ...
    for (const m of data) {
        ...
        rows.push({
            ...
            saving_throw_ability: (m.saving_throw_ability || castToString(m.saving_throw?.ability)) ?? undefined,
...
async function syncJobs() {
    ...
    for (const m of data) {
        ...
        rows.push({
            ...
            hit_die: m.hit_die || m.hitDie
                ? Number.parseInt((m.hitDie || "").replace(/\D/g, "").slice(-2) || String(m.hit_die || 8), 10) || 8
                : 8,
            ...
            type: m.type || "Combat",
            source_book: m.source || "Rift Ascendant Canon",
        });
    }
    const names = rows.map((r) => r.name);
    for (let i = 0; i < names.length; i += 500) {
...
async function syncJobPaths() {
    ...
    for (const m of data) {
        ...
        const jobLookup = JOB_NAME_TO_ID[m.jobName || ""] ||
            JOB_NAME_TO_ID[m.job_name || ""] ||
            JOB_NAME_TO_ID[m.jobId || ""] ||
            m.job_id ||
            m.jobId;
        if (!jobLookup) {
            log(`  [JobPaths] Warning: No Job ID for path "${m.name}" (jobName: "${m.jobName || m.job_name}") — skipping`);
            continue;
        }
        rows.push({
            name: m.name,
            ...
            description: m.description || "",
            flavor_text: m.flavor || m.flavor_text || null,
            path_level: m.path_level ?? m.level ?? 3,
            job_id: jobLookup,
            source_book: m.source || "Rift Ascendant Canon",
        });
    }
...
async function syncBackgrounds() {
    ...
    if (error)
        log(`  [Backgrounds] Error: ${error.message}`);
...
async function syncRelics() {
    ...
    if (error)
        log(`  [Relics] Error: ${error.message}`);
