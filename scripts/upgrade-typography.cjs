...
for (const file of files) {
	...
	if (pRegex.test(content)) {
		...
		const hasAnySystemTextImport = content.includes(
			'@/components/ui/SystemText"',
		);
		const hasSpecificSystemTextImport =
			/import\s+{[^}]*?\bSystemText\b[^}]*?}\s+from\s+"@\/components\/ui\/SystemText"/.test(
				content,
			);
		...
		if (!hasAnySystemTextImport) {
			...
			content = content.replace(
				/import\s*{([^}]+)}\s*from\s*"@\/components\/ui\/SystemText";/,
				'import { SystemText, $1 } from "@/components/ui/SystemText";',
			);
		}
