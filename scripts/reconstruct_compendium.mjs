...
const themeFlavorMap = {
    ...
    Absolute: { adj: "fundamental", verbs: ["mandate", "decree"], noun: "authority-sigil", stat: "Presence", dmg: "force", lore: "a fundamental mandate from the Architect" }
};
...
function generateMechanics(theme, rank, isArtifact) {
    ...
    return {
        ...
        special_abilities: [`Rift ${flavor.adj} Core`, `${flavor.verbs[0].toUpperCase()} Protocol`],
        restrictions: ["Warden Clearance Level 3 Required", "Lattice-Locked"],
        progression: { "Rank B": ["Inert"], "Rank A": ["Resonant"], "Rank S": ["Absolute"] },
        ...
        critical: true,
        fumble: true,
        bonus: { type: "Warden Sync", value: Math.floor(4 * powerMult), ability: flavor.stat, skills: ["Investigation", "Insight"] },
        immunity: [flavor.dmg],
        resistance: ["Force", "Psychic", "Necrotic"],
        vulnerability: ["Absolute Null"],
        special: [`This item is an extension of the ${theme} lattice.`],
        healing: { dice: "4d6", type: "Lattice Mending", bonus: Math.floor(10 * powerMult) }
    };
}

function generateLimitations(theme, rank) {
...
function generateEffects(theme, rank) {
    ...
    return {
        ...
        passive: [`User gains Truesight up to 60 feet.`, `All melee attacks deal an extra 1d8 ${flavor.dmg} damage.`],
        active: [{
...
function generateLore(name, theme) {
    const flavor = themeFlavorMap[theme] || themeFlavorMap.Absolute;
    return {
...
function reconstructItemFromChunk(id, chunk, rank, filePath) {
    ...
    let cleanChunk = chunk.replace(/sovereign/gi, "rift").replace(/sovereignty/gi, "authority");

    const allPossibleKeys = [
        "name", "display_name", "description", "created_at", "updated_at", "source_kind", "source_name",
        "theme_tags", "generated_reason", "discovery_lore", "concentration", "image", "image_url",
        "license_note", "flavor", "lore", "source", "source_book", "tags", "system_interaction",
        "mechanics", "limitations", "effects", "rarity", "cr",
        "rune_type", "rune_category", "effect_type", "activation_action", "activation_cost",
        "activation_cost_amount", "duration", "range", "uses_per_rest", "recharge", "higher_levels",
        "requires_job", "caster_penalty", "martial_penalty", "passive_bonuses", "can_inscribe_on",
        "inscription_difficulty", "caster_requirement_multiplier", "martial_requirement_multiplier",
        "requirement_agi", "requirement_int", "requirement_pre", "requirement_sense", "requirement_str",
        "requirement_vit", "body_part", "ink_type", "active_veins", "resonance_effect",
        "type", "item_type", "attunement", "properties", "simple_properties", "armor_class",
        "armor_type", "damage", "damage_type", "weapon_type", "stealth_disadvantage", 
        "strength_requirement", "weight", "benefits", "prerequisites", "geography", "hazards", 
        "notable_npcs", "cure_lore", "stages"
    ];
    ...
    for (const key of allPossibleKeys) {
        ...
        if (match) {
            ...
            if (cleanChunk[startIdx] === '{') {
                ...
                const candidates = [cleanChunk.indexOf(',', startIdx), cleanChunk.indexOf('\n', startIdx), cleanChunk.indexOf('}', startIdx)]
                    .filter(idx => idx !== -1);
                const realEndIdx = candidates.length > 0 ? Math.min(...candidates) : cleanChunk.length;
            ...
            if (val) {
                try {
                    ...
                    pairs[key] = val.trim().replace(/[,;]\s*$/, '').replace(/\];$/, ']').replace(/\};$/, '}');
                }
            }
        }
    }
    ...
    const isSigil = filePath.includes("sigils");
    ...
    pairs.description = pairs.description || JSON.stringify(`An authoritative rift-artifact of the ${theme} sector. It grants the user a direct link to the dimensional lattice, enabling high-tier ${flavor.verbs[0]} protocols and stabilizing local space.`);
    ...
    pairs.generated_reason = pairs.generated_reason || JSON.stringify("Direct Rift Extraction");
    pairs.discovery_lore = pairs.discovery_lore || JSON.stringify(`Found within the ${theme} Fracture Zone by Warden explorers.`);
    pairs.concentration = pairs.concentration || (isRune ? "true" : "false");
    pairs.image = pairs.image || JSON.stringify(`/images/compendium/rift-${theme.toLowerCase()}.webp`);
    pairs.image_url = pairs.image_url || pairs.image;
    pairs.license_note = pairs.license_note || JSON.stringify("Restricted to Archon Rank Wardens");
    pairs.flavor = pairs.flavor || JSON.stringify(`A cold, ${flavor.adj} lattice pulse emanates from this object.`);
    
    const baseLore = generateLore(name, theme);
    let parsedLore = {};
    if (pairs.lore) {
        try { parsedLore = new Function(`return (${pairs.lore})`)(); } catch(e){}
        if (typeof parsedLore === 'string') { parsedLore = { history: parsedLore }; }
    }
    pairs.lore = JSON.stringify(mergeObjects(baseLore, parsedLore), null, '\t\t').replace(/\n/g, '\n\t\t');

    pairs.source = pairs.source || JSON.stringify("Rift Ascendant Core");
    pairs.source_book = pairs.source_book || JSON.stringify("Manual of Ascension");
    pairs.tags = pairs.tags || JSON.stringify(["rift-tech", isRune ? "rune" : "artifact", theme]);
    pairs.system_interaction = pairs.system_interaction || JSON.stringify(`Direct Lattice Binding Protocol Active.`);
    
    const baseMechanics = generateMechanics(theme, rank, isArtifact);
    let parsedMechanics = {};
    if (pairs.mechanics) {
        try { parsedMechanics = new Function(`return (${pairs.mechanics})`)(); } catch(e){}
    }
    pairs.mechanics = JSON.stringify(mergeObjects(baseMechanics, parsedMechanics), null, '\t\t').replace(/\n/g, '\n\t\t');

    const baseLimitations = generateLimitations(theme, rank);
    let parsedLimitations = {};
    if (pairs.limitations) {
        try { parsedLimitations = new Function(`return (${pairs.limitations})`)(); } catch(e){}
    }
    pairs.limitations = JSON.stringify(mergeObjects(baseLimitations, parsedLimitations), null, '\t\t').replace(/\n/g, '\n\t\t');

    const baseEffects = generateEffects(theme, rank);
    let parsedEffects = {};
    if (pairs.effects) {
        try { parsedEffects = new Function(`return (${pairs.effects})`)(); } catch(e){}
        if (Array.isArray(parsedEffects)) { parsedEffects = { passive: parsedEffects }; }
    }
    pairs.effects = JSON.stringify(mergeObjects(baseEffects, parsedEffects), null, '\t\t').replace(/\n/g, '\n\t\t');
    ...
    if (isRune) {
        pairs.rune_type = pairs.rune_type || JSON.stringify(theme.includes("Blood") || theme.includes("Titanic") ? "martial" : "caster");
        ...
        pairs.activation_action = pairs.activation_action || JSON.stringify("1 Action");
        pairs.activation_cost = pairs.activation_cost || JSON.stringify("20 Essence");
        ...
        pairs.uses_per_rest = pairs.uses_per_rest || JSON.stringify(`Archon Rank: 5/Long Rest`);
        pairs.recharge = pairs.recharge || JSON.stringify("Short Rest");
        pairs.higher_levels = pairs.higher_levels || JSON.stringify(`Increase output by 2d10 ${flavor.dmg} per level.`);
        pairs.requires_job = pairs.requires_job || JSON.stringify(["Warden", "Archon", "Rift-Master"]);
        pairs.caster_penalty = pairs.caster_penalty || JSON.stringify("Lattice Feedback Protocol Engage");
        pairs.martial_penalty = pairs.martial_penalty || JSON.stringify("Physical Strain Protocol Engage");
        pairs.passive_bonuses = pairs.passive_bonuses || JSON.stringify({ [flavor.stat]: 4 }, null, '\t\t').replace(/\n/g, '\n\t\t');
        pairs.can_inscribe_on = pairs.can_inscribe_on || JSON.stringify(["Archon Plate", "Lattice Weapon", "Soul-Anchor"]);
        pairs.inscription_difficulty = pairs.inscription_difficulty || "22";
        pairs.caster_requirement_multiplier = pairs.caster_requirement_multiplier || "1";
        pairs.martial_requirement_multiplier = pairs.martial_requirement_multiplier || "1";
        pairs.requirement_agi = pairs.requirement_agi || (theme === "Storm" ? "18" : "14");
        pairs.requirement_int = pairs.requirement_int || (theme === "Void" ? "20" : "14");
        pairs.requirement_pre = pairs.requirement_pre || (theme === "Solar" ? "20" : "14");
        pairs.requirement_sense = pairs.requirement_sense || (theme === "Aetheric" ? "18" : "14");
        pairs.requirement_str = pairs.requirement_str || (theme === "Blood" ? "20" : "14");
        pairs.requirement_vit = pairs.requirement_vit || (theme === "Titanic" ? "20" : "14");
    }

    if (isTattoo) {
        pairs.attunement = pairs.attunement || "true";
        pairs.body_part = pairs.body_part || JSON.stringify("Rift-Graft");
        pairs.ink_type = pairs.ink_type || JSON.stringify(`${flavor.adj} Lattice Essence`);
        pairs.active_veins = pairs.active_veins || JSON.stringify(["Ascendant Path", "Prime Line"]);
        pairs.resonance_effect = pairs.resonance_effect || JSON.stringify(`Triggers a DC 20 ${flavor.adj} Rift Wave on soul-depletion.`);
    }
...
function processFile(filePath, rank, header) {
    ...
    const cleanedObjects = itemsData.map((itemObj, i) => {
        const start = itemObj.index;
        const end = (i < itemsData.length - 1) ? itemsData[i+1].index : content.length;
    ...
    if (filePath.includes('runes')) {
        if (fileName.startsWith('spell-rank-')) { exportName = `runes_${fileName.split('-').pop()}`; }
        else if (fileName === 'skill-runes') { exportName = 'runes_skill_skills'; }
        else { exportName = `runes_${exportName}`; }
    }
    ...
    let finalHeader = `import type { ${typeStr} } from "@/types/compendium";\n${header}`;
...
if (fs.existsSync(runesDir)) {
    ...
    for (const rf of runeFiles) {
        if (rf.endsWith(".ts") && rf !== "index.ts") {
            processFile(path.join(runesDir, rf), rf.includes("-s") ? "S" : (rf.includes("-a") ? "A" : (rf.includes("-b") ? "B" : (rf.includes("-c") ? "C" : "D"))), standardHeader);
        }
    }
}
