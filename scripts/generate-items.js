...
function generateDetailedDescription(_name, type, base, rarity) {
...
function generateEffect(rarity, _type) {
    const passiveCount = rarity === "common"
        ? 1
        : rarity === "uncommon"
            ? 1
            : rarity === "rare"
    ...
    const effect = { passive: passives };
    if (Math.random() > 0.5 && rarity !== "common") {
        const cost = rarity === "uncommon"
            ? "10 Mana"
            : rarity === "rare"
                ? "25 Mana"
                : "50 Mana";
...
for (let i = 0; i < items.length; i += chunkSize) {
    ...
    const jsonStr = JSON.stringify(chunk, null, "\t").replace(/"([^"]+)":/g, "$1:");
