...
const titles = ["Void Engineer", "System Architect", "Rift Diver", "Mana Miner", "Shadow Broker", "Dimensional Smuggler", "Astral Medic", "Guild Enforcer", "Gate Cleaner", "Essence Harvester", "Dungeon Mapper", "Relic Appraiser", "Umbral Scout", "System Glitch", "Awakened Civilian", "Corporate Ascendant"];
...
const tools = ["Thieves' Tools", "Smith's Tools", "Alchemist's Supplies", "Tinker's Tools", "Herbalism Kit", "Navigator's Tools", "Poisoner's Kit"];
...
while (backgrounds.length < targetCount) {
    ...
    const bg = {
        ...
        bonds: [
            "Someone in my past life saved me during my awakening."
        ],
        flaws: [
            "I default to my old civilian habits when panicked."
        ],
...
const jsonStr = JSON.stringify(backgrounds, null, '\t').replace(/"([^"]+)":/g, '$1:');
const content = `// Generated Backgrounds Expansion\nexport const expandedBackgrounds = ${jsonStr};\n`;
fs.writeFileSync(path.join(process.cwd(), 'src', 'data', 'compendium', 'backgrounds-part2.ts'), content);
