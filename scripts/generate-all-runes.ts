...
function generateRune(item: any, type: string, subCategory: string = ""): any {
    ...
    if (item.effects && typeof item.effects === 'object') {
        effectDesc = item.effects.primary || (Array.isArray(item.effects) ? item.effects[0] : effectDesc);
    }
    ...
    return {
        ...
        tags: ["rune", "consumable", type.toLowerCase(), ... (item.tags || [])],
...
const MAPPINGS = [
    { src: 'spells/rank-d.ts', out: 'spell-rank-d.ts', var: 'spells_d', runeVar: 'spell_rank_d_runes', type: 'Spell', sub: 'Spell' },
    { src: 'spells/rank-c.ts', out: 'spell-rank-c.ts', var: 'spells_c', runeVar: 'runes_c', type: 'Spell', sub: 'Spell' },
    { src: 'spells/rank-b.ts', out: 'spell-rank-b.ts', var: 'spells_b', runeVar: 'runes_b', type: 'Spell', sub: 'Spell' },
    { src: 'spells/rank-a.ts', out: 'spell-rank-a.ts', var: 'spells_a', runeVar: 'runes_a', type: 'Spell', sub: 'Spell' },
    { src: 'spells/rank-s.ts', out: 'spell-rank-s.ts', var: 'spells_s', runeVar: 'runes_s', type: 'Spell', sub: 'Spell' },
    { src: 'techniques.ts', out: 'technique-techniques.ts', var: 'techniques', runeVar: 'technique_runes', type: 'Technique', sub: 'Technique' },
    { src: 'powers.ts', out: 'power-powers.ts', var: 'powers', runeVar: 'runes_power_powers', type: 'Power', sub: 'Power' },
];
...
async function run() {
    ...
    for (const map of MAPPINGS) {
        try {
            ...
            const runes = sourceItems.map(item => generateRune(item, map.type, map.sub));
            ...
            console.log(`  Successfully generated ${runes.length} runes -> ${map.out}`);
