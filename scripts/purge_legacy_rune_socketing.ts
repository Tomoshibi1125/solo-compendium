...
for (const hook of hooksToRemove) {
    ...
    const regex = new RegExp(`(//[a-zA-Z0-9\\s()\\-_'"]*\\s*)?export function ${hook}\\b[\\s\\S]*?(?=\\n// |\\nexport |$)`, 'g');
    content = content.replace(regex, '');
}

content = content.replace(/export \{ useCharacterRuneInscriptions as useRunes \};\n/g, '');
...
if (fs.existsSync(restSystemPath)) {
    ...
    restContent = restContent.replace(/\s*\/\/\s*Reset rune uses if applicable\s*try\s*\{\s*await\s*resetRuneUses\([^;]+;\s*\}\s*catch\s*\([^\{]+\{\s*logger\.error\([^;]+;\s*\/\/[^\n]+\n\s*\}/g, '');
    
    // Remove resetRuneUses function definition
    restContent = restContent.replace(/\/\*\*[\s\S]*?async function resetRuneUses[\s\S]*?\n\}\n/g, '');
    restContent = restContent.replace(/import \{ calculateRuneMaxUses \} from "\.\/runeAutomation";\n/g, '');
