...
if (arrayStart === -1) { console.error('Could not find spells array'); process.exit(1); }
...
const TEMPLATE_PATTERN = /A powerful \w+ spell that \w+ targets with devastating force/i;
...
const ELEMENT_DESCRIPTIONS = {
  shadow: ['darkness coalesces', 'shadows gather', 'void energy pulses', 'umbral power surges'],
  void: ['dimensional rift tears open', 'the void beckons', 'reality thins', 'entropy accelerates'],
  abyssal: ['abyssal energy erupts', 'demonic resonance builds', 'hellfire ignites', 'the abyss responds'],
  fire: ['flames erupt', 'heat distorts the air', 'embers spiral', 'infernal energy blazes'],
  ice: ['frost crystallizes', 'cold bites deep', 'permafrost spreads', 'glacial power flows'],
  lightning: ['electricity arcs', 'thunder rumbles', 'static crackles', 'voltage surges'],
  arcane: ['mana crystallizes', 'arcane sigils flare', 'magical weave tightens', 'raw power coalesces'],
  holy: ['divine light blazes', 'radiance pours forth', 'sacred energy surges', 'celestial power descends'],
  necrotic: ['death energy gathers', 'life force drains', 'decay spreads', 'entropy accelerates'],
  force: ['invisible force hammers', 'pure energy strikes', 'kinetic power releases', 'raw force detonates'],
  psychic: ['minds shatter', 'thought becomes weapon', 'consciousness fragments', 'mental energy spikes'],
};

const TYPE_VERBS = {
  Attack: ['strikes', 'blasts', 'assaults', 'devastates', 'hammers', 'shreds', 'pierces', 'rends'],
  Defense: ['shields', 'protects', 'wards', 'deflects', 'fortifies', 'guards', 'barriers against'],
  Utility: ['manipulates', 'alters', 'transforms', 'reshapes', 'channels', 'weaves', 'crafts'],
  Healing: ['mends', 'restores', 'rejuvenates', 'revitalizes', 'regenerates', 'purifies', 'heals'],
};

const RANK_POWER = {
  D: 'minor', C: 'moderate', B: 'significant', A: 'formidable', S: 'overwhelming',
};
...
function getElement(name) {
  ...
  if (lower.includes('fire') || lower.includes('flame') || lower.includes('infern') || lower.includes('blaze') || lower.includes('magma')) return 'fire';
  if (lower.includes('ice') || lower.includes('frost') || lower.includes('frozen') || lower.includes('cryo') || lower.includes('glacial')) return 'ice';
  if (lower.includes('lightning') || lower.includes('thunder') || lower.includes('storm') || lower.includes('volt') || lower.includes('electric')) return 'lightning';
  if (lower.includes('holy') || lower.includes('divine') || lower.includes('sacred') || lower.includes('celestial') || lower.includes('radiant')) return 'holy';
  if (lower.includes('death') || lower.includes('necro') || lower.includes('undead') || lower.includes('wither')) return 'necrotic';
  if (lower.includes('psychic') || lower.includes('mind') || lower.includes('mental') || lower.includes('psionic')) return 'psychic';
  if (lower.includes('force') || lower.includes('kinetic') || lower.includes('gravity')) return 'force';
  return 'arcane';
}
...
function generateDescription(spell) {
  ...
  const elemDescs = ELEMENT_DESCRIPTIONS[element] || ELEMENT_DESCRIPTIONS.arcane;
...
function generateEffect(spell) {
  const element = getElement(spell.name);
  const dmgType = element === 'shadow' ? 'necrotic' : element === 'void' ? 'force' : element === 'abyssal' ? 'fire' : element === 'holy' ? 'radiant' : element;
...
console.log(`Found ${templatedCount} templated entries, enriched ${enrichedCount}`);

// ── Write back ──────────────────────────────────────────────────────────
const output = header +
  'export const spells = [\n' +
  entries.map(e => '  ' + JSON.stringify(e, null, 2).split('\n').join('\n  ')).join(',\n') +
  '\n];\n';
