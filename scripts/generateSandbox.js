...
function createEnemyToken(
	...
	return {
		...
		imageUrl: imageOverride || \`/generated/compendium/anomalies/\${anomalyId}.webp\`,
	};
}
...
const mapConfigs = [
	{ name: "Hub: Ground Zero Safehouse", image: "hub_map.jpg", audio: "ambient_bunker.mp3", type: "hub" },
	{ name: "Region: The Ruined Subway", image: "subway_map.jpg", audio: "ambient_subway.mp3", type: "combat" },
	{ name: "Hub: The Lower City Bazaar", image: "bazaar_map.jpg", audio: "ambient_bunker.mp3", type: "hub" },
	{ name: "Region: Overgrown Botanical Sector", image: "overgrown_map.jpg", audio: "ambient_explore.mp3", type: "combat" },
	{ name: "Region: The Aetheric Sewer Network", image: "sewer_map.jpg", audio: "ambient_subway.mp3", type: "combat" },
	{ name: "Region: Abandoned Military Hospital", image: "hospital_map.jpg", audio: "ambient_explore.mp3", type: "combat" },
	{ name: "Region: Downtown Financial Ruins", image: "downtown_map.jpg", audio: "ambient_explore.mp3", type: "combat" },
	{ name: "Region: The Outer Slums Quarantine", image: "slums_map.jpg", audio: "ambient_explore.mp3", type: "combat" },
	{ name: "Region: The Inner Citadel approach", image: "citadel_map.jpg", audio: "ambient_combat.mp3", type: "combat" },
	{ name: "Region: The Throne of the Regent", image: "throne_map.jpg", audio: "ambient_boss.mp3", type: "boss" }
];

export const sandboxVTTScenes: (VTTScene & { audioTracks?: {url: string; name: string}[] })[] = mapConfigs.map((config, index) => {
	...
	if (config.type === "combat") {
		...
		tokens.push(createEnemyToken("final-boss", getRandomAnomalyId(), "The Regent", 25, 25, "huge"));
	} else {
		// Hub has some NPCs
		tokens.push(createEnemyToken(\`npc-\${index}-1\`, "anomaly-0010", "Quartermaster", 15, 15, "medium"));
		tokens.push(createEnemyToken(\`npc-\${index}-2\`, "anomaly-0012", "Guild Envoy", 20, 20, "medium"));
	}
	...
	return {
		...
		audioTracks: [{ url: \`/generated/compendium/sandbox_assets/\${config.audio}\`, name: config.name + " Audio" }],
		tokens: tokens,
		drawings: [], annotations: [], walls: [], lights: []
	};
});
