...
function generateLore(type, name) {
...
function _enrichPowerOrTechnique(pt, isPower) {
...
// FILE PROCESSING
// -------------------------------------------------------------
function _processFile(filePath, _type) {
...
async function superRegexEnrichment() {
    ...
    for (const f of filesToEnrich) {
        ...
        content = content.replace(nameRegex, (match, prefix, name, suffix) => {
            ...
            if (f.includes("power"))
                t = "Awakened Power";
            if (f.includes("technique"))
                t = "Combat Technique";
            if (f.includes("monster"))
                t = "Rift Entity";
