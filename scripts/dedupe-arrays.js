...
function arrayToTypeScriptExport(arrayName, data, headerDocs) {
    const jsonStr = JSON.stringify(data, null, "\t").replace(/"([^"]+)":/g, "$1:");
...
// 2. Dedupe Anomalies
const anomalyPath = path.join(process.cwd(), "src/data/compendium/anomalies/rank-d.ts");
...
fs.writeFileSync(anomalyPath, arrayToTypeScriptExport("anomalies_d", uniqueD, anomalyHeader));
