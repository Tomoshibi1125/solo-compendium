...
function arrayToTypeScriptExport(arrayName, data, headerDocs) {
    const jsonStr = JSON.stringify(data, null, "\t").replace(/"([a-zA-Z_$][0-9a-zA-Z_$]*)":/g, "$1:");
...
async function dedupeLocal(modulePath, exportName, header) {
    ...
    const jsonStr = JSON.stringify(unique, null, "\t").replace(/"([a-zA-Z_$][0-9a-zA-Z_$]*)":/g, "$1:");
