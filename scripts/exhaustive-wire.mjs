...
const wAliases = Array.from(typeAliases).sort((a, b) => {
	const na = parseInt(a.slice(1).split("_")[0], 10) || 0;
	const nb = parseInt(b.slice(1).split("_")[0], 10) || 0;
	return na - nb;
});
...
const matrixLines = wAliases.map((alias) => {
	if (alias.includes("FilterableQuery"))
		return `\t${alias}: ${alias}<unknown>;`;
	if (alias.includes("TablesInsert") || alias.includes("TablesUpdate"))
		return `\t${alias}: ${alias}<"characters">;`;
...
const effectLines = fAliases.map((alias) => {
...
console.log(
	`Successfully wired ${wAliases.length} types and ${fAliases.length} values.`,
);
