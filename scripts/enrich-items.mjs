...
const PARTS = ['items-part1.ts', 'items-part2.ts', 'items-part3.ts', 'items-part4.ts', 'items-part5.ts'];
...
function parseItemsFile(filePath) {
  ...
  for (let i = 0; i < arrayBody.length; i++) {
    ...
    if (ch === '{') { if (depth === 0) current = '{'; else current += ch; depth++; }
    else if (ch === '}') { depth--; current += ch; if (depth === 0) { try { entries.push(JSON.parse(current.replace(/,\s*}/g, '}').replace(/,\s*]/g, ']'))); } catch {} current = ''; } }
...
const WEAPON_DATA = {
  ...
  longsword: { damage: '1d8', damageType: 'slashing', versatile: '1d10', weight: 3 },
  shortsword: { damage: '1d6', damageType: 'piercing', finesse: true, weight: 2 },
  ...
  staff: { damage: '1d6', damageType: 'bludgeoning', versatile: '1d8', weight: 4 },
  ...
  trident: { damage: '1d6', damageType: 'piercing', versatile: '1d8', weight: 4 },
...
const RARITY_VALUE = { common: 50, uncommon: 500, rare: 5000, epic: 25000, legendary: 50000 };
const RARITY_WEIGHT_MULT = { common: 1, uncommon: 1, rare: 0.9, epic: 0.8, legendary: 0.7 };
...
const ELEMENTS = ['fire', 'ice', 'lightning', 'necrotic', 'radiant', 'force', 'psychic', 'thunder', 'poison', 'acid'];
...
function enrichItem(item) {
  ...
  if (!item.weight && item.weight !== 0) {
    if (item.type === 'weapon') item.weight = Math.round((getWeaponBase(item.name).weight || 3) * weightMult);
    else if (item.type === 'armor') item.weight = Math.round((getArmorBase(item.name).weight || 20) * weightMult);
  ...
  if (item.type === 'weapon' && !item.properties?.weapon) {
    ...
    item.properties = {
      ...
      weapon: {
        ...
        ...(base.versatile ? { versatile: bonus > 0 ? `${base.versatile} + ${bonus}` : base.versatile } : {}),
  ...
  if (!item.effects || (!item.effects.passive && !item.effects.active)) {
    const effectPool = item.type === 'weapon' ? EFFECTS_WEAPON
      : item.type === 'armor' ? EFFECTS_ARMOR
      : item.type === 'consumable' ? EFFECTS_CONSUMABLE
      : EFFECTS_ACCESSORY;
    ...
    for (let i = 0; i < numEffects; i++) {
      let idx;
      do { idx = Math.floor(Math.random() * effectPool.length); } while (usedIndices.has(idx) && usedIndices.size < effectPool.length);
      usedIndices.add(idx);
  ...
  if (item.attunement === undefined && ['rare', 'epic', 'legendary'].includes(item.rarity)) {
  ...
  if (item.description && /legendary \w+ forged in the shadow realm/i.test(item.description)) {
    const element = pick(ELEMENTS);
    const adjectives = ['ancient', 'enchanted', 'cursed', 'blessed', 'awakened', 'forged', 'crafted', 'imbued'];
    const origins = ['a fallen gate boss', 'a master artificer', 'the depths of an A-rank gate', 'a forgotten dungeon', 'a legendary hunter\'s collection'];
    item.description = `An ${pick(adjectives)} ${item.type} recovered from ${pick(origins)}. Infused with ${element} energy, it resonates with power when wielded by a worthy hunter. ${item.rarity === 'legendary' ? 'Only a handful exist in the known world.' : ''}`.trim();
  }
...
console.log(`Total: enriched ${totalEnriched} items across ${PARTS.length} files`);
