...
const inferDamageType = (name, effect) => {
    const text = `${name} ${effect}`.toLowerCase();
    if (text.includes("shadow") || text.includes("umbral"))
        return "necrotic";
    if (text.includes("void") || text.includes("abyss"))
        return "force";
...
const getAnomalyBaselineHP = (rank) => {
    switch (rank) {
        case "D":
            return 45;
        case "C":
...
const getAnomalyBaselineAC = (rank) => {
    switch (rank) {
        case "D":
            return 12;
        case "C":
...
const inferAnomalyScores = (anomalyType, rank) => {
...
const ensureSpellStructured = (spell) => {
    const out = {
        id: spell.id,
        name: spell.name,
    ...
    const legacyRangeValue = typeof spell.range === "number"
        ? spell.range
    ...
    const pickRand = (arr) => arr[Math.floor(rand() * arr.length)];
    ...
    const mechanicsOut = out.mechanics;
...
const ensureAnomalyStructured = (anomaly) => {
    const inferredScores = inferAnomalyScores(anomaly.type, anomaly.rank);
    const armorClass = typeof anomaly.stats?.armor_class === "number"
        ? anomaly.stats.armor_class
        : typeof anomaly.ac === "number"
            ? anomaly.ac
            : getAnomalyBaselineAC(anomaly.rank);
    const hitPoints = typeof anomaly.stats?.hit_points === "number"
        ? anomaly.stats.hit_points
        : typeof anomaly.hp === "number"
            ? anomaly.hp
            : getAnomalyBaselineHP(anomaly.rank);
    const crValue = typeof anomaly.stats?.challenge_rating === "number"
        ? anomaly.stats.challenge_rating
        : rankToCr(anomaly.rank);
    const prof = typeof anomaly.stats?.proficiency_bonus === "number"
        ? anomaly.stats.proficiency_bonus
        : crToProf(crValue);
    const out = {
        id: anomaly.id,
        name: anomaly.name,
        ...
        stats: {
            ability_scores: {
                strength: anomaly.stats?.ability_scores?.strength ?? inferredScores.strength,
                agility: anomaly.stats?.ability_scores?.agility ?? inferredScores.dexterity,
                vitality: anomaly.stats?.ability_scores?.vitality ??
                    inferredScores.constitution,
                intelligence: anomaly.stats?.ability_scores?.intelligence ??
                    inferredScores.intelligence,
                sense: anomaly.stats?.ability_scores?.sense ?? inferredScores.wisdom,
                presence: anomaly.stats?.ability_scores?.presence ?? inferredScores.charisma,
            },
    ...
    if (!actions || actions.length === 0) {
        ...
        const templated = abilityActions.slice(0, 3).map((name, idx) => {
            const actionDesc = idx === 0 ? pickRand(actionDescPrimary) : pickRand(actionDescSpecial);
        ...
        out.actions = templated;
    }
    return out;
};
const replaceExportedArray = (fileText, exportConstName, newArrayText) => {
...
const formatArray = (value) => JSON.stringify(value, null, 2);
async function main() {
    const args = new Set(process.argv.slice(2));
    const write = args.has("--write");
    const projectRoot = path.resolve(process.cwd());
    const spellsPath = path.join(projectRoot, "src", "data", "compendium", "spells.ts");
    const anomaliesPath = path.join(projectRoot, "src", "data", "compendium", "anomalies.ts");
    const enrichedSpells = rawSpells.map(ensureSpellStructured);
    const enrichedAnomalies = rawAnomalies.map(ensureAnomalyStructured);
    if (!write) {
    ...
    const anomaliesUpdated = replaceExportedArray(anomaliesText, "anomalies", formatArray(enrichedAnomalies));
