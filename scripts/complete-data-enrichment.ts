...
function getRandomStr(arr: string[]): string {
	return arr[Math.floor(Math.random() * arr.length)];
}

function getRandomStrArr(arr: string[][]): string[] {
	return arr[Math.floor(Math.random() * arr.length)];
}
...
function stringifyWithOrder(obj: Record<string, unknown>): string {
	return JSON.stringify(obj, null, "\t");
}

interface CompendiumLoreObj {
	origin: string;
	history: string;
	curse: string;
	personality: string;
...
for (const file of filesToProcess) {
	...
	for (const item of dataArray) {
		...
		for (const key of [
			...
			if (typeof rec[key] === "string") {
				rec[key] = sanitizeText(rec[key] as string);
			}
		}
		...
		if (rec.lore && typeof rec.lore === "string") {
			const oldstr = rec.lore as string;
			rec.lore = {
				origin: oldstr,
				history: "",
				curse: "",
	...
	const objectStrings = dataArray.map((obj) => {
		const str = stringifyWithOrder(obj as unknown as Record<string, unknown>);
		return str
			.split("\n")
			.map((line) => `\t${line}`)
			.join("\n");
