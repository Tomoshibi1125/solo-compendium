...
function processItems(arr: any[], isCombat: boolean) {
  return arr.map(obj => {
    ...
    if (!obj.lore.origin || obj.lore.origin === "") obj.lore.origin = rand(origins);
    if (!obj.lore.history || obj.lore.history === "") obj.lore.history = "Its true history remains a protected Guild secret.";
    if (!obj.lore.curse || obj.lore.curse === "") obj.lore.curse = isCombat ? "Slowly drains ambient stamina from the wielder." : "Emanates a faint aura of dread.";
    if (!obj.lore.personality || obj.lore.personality === "") obj.lore.personality = "Silent, waiting.";
    if (!obj.lore.current_owner || obj.lore.current_owner === "") obj.lore.current_owner = "Held by the Vanguard Guild.";
    if (!obj.lore.prior_owners || obj.lore.prior_owners.length === 0) obj.lore.prior_owners = ["A rogue Awakened"];
...
function dump(filepath: string, exportName: string, typeName: string, importPathStr: string | null, arr: any[], additionalHeader: string = '') {
