...
function generateMechanics(rankType: string, isCombat: boolean) {
  ...
  if (isCombat) {
    if (rankType === 'D') { dmgStr = rand(dmgD); dc = 12; }
    else if (rankType === 'C') { dmgStr = rand(dmgC); dc = 14; }
    else if (rankType === 'B') { dmgStr = rand(dmgB); dc = 16; }
    else if (rankType === 'A') { dmgStr = rand(dmgA); dc = 18; }
    else if (rankType === 'S') { dmgStr = rand(dmgS); dc = 22; }
    
    // Default tier mapping for unnamed
    if (rankType === 'power' || rankType === 'technique' || rankType === 'feat') { 
      dmgStr = rand(dmgC); dc = 15; 
    }
  }
...
function processArray(
  ...
  return dataOrigin.map((old, index) => {
    ...
    if (baseName.includes("Generic") || baseName.includes("Placeholder")) baseName = generateCustomName();
...
function dump(filename: string, exportName: string, typeName: string, importPathStr: string, arr: any[]) {
