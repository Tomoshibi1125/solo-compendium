...
async function generateWithAI(
	...
	const minifiedInput = batch.map((b) => ({
		...
		original_damage:
			b.mechanics?.damage_profile || b.mechanics?.attack?.damage || "",
		rank: b.rank || "",
		level: b.level || "",
	}));
	...
	for (let i = 0; i < dataArray.length; i += BATCH_SIZE) {
		process.stdout.write(
			"[" +
				...
				" of " +
				Math.ceil(dataArray.length / BATCH_SIZE) +
				"... ",
		);
				...
				if (generated) {
					...
					if (type === "sigils") {
						item.effect_description =
							generated.effect_description || item.effect_description;
						if (item.mechanics) {
						...
						item.effects.secondary =
							generated.secondary || item.effects.secondary;
						item.effects.tertiary = generated.tertiary || item.effects.tertiary;
					} else if (type === "feats") {
						if (!item.benefits) item.benefits = {};
						...
						item.discovery_lore =
							generated.discovery_lore || item.discovery_lore;
						item.effect_description =
							generated.effect_description || item.effect_description;
						...
						if (
							...
							item.effects = [generated.primary, generated.secondary].filter(
								Boolean,
							);
						}
					}
		...
		console.log(
			aiData ? "Success." : "Failed after retries, keeping original.",
		);
...
async function run() {
	...
	const targets = [
		...
		},
		{
			...
			name: "tattoos",
			type: "tattoos",
			header: "import type { CompendiumTattoo } from '@/types/compendium';",
		},
		{
			file: "src/data/compendium/feats-comprehensive.ts",
			name: "comprehensiveFeats",
			type: "feats",
			header: "import type { CompendiumFeat } from '../../types/compendium';",
		},
		...
		{
			file: "src/data/compendium/runes/spell-rank-s.ts",
			name: "runes_s",
			type: "runes",
			header:
		...
		{
			file: "src/data/compendium/runes/power-powers.ts",
			name: "runes_powers",
			type: "runes",
			header:
				"import type { CompendiumRune } from '../../../types/compendium';",
		...
		{
			file: "src/data/compendium/runes/technique-techniques.ts",
			name: "runes_techniques",
			type: "runes",
			header:
				"import type { CompendiumRune } from '../../../types/compendium';",
		},
