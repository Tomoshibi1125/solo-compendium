...
const migrateFile = (filePath: string) => {
  ...
  if (content.includes('activation_type_migrated:')) {
    content = content.replace(/activation_type_migrated:\s*"(.*?)"/g, 'activation: { type: "$1" }');
    changed = true;
  }
  
  // 3. Special handler for range_migrated -> range object
  if (content.includes('range_migrated:')) {
    content = content.replace(/range_migrated:\s*"(.*?)"/g, 'range: { type: "$1" }');
    changed = true;
  }
  ...
  if (content.includes('armor_class_migrated:') || content.includes('hit_points_migrated:')) {
  ...
  content = content.replace(/activation:\s*"(.*?)"/g, 'activation: { type: "$1" }');
