...
if (reportContent.charCodeAt(0) === 0xFEFF || reportContent.charCodeAt(0) === 0xFFFE) {
...
if (startIdx !== -1) {
    try {
        ...
        console.error("Failed to parse JSON even after finding known start signature.");
    }
} else {
    // Try to just take everything after the first newline
    try {
        const afterNewline = reportContent.substring(reportContent.indexOf('\n') + 1).trim();
...
issues.forEach(issue => {
    if (issue.type === 'exports' || issue.type === 'types' || issue.type === 'duplicates') {
...
fs.writeFileSync('knip_clean_data.json', JSON.stringify(itemsToFix, null, 2), 'utf8');
