...
function findBestMatches(regex: RegExp, limitPerLevel: number = 2) {
	...
	for (const spell of sortedSpells) {
		if (
			spell.level >= 1 &&
			spell.level <= 5 &&
			matches[spell.level].length < limitPerLevel
		) {
...
// Herald
newGrants += generateGrant(
	"Herald",
	"Path of the Restoration Mandate",
	/heal|cure|mend|revive|restore|vitality|life|bless/,
	["Herald"],
);
newGrants += generateGrant(
	"Herald",
	"Path of the Combat Mandate",
	/strike|weapon|war|battle|smite|shield|armor/,
	["Herald", "Mage"],
);
newGrants += generateGrant(
	"Herald",
	"Path of the Triage Mandate",
	/death|grave|necrotic|false life|vampiric|decay|wither|doom/,
	["Revenant"],
);
...
newGrants += generateGrant(
	...
	/dream|sleep|charm|fey|illusion|mind|peace/,
	["Idol", "Esper"],
);
newGrants += generateGrant(
	"Summoner",
	"Path of the Pack Commander",
	/beast|pack|animal|summon|wolf|hunt/,
	["Summoner"],
);
newGrants += generateGrant(
	"Summoner",
	"Path of the Symbiotic Host",
	/spore|poison|disease|decay|fungus|cloudkill/,
	["Revenant", "Mage"],
);
newGrants += generateGrant(
	"Summoner",
	"Path of the Cosmic Conduit",
	/star|moon|sun|radiant|cosmic|divination|meteor/,
	["Herald", "Mage"],
);
...
newGrants += generateGrant(
	...
	/blade|smite|shield|blur|weapon|strike|curse/,
	["Mage", "Destroyer"],
);
newGrants += generateGrant(
	"Contractor",
	"Path of the Deep Dweller",
	/water|ocean|sea|cold|ice|tentacle|drown/,
	["Mage"],
);
...
newGrants += generateGrant(
	...
	/dragon|fire|fly|breath|wing|elemental|scale/,
	["Mage"],
);
newGrants += generateGrant(
	"Esper",
	"Path of the Void Resonance",
	/shadow|dark|blind|stealth|void|night|gloom/,
	["Mage", "Revenant"],
);
newGrants += generateGrant(
	"Esper",
	"Path of the Tempest Core",
	/storm|thunder|lightning|wind|tempest|shock/,
	["Mage"],
);
newGrants += generateGrant(
	"Esper",
	"Path of the Psionic Breach",
	/mind|psychic|telepath|telekinesis|charm|brain/,
	["Esper"],
);
...
newGrants += generateGrant(
	...
	/fear|fright|terror|doom|undead|control/,
	["Contractor", "Mage"],
);
newGrants += generateGrant(
	"Revenant",
	"Path of the Entropy Drinker",
	/blood|drain|vampire|life steal|siphon/,
	["Revenant"],
);
newGrants += generateGrant(
	"Revenant",
	"Path of the Wither Guard",
	/blight|wither|decay|disease|rot|necrotic/,
	["Revenant"],
);
newGrants += generateGrant(
	"Revenant",
	"Path of the Plague Weaver",
	/plague|poison|disease|contagion|sickness|toxic/,
	["Revenant", "Summoner"],
);
...
newGrants += generateGrant(
	...
	/fear|command|hold|conquest|dominate|crush/,
	["Contractor", "Esper"],
);
newGrants += generateGrant(
	"Holy Knight",
	"Path of the Atonement Mandate",
	/peace|sanctuary|calm|sleep|protect|ward/,
	["Herald"],
);
newGrants += generateGrant(
	"Holy Knight",
	"Path of the Exaltation Mandate",
	/glory|hero|enhance|haste|speed|brave|courage/,
	["Herald", "Idol"],
);
...
newGrants += generateGrant(
	...
	/shadow|dark|stealth|gloom|fear|hide|invisible/,
	["Mage"],
);
newGrants += generateGrant(
	"Stalker",
	"Path of the Apex Slayer",
	/protect|zone|truth|banish|monster|slay|hunt/,
	["Herald", "Mage"],
);
newGrants += generateGrant(
	"Stalker",
	"Path of the Hive Synchronist",
	/swarm|web|insect|bug|fly|gaseous/,
	["Summoner", "Mage"],
);
...
newGrants += generateGrant(
	...
	/armor|shield|missile|thunder|shatter|construct/,
	["Mage"],
);
newGrants += generateGrant(
	"Technomancer",
	"Design: Resonance Siege",
	/siege|blast|fire|wall|force|explosion|artillery/,
	["Mage"],
);
newGrants += generateGrant(
	"Technomancer",
	"Design: Synchronist Binary",
	/hero|ward|bond|smite|vitality|construct|protect/,
	["Herald"],
);
