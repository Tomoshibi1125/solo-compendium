...
function getMechanics(name, rank, isUtility) {
	...
	if (isUtility) {
		const effects = {
			Blood: "blinded by erupting ichor.",
			Solar: "burned, taking half damage at start of next turn.",
			Void: "pulled 10ft into the entropy field.",
			Storm: "paralyzed until end of next turn.",
			Glacial: "restrained by creeping frost.",
			Aetheric: "pushed 15ft by dimensional pressure.",
			Titanic: "knocked prone by seismic impact.",
			Absolute: "stunned until the end of its next turn.",
		};
		return {
			attack: {
				mode: "ranged",
				resolution: "spell_attack",
				damage: { dice: count - 2 + die, type: dmgType },
			},
			saving_throw: {
				ability: "CON",
				dc: "spell_save",
				failure: effects[theme],
			},
		};
	}
	return {
		attack: {
			mode: "ranged",
			resolution: "spell_attack",
			damage: { dice: count + die, type: dmgType },
			critical: true,
		},
	};
}
...
function enrichFile(filePath, rank) {
	...
	const regex =
		/\{\s+id:\s+"(spell|power|tech)-.*?",\s+name:\s+"(.*?)"[\s\S]*?mechanics:\s+\{[\s\S]*?\},\s+\}/g;
	...
	content = content.replace(regex, (match, type, name) => {
		...
		const desc =
			"A specialized manifestation of " +
			theme +
			" Resonance. This " +
			(type === "spell" ? "technique" : "form") +
			" allows the caster to weave the " +
			name +
			" into an Absolute Decree.";

		let newBlock = match.replace(
			/description:\s+(?:".*?"|`.*?`|\n\s+".*?")/,
...
["rank-a.ts", "rank-b.ts", "rank-c.ts", "rank-d.ts"].forEach((f) =>
	enrichFile(
		path.join(baseDir, "spells", f),
		f.split("-")[1].charAt(0).toUpperCase(),
	),
);
